...
class Attendance_cal_model extends CI_Model {
    ...
    COUNT(case when attend_pa_id = 0 then 1 end) absent_count');
        ...
        if($this->session->userdata('miconlogin') && is_array($this->session->userdata['miconlogin']) && !empty($this->session->userdata['miconlogin']) && isset($this->session->userdata['miconlogin']['typeid']) && ($this->encrypt_decrypt('decrypt',$this->session->userdata['miconlogin']['typeid']) != 3) && ($this->encrypt_decrypt('decrypt',$this->session->userdata['miconlogin']['dep_id']) != 2) &&  isset($this->session->userdata['miconlogin']['userid']) && ($this->session->userdata['miconlogin']['userid'] != ''))
        {
        ...
        if($this->input->get('exe') && ($this->input->get('exe') != ''))
		{
			$exe = $this->input->get('exe');
			$this->db->where('attend_auid',$exe);
		}
	...
	public function get_Salary_data()
	{
		...
		{
			$stdate = $this->input->get('inq_start_date');
			$this->db->where('attend_date_name >=',$stdate);
		}
		if($this->input->get('inq_end_date') && ($this->input->get('inq_end_date') != ''))
		{
			$stdate = $this->input->get('inq_end_date');
			$this->db->where('attend_date_name <=',$stdate);
		}
		if($this->input->get('exe') && ($this->input->get('exe') != ''))
		{
