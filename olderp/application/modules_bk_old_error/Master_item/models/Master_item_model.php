...
class Master_item_model extends CI_Model {
	...
	public function add($data)
	{
		...
		$item = array(
			...
			'master_item_modal_code' => $data['master_item_modal_code'],
			'master_item_keyword' => $data['master_item_keyword'],
			'master_item_description' => $data['master_item_description'],
			'master_item_stock' => $data['master_item_stock'],
			'master_item_ldescription' => $data['master_item_description'],
	...
	public function importcsv($data)
	{
		...
		$this->db->where('hsn_hcode',$data['HSN CODE']);
		$query = $this->db->get();
		$hsn_data = $query->row_array();
		//echo '<pre>';print_r($hsn_data);die;

		$this->db->select('*');
		$this->db->from('tbl_master_item_unit');
		//$this->db->Like('master_item_unit_name',$data['ITEM UNIT']);
		$this->db->Like('UPPER(master_item_unit_name)', strtoupper($data['ITEM UNIT']),'both',false);
		$query = $this->db->get();
		$unit_data = $query->row_array();
		//echo '<pre>';print_r($unit_data);die;
		

		if($hsn_data != 0 && $unit_data != 0)
		{
			$item = array(
			'master_item_name' => $data['ITEM NAME'],
			'master_item_modal_code' => $data['MODEL CODE'],
			'master_item_keyword' => $data['KEY WORD'],
			'master_item_description' => $data['ITEM DESCRIPTION'],
			'master_item_stock' => $data['STOCK'],
			'master_item_ldescription' => $data['ITEM DESCRIPTION'],
			//'master_item_make' => $vendor_data['master_vendor_id'],
			'master_item_make_name' => $data['MAKE'],
			'master_item_part_no' => isset($data['PART NO.']) && ($data['PART NO.'] != '') ? $data['PART NO.'] : url_title(convert_accented_characters($data['ITEM NAME'].'-'.$data['MODEL CODE']), 'dash', TRUE),
			'master_item_rate' => $data['PRICE'],
			'master_item_unit' => $unit_data['master_item_unit_id'],
			'master_item_weight' => $data['ITEM WEIGHT'],
			'master_item_hsncode' => $hsn_data['hsn_id'],
			'master_item_tax' => $data['TAX'],
			...
			'master_item_ldescription' => $data['ITEM DESCRIPTION'],
			'master_item_make_name' => $data['MAKE'],
			'master_item_part_no' => url_title(convert_accented_characters($data['ITEM NAME'].'-'.$data['MODEL CODE']), 'dash', TRUE),
			'master_item_rate' => $data['PRICE'],
			'master_item_unit' => $unitid,
			'master_item_weight' => $data['ITEM WEIGHT'],
			'master_item_hsncode' => $hsnid,
			'master_item_tax' => $data['TAX'],
			'master_item_created_date' => date('Y-m-d H:i:s'),
			'master_item_updated_date' => date('Y-m-d H:i:s')
			);
	...
	public function copy_item($id)
	{
		...
		$query = $this->db->get();
		if($query->num_rows() == 1){
			$itemdatas = $query->row_array();
			$set = 0; $j = 0;
			while ($set != 1) { $j++;
				$itemdatas['master_item_part_no'] = $itemdatas['master_item_part_no'].$j;
				$this->db->select('*');
				$this->db->from('tbl_master_item');
				$this->db->where('master_item_part_no',$itemdatas['master_item_part_no']);
				$query = $this->db->get();
				if($query->num_rows() == 0){
					$set = 1;
				}
			}
			unset($itemdatas['master_item_id']);

			$this->db->insert('tbl_master_item',$itemdatas);
			$result['status'] = true;
	...
	{ 
		...
		'master_item_modal_code' => $data['master_item_modal_code'],
			...
			'master_item_description' => $data['master_item_description'],
			'master_item_keyword' => isset($data['master_item_keyword']) ? $data['master_item_keyword'] : '',
			'master_item_location' => isset($data['master_item_location']) ? $data['master_item_location'] : '',
			'master_item_stock' => $data['master_item_stock'],
			'master_item_ldescription' => isset($data['master_item_ldescription']) ? $data['master_item_ldescription'] : '',
			'master_item_tags' => isset($data['master_item_tags']) ? $data['master_item_tags'] : '',
			'master_item_make' => $data['master_item_make'],
			...
			'master_item_weight' => $data['master_item_weight'],
			'master_item_dimension' => isset($data['master_item_dimension']) ? $data['master_item_dimension'] : '',
			'master_item_hsncode' => $data['master_item_hsn'],
			'master_item_tax' => $data['master_item_Tax'],
			'master_item_limit' => $data['master_item_limit'],
			'master_item_updated_date' => $data['master_item_updated_date']
...
*/
	...
	public function get_master_items($Start,$end)
	{
		...
		$this->db->select('tbl_master_item.*,tbl_hsn_code.*,tbl_master_item_unit.*,tbl_master_party.*,(select ROUND(SUM(tcredit.tran_itm_qty),2) as tcreditpoints FROM tbl_transaction as tcredit WHERE tcredit.tran_cr_or_dr = 1 AND tcredit.tran_itm_id = tbl_master_item.master_item_id AND tcredit.tran_is_hold = '."'0'".' ) as tcreditpoints,(select ROUND(SUM(tdebit.tran_itm_qty),2) as tdebitpoints FROM tbl_transaction as tdebit WHERE tdebit.tran_cr_or_dr = 2 AND tdebit.tran_itm_id = tbl_master_item.master_item_id AND tdebit.tran_is_hold = '."'0'".' ) as tdebitpoints');
		$this->db->from('tbl_master_item');
		...
		if($this->input->post('master_item_name') && ($this->input->post('master_item_name') != ''))
        {
           $this->db->like('master_item_name', $this->input->post('master_item_name'));   
        }
   	   if($this->input->post('master_item_modal_code') && ($this->input->post('master_item_modal_code') != ''))
        {
           $this->db->like('master_item_modal_code', $this->input->post('master_item_modal_code'));   
        }
         if($this->input->post('master_item_keyword') && ($this->input->post('master_item_keyword') != ''))
        {
           $this->db->like('master_item_keyword', $this->input->post('master_item_keyword'));   
        }
        if($this->input->post('master_item_part_no') && ($this->input->post('master_item_part_no') != ''))
        {
           $this->db->like('master_item_part_no', $this->input->post('master_item_part_no'));   
        }
          
        if($this->input->post('master_item_description') && ($this->input->post('master_item_description') != ''))
        {
           $this->db->like('master_item_description', $this->input->post('master_item_description'));   
        }
   
    	 if($this->input->post('hsn_hcode') && ($this->input->post('hsn_hcode') != ''))
        {
           $this->db->where('hsn_hcode', $this->input->post('hsn_hcode'));   
        }
        if($this->input->post('master_item_make_name') && ($this->input->post('master_item_make_name') != ''))
        {
           $this->db->like('master_item_make_name', $this->input->post('master_item_make_name'));   
        }
        ...
        if($this->input->post('master_item_weight') && ($this->input->post('master_item_weight') != ''))
        {
           $this->db->where('master_item_weight', $this->input->post('master_item_weight'));   
        }
        if($this->input->post('master_item_rate') && ($this->input->post('master_item_rate') != ''))
        {
           $this->db->where('master_item_rate', $this->input->post('master_item_rate'));   
        }
        if($this->input->post('master_item_tax') && ($this->input->post('master_item_tax') != ''))
        {
           $this->db->where('master_item_tax', $this->input->post('master_item_tax'));   
        }
