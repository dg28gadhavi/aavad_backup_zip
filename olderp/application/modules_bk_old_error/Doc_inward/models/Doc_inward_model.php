...
class Doc_inward_model extends CI_Model {
	...
	{   
		if($this->uri->segment(3))
		{
			$id = $this->encrypt_decrypt('decrypt',$this->uri->segment(3));
   			$item = array(
			//'doc_inward_code' => $data['doc_inward_code'],
			'doc_inward_srno' => $data['doc_inward_srno'],
			'doc_inward_partyid' => $data['doc_inward_partyid'],
			'doc_inward_vendorid' => $data['doc_inward_vendorid'],
			'doc_inward_docketno' => $data['doc_inward_docketno'],
			'doc_inward_courier_name' => $data['doc_inward_courier_name'],
			'doc_inward_detail' => $data['doc_inward_detail'],
			//'inw_lr_date' => date("Y-m-d", strtotime($data['inw_lr_date'])),
			'doc_inward_desc' => $data['doc_inward_desc'],
			'doc_inw_isdelete' => '0',
			'doc_inward_remark' => $data['doc_inward_remark'],
			...
			//'doc_inward_created_date' => $data['doc_inward_created_date'],
			'doc_inward_updated_date' => $data['doc_inward_updated_date']
			);
			//echo '<pre>';print_r($item);die;
		$this->db->where('doc_inward_id', $id);
		$this->db->update('tbl_doc_inward',$item); 
		...
		$lid = $id;
			...
			//echo '<pre>';print_r($item);die;
		$this->db->insert('tbl_doc_inward',$item);
		$lid = $this->db->insert_id();
		$update = array('doc_inward_code' => 'DINW'.$lid);
		$this->db->where('doc_inward_id',$lid);
		$this->db->update('tbl_doc_inward',$update);
	...
	public function other_add($data)
	{
		...
		$item = array(
			...
			'Doc_inward_sub' => $data['Doc_inward_sub'],
			'Doc_inward_desc' => $data['Doc_inward_desc'],
			'Doc_inward_tc_price' => $data['Doc_inward_tc_price'],
			'Doc_inward_tc_wrnty' => $data['Doc_inward_tc_wrnty'],
			'Doc_inward_tc_pf' => $data['Doc_inward_tc_pf'],
			'Doc_inward_tc_deli' => $data['Doc_inward_tc_deli'],
			'Doc_inward_tc_paynt' => $data['Doc_inward_tc_paynt'],
			'Doc_inward_tc_ovali' => $data['Doc_inward_tc_ovali'],
			'Doc_inward_tc_frght' => $data['Doc_inward_tc_frght'],
			'Doc_inward_tc_gst' => $data['Doc_inward_tc_gst'],
	...
	public function item_add($data)
	{
		...
		$item1 = array(
				...
				$this->db->insert('tbl_doc_inward_item',$item1);
				$item_lid = $this->db->insert_id();

				$this->db->select('SUM(inwi_ftotal) as count');
        		$this->db->from('tbl_doc_inward_item');
        		$this->db->where('inwi_isdelete','0');
        		$this->db->where('inwi_doc_inward_id',$lid);
        		$query = $this->db->get();
  ...
  //       echo "<pre>"; print_r($wo_stock); die;
		...
		if($creditcutstock >= 0 && $data['inwi_qty'] > 0)
		{
			...
			$item1 = array(
						...
						//echo '<pre>'; print_r($item1); die;
					$this->db->where('inwi_id',$sqiitemid);
					$this->db->update('tbl_doc_inward_item',$item1);
	...
	public function get($id)
	{
		...
		$this->db->select('*');
		$this->db->from('tbl_doc_inward');
		$this->db->join('tbl_master_party','tbl_doc_inward.doc_inward_partyid = tbl_master_party.master_party_id');
		$this->db->where('doc_inward_id',$id);
		$this->db->where('doc_inw_isdelete','0');
		//$this->db->where('sq_cid',$this->session->userdata['login']['aus_Id']);
		$query = $this->db->get();
	...
	public function get_edit_items()
	{
		...
		$this->db->select('*');
		$this->db->from('tbl_doc_inward_item');
		//$this->db->join('tbl_master_party','tbl_doc_inward.vendor = tbl_master_party.master_party_id');
		$this->db->where('sqi_Doc_inward_id ',$id);
		//$this->db->where('sq_cid',$this->session->userdata['login']['aus_Id']);
		$query = $this->db->get();
		//echo "<pre>";print_r($query->result_array());die;
		return $query->result_array();
	...
	public function get_Doc_inward()
	{
		
		$this->db->select('*');//sum(tbl_doc_inward.inv_grand_total) as grand_total_view
		$this->db->from('tbl_doc_inward');
		$this->db->where('dis_isdelete',0);
		$this->db->order_by('tbl_doc_inward.dis_id','DESC');
  ...
  //       }
	...
	public function get_Doc_inward_report()
	{
		die;
		$this->db->select('*');//sum(tbl_doc_inward.inv_grand_total) as grand_total_view
		$this->db->from('tbl_doc_inward');
		$this->db->where('sq_isdeleted',0);
		$this->db->join('tbl_mode_inquiry','tbl_doc_inward.sq_mode_inq = tbl_mode_inquiry.mode_inquiry_id');
		$this->db->join('tbl_admin_users','tbl_doc_inward.sq_end_st = tbl_admin_users.aus_Id');
		//$this->db->where('sq_cid',$this->session->userdata['login']['aus_Id']);
		$this->db->order_by('tbl_doc_inward.sq_id','DESC');
		if($this->input->get('party')){
			$this->db->where('tbl_doc_inward.sq_id', $this->input->get('party'));
		}
	...
	public function sales_inq_report()
	{
		...
		$this->db->from('tbl_doc_inward');
		$this->db->join('tbl_master_party','tbl_doc_inward.doc_inward_partyid = tbl_master_party.master_party_id');
		$this->db->join('tbl_master_vendor','tbl_doc_inward.doc_inward_vendorid = tbl_master_vendor.master_vendor_id');
		$this->db->where('doc_inw_isdelete','0');
		if($this->session->userdata('miconlogin') && is_array($this->session->userdata['miconlogin']) && !empty($this->session->userdata['miconlogin']) && isset($this->session->userdata['miconlogin']['typeid']) && ($this->encrypt_decrypt('decrypt',$this->session->userdata['miconlogin']['typeid']) != 3) &&  isset($this->session->userdata['miconlogin']['userid']) && ($this->session->userdata['miconlogin']['userid'] != ''))
		{
			...
			{
				//$this->db->where_in('tbl_doc_inward.doc_inward_id', $this->session->userdata['miconlogin']['all_users']);
				//$this->db->where('tbl_doc_inward.sq_cid', $this->encrypt_decrypt('decrypt',$this->session->userdata['miconlogin']['userid']));
			}else{
				//$this->db->where('tbl_doc_inward.doc_inward_id', $this->encrypt_decrypt('decrypt',$this->session->userdata['miconlogin']['userid']));
			}
		}
		if($this->input->get('doc_inward_created_date') && ($this->input->get('doc_inward_created_date') != ''))
		{
			//echo "<pre>";print_r($this->input->get('doc_inward_created_date'));die;
			$stdate = date("Y-m-d", strtotime($this->input->get('doc_inward_created_date')));
			$this->db->where('doc_inward_created_date >=',$stdate);
		}
		if($this->input->get('doc_inward_updated_date') && ($this->input->get('doc_inward_updated_date') != ''))
		{
			$stdate = date("Y-m-d", strtotime($this->input->get('doc_inward_updated_date')));
			$this->db->where('doc_inward_created_date <=',$stdate);
		}
	
		if($this->input->post('doc_inward_code') && ($this->input->post('doc_inward_code') != ''))
        {
           $this->db->like('doc_inward_code', $this->input->post('doc_inward_code'));   
        }
        if($this->input->post('doc_inward_srno') && ($this->input->post('doc_inward_srno') != ''))
        {
           $this->db->like('doc_inward_srno', $this->input->post('doc_inward_srno'));   
        }
        if($this->input->post('doc_inward_courier_name') && ($this->input->post('doc_inward_courier_name') != ''))
        {
           $this->db->like('doc_inward_courier_name', $this->input->post('doc_inward_courier_name'));   
        }
         if($this->input->post('doc_inward_partyid') && ($this->input->post('doc_inward_partyid') != ''))
        {
           $this->db->like('master_party_name', $this->input->post('doc_inward_partyid'));   
        }
         if($this->input->post('doc_inward_docketno') && ($this->input->post('doc_inward_docketno') != ''))
        {
           $this->db->like('doc_inward_docketno', $this->input->post('doc_inward_docketno'));   
        }
         if($this->input->post('doc_inward_detail') && ($this->input->post('doc_inward_detail') != ''))
        {
           $this->db->like('doc_inward_detail', $this->input->post('doc_inward_detail'));   
        }
         if($this->input->post('doc_inward_remark') && ($this->input->post('doc_inward_remark') != ''))
        {
           $this->db->like('doc_inward_remark', $this->input->post('doc_inward_remark'));   
        }
       
        $this->db->order_by('doc_inward_id','DESC');
        //$this->db->order_by('doc_inward_srno','DESC');
		$query = $this->db->get();
		//echo "<pre>";print_r($query->result_array());die;
		return $query->result_array();
	}
	...
	{
		//echo "total"; die;
	  	$this->db->select('count(DISTINCT doc_inward_id) as total');
		$this->db->from('tbl_doc_inward');
		$this->db->where('doc_inw_isdelete','0');
		$query = $this->db->get();
		...
		//$this->db->select('SUM(tbl_doc_inward_item.inwi_stock) as stock');
	...
	{
		$this->db->select('SUM(tbl_doc_inward_itax.sqit_tax_amount) as total,UPPER(tbl_doc_inward_itax.sqit_tax_name) as tax_name, tbl_doc_inward_itax.*');
		//SUM(IF(itmtax_tax_name = 'CGST', itmtax_tax_amount, 0)) AS 'CGST',
		$this->db->from('tbl_doc_inward_itax');
	...
	{
		$this->db->select('*');
		$this->db->from('tbl_doc_inward_btax');
	...
	public function get_pdfdata($lid)
	{
		...
		$this->db->select('*');
		$this->db->from('tbl_doc_inward');
		$this->db->where('tbl_doc_inward.doc_inward_id',$myid);
		$query = $this->db->get();
		...
		$this->db->select('*');
		$this->db->from('tbl_doc_inward');
		$this->db->join('tbl_doc_inward_item','tbl_doc_inward.doc_inward_id = tbl_doc_inward_item.inwi_doc_inward_id','left');
		$this->db->where('tbl_doc_inward.doc_inward_id',$myid);
		$this->db->where('tbl_doc_inward_item.inwi_isdelete',0);
		$query = $this->db->get();
	...
	public function get_items($id)
	{
		$this->db->select('*');
		$this->db->from('tbl_doc_inward_item');
		//$this->db->join('tbl_hsn_code','tbl_hsn_code.hsn_id = tbl_doc_inward_item.sqi_itm_hsncode','left');
		//$this->db->join('tbl_master_item','tbl_master_item.master_item_id = tbl_doc_inward_item.sqi_itm_pno','left');
		$this->db->where('inwi_doc_inward_id',$id);
		$this->db->where('inwi_isdelete',0);
	...
	public function get_new_data()
	{
		...
		foreach ($data as $key => $value) 
		{
			...
			$this->db->where('tbl_master_party_old.master_party_id',$value['doc_inward_partyid']);
			$query = $this->db->get();

			if($query->num_rows()==1)
			{
				$party_old = $query->row_array();

				$this->db->select('*');
				$this->db->from('tbl_master_party');
				$this->db->where('tbl_master_party.master_party_name',$party_old['master_party_name']);
				$query = $this->db->get();

				if($query->num_rows()==1)
				{
					$party_new = $query->row_array();
					$new_cust_id = $party_new['master_party_id'];
					$update =array('doc_inward_partyid'=>$new_cust_id);

					$this->db->where('doc_inward_id', $value['doc_inward_id']);
					$this->db->update('tbl_doc_inward',$update);
				...
				{
					unset($party_old['master_party_id']);
					$party_old['master_party_gst'] = $party_old['master_party_gst_no'];
					unset($party_old['master_party_gst_no']);
					$this->db->insert('tbl_master_party',$party_old);
	...
	public function copy_all_master_party(){
		$this->db->select('*');
		$this->db->from('tbl_master_party_old');
		$query = $this->db->get();
	...
	public function add_sqoute($id)
	{
		...
		$item = array(
			...
			'sale_quotation_term' => $item['pdetails']['Doc_inward_desc'],
		...
		$this->db->select('*');
		$this->db->from('tbl_doc_inward_item');
		$this->db->where('sqi_Doc_inward_id',$this->input->get('id'));
		$this->db->where('sqi_is_bom !=',1);
		$query = $this->db->get();
		...
		$this->db->select('*');
		$this->db->from('tbl_doc_inward_bom');
	...
	public function Doc_inward_report()
	{
		$this->db->select('*');
		$this->db->from('tbl_doc_inward');
		//$this->db->join('tbl_master_party', 'tbl_master_party.master_party_id = tbl_doc_inward.vendor');
		if($this->input->get('party')){
			$this->db->where('tbl_doc_inward.sq_id', $this->input->get('party'));
		}
		if($this->input->get('created_start_date') && ($this->input->get('created_start_date') != ''))
	...
	//**************************************************End customer wise
		// $this->db->select('*');
		// $this->db->from('tbl_doc_inward_item');
		// $this->db->join('tbl_master_item', 'tbl_master_item.master_item_id = tbl_doc_inward_item.sqi_Doc_inward_id');
		// $this->db->join('tbl_doc_inward', 'tbl_doc_inward.sq_id = tbl_doc_inward_item.sqi_Doc_inward_id');
		// if($this->input->get('items')){
		// 	$this->db->where('tbl_doc_inward_item.sqi_Doc_inward_id', $this->input->get('items'));
		// }
		// if($this->input->get('created_start_date') && ($this->input->get('created_start_date') != ''))
		// {
		// 	$stdate = date("Y-m-d", strtotime($this->input->get('created_start_date')));
	...
	{
		$this->db->select('tbl_doc_inward.sq_id,DATE_FORMAT(tbl_doc_inward.sq_enq_date, "%d-%m-%Y"),tbl_doc_inward.vendor,tbl_doc_inward.sq_con_person,tbl_doc_inward.sq_mobile,tbl_doc_inward.sq_email,tbl_master_city.city_name,tbl_master_item.master_item_part_no,tbl_doc_inward_item.sqi_itm_qty,tbl_doc_inward_item.sqi_itm_price,tbl_doc_inward_item.sqi_itm_total,tbl_doc_inward.sq_end_st,source.source_cat_name as sourcename,tbl_doc_inward.sq_remarks');//sum(tbl_doc_inward.inv_grand_total) as grand_total_view
		$this->db->from('tbl_doc_inward');
		$this->db->join('tbl_doc_inward_item','tbl_doc_inward_item.sqi_Doc_inward_id = tbl_doc_inward.sq_id','left');
		$this->db->join('tbl_master_item','tbl_master_item.master_item_id = tbl_doc_inward_item.sqi_itm_pno','left');
		$this->db->join('tbl_mode_inquiry','tbl_doc_inward.sq_mode_inq = tbl_mode_inquiry.mode_inquiry_id','left');
		$this->db->join('tbl_country','tbl_doc_inward.sq_country = tbl_country.country_id','left');
		$this->db->join('tbl_master_city','tbl_doc_inward.sq_city = tbl_master_city.city_id','left');
		$this->db->join('tbl_master_state','tbl_doc_inward.sq_state = tbl_master_state.state_id','left');
		$this->db->join('tbl_sale_brand','tbl_doc_inward.sq_id = tbl_sale_brand.sqb_sqid','left');
		//$this->db->join('tbl_sale_quotation_brand','tbl_doc_inward.sa_id = tbl_sale_quotation_brand.sab_sq_id','left');
		$this->db->join('tbl_source_cat as source','tbl_doc_inward.sq_source_cat = source.source_cat_id');
		// $this->db->join('tbl_source_cat as subsource','tbl_doc_inward.sa_subsource_cat =  subsource.source_main_cat','left');
		//$this->db->join('tbl_admin_users','tbl_doc_inward.sq_end_st = tbl_admin_users.aus_Id');
		//$this->db->where('tbl_doc_inward_item.sai_oa_completed',0);
		$this->db->where('tbl_doc_inward_item.sqi_item_isdelete',0);
		$this->db->where('tbl_doc_inward.sq_isdeleted',0);
		if($this->session->userdata('miconlogin') && is_array($this->session->userdata['miconlogin']) && !empty($this->session->userdata['miconlogin']) && isset($this->session->userdata['miconlogin']['typeid']) && ($this->encrypt_decrypt('decrypt',$this->session->userdata['miconlogin']['typeid']) != 3) &&  isset($this->session->userdata['miconlogin']['userid']) && ($this->session->userdata['miconlogin']['userid'] != ''))
		{
		$this->db->where('tbl_doc_inward.sq_cid', $this->encrypt_decrypt('decrypt',$this->session->userdata['miconlogin']['userid']));
		}
		...
		if($this->input->get('vendor')){
			$this->db->where('tbl_doc_inward.sq_id', $this->input->get('vendor'));
		}
		if($this->input->get('conper')){
			$this->db->where('tbl_doc_inward.sq_id', $this->input->get('conper'));
		}
		if($this->input->get('created_start_date') && ($this->input->get('created_start_date') != ''))
		{
			$stdate = date("Y-m-d", strtotime($this->input->get('created_start_date')));
		...
		{
			$this->db->where('tbl_doc_inward.sq_country',$this->input->get('country'));
		}
		if($this->input->get('state') && ($this->input->get('state') != ''))
		{
			$this->db->where('tbl_doc_inward.sq_state',$this->input->get('state'));
		}
		if($this->input->get('city') && ($this->input->get('city') != ''))
		{
			$this->db->where('tbl_doc_inward.sq_city',$this->input->get('city'));
		}
		if($this->input->get('mobile') && ($this->input->get('mobile') != ''))
		{
			$this->db->where('tbl_doc_inward.sq_mobile',$this->input->get('mobile'));
		}
		if($this->input->get('status') && ($this->input->get('status') != ''))
		{
			...
			{
				$this->db->where('tbl_doc_inward.sq_inq_sts',1);
			}
			if($this->input->get('status') == 'Pending')
			{
				$this->db->where('tbl_doc_inward.sq_inq_sts',2);
			}
			if($this->input->get('status') == 'Completed')
			{
				$this->db->where('tbl_doc_inward.sq_inq_sts',3);
			}
			
		}
		if($this->input->get('sq_source_cat') && ($this->input->get('sq_source_cat') != ''))
		{
			$this->db->where('tbl_doc_inward.sq_source_cat',$this->input->get('sq_source_cat'));
		}
		if($this->input->get('sq_subsource_cat') && ($this->input->get('sq_subsource_cat') != ''))
		{
			$this->db->where('tbl_doc_inward.sq_subsource_cat',$this->input->get('sq_subsource_cat'));
		}
		if($this->input->get('sq_end_st') && ($this->input->get('sq_end_st') != ''))
		{
			$this->db->where('tbl_doc_inward.sq_end_st',$this->input->get('sq_end_st'));
		}
		//********************************************************************************************
	...
	public function inw_no_get()
	{
		...
		$autoid['doc_inward_id'] = isset($autoid['doc_inward_id']) ? $autoid['doc_inward_id'] : '';
		 //echo "<pre>"; print_r($autoid['doc_inward_id']); die;
		return $code['pre_doc_inward'].''.($autoid['doc_inward_id']+1);
		// echo "<pre>"; print_r($code_get); die;
	}
    public function get_report_result()
    {
    	$this->db->select('sum(tbl_doc_inward.sq_grd_ttl) as grand_total_view');
    	$this->db->from('tbl_doc_inward');
    	$query = $this->db->get();
		//echo'<pre>';print_r($query->result_array());die;
		return $query->result_array();
    ...
    {
    	$this->db->select('sum(tbl_doc_inward_item.sqi_itm_ftotal) as sub_total_view');
    	$this->db->from('tbl_doc_inward_item');
    	$query = $this->db->get();
		//echo'<pre>';print_r($query->result_array());die;
		return $query->result_array();
    ...
    {
    	$this->db->select('sum(tbl_doc_inward_itax.sqit_tax_amount) as grand_tax_amount');
    	$this->db->from('tbl_doc_inward_itax');
    	$query = $this->db->get();
		return $query->result_array();
    }
    public function get_inqno_wise()
    {
    	$this->db->select('*');
		$this->db->from('tbl_doc_inward');
		$this->db->join('tbl_master_party', 'tbl_master_party.master_party_id = tbl_doc_inward.vendor','left');
		$this->db->order_by('tbl_doc_inward.sq_id','DESC');
    	$query = $this->db->get();
		return $query->result_array();
    }
    ...
    {
		$this->db->set('doc_inw_isdelete','1');
		$this->db->where('doc_inward_id', $id);
		$this->db->update('tbl_doc_inward');
	...
	public function get_usalesdata($idenc)
	{
		$this->db->select('*');
		$this->db->from('tbl_doc_inward');
		$this->db->join('tbl_doc_inward_item', 'tbl_doc_inward.sq_id = tbl_doc_inward_item.sqi_Doc_inward_id');
		$this->db->join('tbl_master_item','tbl_doc_inward_item.sqi_itm_name = tbl_master_item.master_item_id');
	...
	public function get_vendors()
	{
		...
		$this->db->where('master_vendor_isdelete', 0);
		$query = $this->db->get();
		return $query->result_array();
	}
	...
	{//tbl_followup_method
		$this->db->select('tbl_doc_inward.sq_id as id,tbl_doc_inward.sq_enq_date as inq_dt,tbl_sales_inq_followup.fu_followdate as followdate,tbl_sales_inq_followup.fu_id as fuid,tbl_doc_inward.sq_no as no,tbl_doc_inward.sq_con_person as name,tbl_doc_inward.sq_mobile as mno,tbl_doc_inward.sq_inq_sts as stname,tbl_admin_users.au_fname as executive,tbl_sales_inq_followup.fu_followupst as folst');
		$this->db->from('tbl_doc_inward');
		$this->db->join('tbl_sales_inq_followup', 'tbl_sales_inq_followup.fu_inq_id = tbl_doc_inward.sq_id','left');
		...
		if($this->input->post('name') && $this->input->post('name') != ''){
			$this->db->like('tbl_doc_inward.sq_con_person', $this->input->post('name'));
		}
		if($this->input->post('cno') && $this->input->post('cno') != ''){
			$this->db->like('tbl_doc_inward.sq_mobile', $this->input->post('cno'));
		}
		...
		if($this->input->post('status') && $this->input->post('status') != ''){
			$this->db->like('tbl_doc_inward.sq_inq_sts', $this->input->post('status'));
		}
	...
	public function get_listofollow()
	{
        ...
        $this->db->select('tbl_doc_inward.sq_id as id,tbl_doc_inward.sq_enq_date as inq_dt,tbl_sales_inq_followup.fu_followdate as followdate,tbl_sales_inq_followup.fu_remark as fu_remark,tbl_doc_inward.sq_mobile as mno,tbl_doc_inward.sq_con_person as name,tbl_doc_inward.sq_mobile as no,tbl_doc_inward.sq_inq_sts as stname,tbl_admin_users.au_fname as executive,tbl_sales_inq_followup.fu_id as fuid,tbl_sales_inq_followup.fu_followupst as folst');
        $this->db->from('tbl_doc_inward');
         $this->db->join('tbl_sales_inq_followup', 'tbl_sales_inq_followup.fu_inq_id = tbl_doc_inward.sq_id','left');
         $this->db->join('tbl_admin_users', 'tbl_admin_users.au_id = tbl_sales_inq_followup.fu_followexe','left');
	...
	public function delete_Doc_inward_item($id,$sa_id)
    {
    	$this->db->select('*');
		$this->db->from('tbl_doc_inward_item');
		$this->db->where('inwi_doc_inward_id',$sa_id);
		...
		if($deb_qty['inwi_qty'] <= $initial_stock)
		{
			...
			$this->db->where('inwi_id', $id);
			$this->db->delete('tbl_doc_inward_item');

			$log = array(
					'adlog_name' => $this->session->userdata['miconlogin']['email'],
					'adlog_datetime' => date('Y-m-d H:i:s'),
					'adlog_ip' =>$_SERVER['REMOTE_ADDR'],
					'adlog_module' => 'Doc_inward Item delete',
					'adlog_delete' => 1
				);
			$this->db->insert('tbl_adminlogs',$log);
  ...
  //       		$this->db->where('sqi_Doc_inward_id',$sa_id);
  //       		$query = $this->db->get();
  //       		$value['count']= $query->row_array();
  //       		//echo "<pre>";print_r($value['count']['count']);die;
  //       		$item = array(
	...
	public function create_qoute($id)
	{
		...
		$this->db->update('tbl_doc_inward',$stupdate);
		...
		$item = array(
			...
			'sale_quotation_sub' => $value['salesinq']['Doc_inward_sub'],
			...
			//'sq_grd_ttl_words' => $data['grdttlinword'],
			'sa_remarks' => $value['salesinq']['sq_remarks'],
			'sale_quotation_term' => $value['salesinq']['Doc_inward_desc'],
			'sa_tc_price' => $value['salesinq']['Doc_inward_tc_price'],
			'sa_tc_wrnty' => $value['salesinq']['Doc_inward_tc_wrnty'],
			'sa_tc_pf' => $value['salesinq']['Doc_inward_tc_pf'],
			'sa_tc_deli' => $value['salesinq']['Doc_inward_tc_deli'],
			'sa_tc_paynt' => $value['salesinq']['Doc_inward_tc_paynt'],
			'sa_tc_ovali' => $value['salesinq']['Doc_inward_tc_ovali'],
			'sa_tc_frght' => $value['salesinq']['Doc_inward_tc_frght'],
			'sa_tc_gst' => $value['salesinq']['Doc_inward_tc_gst'],
