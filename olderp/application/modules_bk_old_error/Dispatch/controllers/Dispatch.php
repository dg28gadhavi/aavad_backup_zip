...
class Dispatch extends CI_Controller {
	...
	public function index()
	{
		...
		{
			$this->session->set_flashdata('rights_error', "You Don't have rights to access Dispatch VIew functionality");
			redirect(base_url());
		}
		$this->data['main_content'] = 'Dispatch_grid_view';
		$this->load->view('includes/template',$this->data);
	}

	public function ajax()
	{
		$user = $this->Dispatch_model->get_Dispatch();
		...
		if($right_status == false)
		{
			...
			$editstr = '<a href="'.base_url().'Dispatch/edit/'.$idenc.'" class="btn btn-sm btn-outline green"><i class="fa fa-pencil"></i></a>';
		}
		...
		if($right_status == false)
		{
			...
			$deletestr = '<a href="'.base_url().'Dispatch/delete/'.$idenc.'"onclick="return confirm('."'Are you sure you want to Delete this record?'".')" class="btn btn-sm btn-outline red"><i class="fa fa-times"></i></a>';
		}
		if($right_status == false)
		{
			$viewpdfstr = '';
	...
	public function dispatch_report()
	{
		...
		$this->data['admins'] = $this->Dispatch_model->get_admin();
		$this->data['dis_sts'] = $this->Dispatch_model->get_dis_sts();
		//$this->data['Dispatch'] = $this->Dispatch_model->get_Dispatch();
		$this->data['main_content'] = 'Dispatch_report';
		$this->load->view('includes/template',$this->data);
	}

	public function dispatch_demoreport()
	{
		ini_set('memory_limit', '-1');
		$right_status = $this->check_rights('view');
		if($right_status == false)
		...
		$this->data['admins'] = $this->Dispatch_model->get_admin();
		//$this->data['custometyps'] = $this->Dispatch_model->get_customertype();
		//$this->data['Dispatch'] = $this->Dispatch_model->get_Dispatch();
		$this->data['main_content'] = 'Dispatch_demoreport';
		$this->load->view('includes/template',$this->data);
	}
	public function demoreturn()
	{
		...
		//$this->data['custometyps'] = $this->Dispatch_model->get_customertype();
		//$this->data['Dispatch'] = $this->Dispatch_model->get_Dispatch();
		$this->data['main_content'] = 'Dispatch_demoreturn';
		$this->load->view('includes/template',$this->data);
	}

	public function itmtodis()
	{
		$response=$this->Dispatch_model->itmtodis();
		if(isset($response['status']) && ($response['status'] == true))
		{
			$this->session->set_flashdata('success', 'Details of item added successfully.');
				redirect(base_url('Dispatch/other_details/'.$this->uri->segment(3).'?acttype=edit&itemid='.$response['last_insert_id']), 'refresh');
		}else{
			$this->session->set_flashdata('error_item', $response['msg']);
			//die;
			redirect(base_url('Dispatch/other_details/'.$this->uri->segment(3)), 'refresh');
		}
	}
	...
	public function sales_b2b_inq_report()
	{
		...
		$b2b_info=$this->Dispatch_model->b2b_type_info();
		...
		$enddate=date('d-F-Y');
										$url='https://www.tradeindia.com/utils/my_inquiry.html?userid='.$b2b_info['b2b_trad_uid'].'&profile_id='.$b2b_info['b2b_trad_pid'].'&key='.$b2b_info['b2b_trad_key'].'&from_date='.$sdate_trade."&to_date=".$edate_trade.'';
								           ...
								           $inq_trade=$this->Dispatch_model->insert_trade_btob_inq($json_trade);
		                 ...
		                 //                   $ch = curl_init($ind_url);
								           ...
								           $inq_india=$this->Dispatch_model->insert_india_btob_inq($jsonresult);
		...
		//$this->data['b2b_inq'] = $this->Dispatch_model->get_b2binq();
	...
	{
		ini_set('memory_limit', '-1');
		$user = $this->Dispatch_model->get_b2binq();
		...
		{
			$editstr = '<a title="Edit" href="'.base_url().'Dispatch/add_b2b_comments/'.$idenc.'" class="btn btn-sm btn-outline green"><i class="fa fa-comments"></i></a>';
		}
		...
		{
			$assignstr = '<a title="Assign To Yourself" href="'.base_url().'Dispatch/assign_b2b_inq/'.$idenc.'"onclick="return confirm('."'Are you sure you want to Assign this Inquiry?'".')" class="btn btn-sm btn-outline green"><i class="fa fa-check-circle"></i></a>';
		...
		if($right_status == false)
		{
			...
			$quotestr = '<a title="Create Quotation" href="'.base_url().'Dispatch/create_quote/'.$idenc.'"onclick="return confirm('."'Are you sure you want to Create Quotation?'".')" class="btn btn-sm btn-outline red"><i class="fa fa-plus-circle"></i></a>';
		}
		$right_status = $this->check_rights('view');
		if($right_status == false)
		{
		...
		if($enid && ($enid != ''))
		{
			...
			if($success == TRUE)
			{
				if($this->input->post(NULL,FALSE))
				{
					...
					$lid = $this->Dispatch_model->edit_b2b($value,$idencr);
					if($lid)
					{	
						$this->session->set_flashdata('success', 'sales_b2b_inq_report edited successfully.');
						redirect(base_url('Dispatch/add_b2b_comments/'.$enid), 'refresh');
					}else{
			...
			if($success == FALSE)
			{
				...
				{
					$this->data['list'] = $this->Dispatch_model->get_b2binq_id($idencr);
					$this->data['chat_list'] = $this->Dispatch_model->get_b2biq_chat($idencr);
					//echo "<pre>"; print_r($this->data['list']); die;
					if(!empty($this->data['list']))
					{
						//echo "hi"; die;
						$this->data['action'] = "Dispatch/add_b2b_comments/".$enid;
					...
					else
					{
						 ...
						 redirect(base_url('Dispatch/sales_b2b_inq_report'), 'refresh'); 
					}
				}
				else{
					$this->session->set_flashdata('error', 'Sales_b2b_enq_form_view not Available!!');
	...
	public function delete_chat()
	{
				...
				$lid = $this->Dispatch_model->delete_chat($value,$chat_id);
				if($lid)
				{
					...
					redirect(base_url('Dispatch/add_b2b_comments/'.$idencr), 'refresh');
				}
			 	redirect(base_url('Dispatch/add_b2b_comments/'.$idencr), 'refresh');
	}
	public function assign_b2b_inq()
	{
		...
		$value = array();
				...
				$value = $this->security->xss_clean($value);
				$value['b2bu_udate'] = date('Y-m-d H:i:s');
				$lid = $this->Dispatch_model->assign_b2b_inq($idencr);
				if($lid)
				{
	...
	public function followup()
	{
		...
		$this->data['total'] = $this->Dispatch_model->count();
		$this->data['listfolloup'] = $this->Dispatch_model->get_listofollow();
	...
	{
		$user = $this->Dispatch_model->get_followup();
		//echo "<pre>"; print_r($user); die;
		$iTotalRecords = count($user);
		$iDisplayLength = intval($_REQUEST['length']);
		$iDisplayLength = $iDisplayLength < 0 ? $iTotalRecords : $iDisplayLength; 
		...
		if($right_status == false)
		{
			...
			$editstr = '<a href="'.base_url().'Dispatch/other_details/'.$idenc.'" class="btn btn-sm btn-outline green"><i class="fa fa-search"></i> Edit</a>';
		}
		$right_status = $this->check_rights('delete');
		if($right_status == false)
		{
			...
			$deletestr = '<a href="'.base_url().'Dispatch/delete_fup_sec/'.$fuidenc.'"onclick="return confirm('."'Are you sure you want to Delete this record?'".')" class="btn btn-sm btn-outline red"><i class=""></i> Delete</a>';
		}
		$actstr = '';
		if($user[$i]['folst'] == 6){
			$actstr = '<a href="'.base_url().'Dispatch/status_act/'.$fuidenc.'"onclick="return confirm('."'Are you sure you want to Active this record?'".')" class="btn btn-sm btn-outline red"><i class=""></i> Active</a>';
		}
		if($user[$i]['folst'] == 5){
		$actstr = '<a href="'.base_url().'Dispatch/status_deact/'.$fuidenc.'"onclick="return confirm('."'Are you sure you want to Deactive this record?'".')" class="btn btn-sm btn-outline red"><i class=""></i> Deactivate</a>';
		}
	...
	public function ajax_salesinq_report()
	{
		...
		$count = $this->Dispatch_model->get_disptch_count();
		$iTotalRecords = $count;
		$iDisplayLength = intval($_REQUEST['length']);
		$iDisplayLength = $iDisplayLength < 0 ? $iTotalRecords : $iDisplayLength; 
		$iDisplayStart = intval($_REQUEST['start']);
		...
		$user = $this->Dispatch_model->sales_inq_report($iDisplayStart,$iDisplayLength);
		...
		$status = $status_list[rand(0, 2)];
		$id = ($i + 1);
		$idenc = $this->encrypt_decrypt('encrypt',$user[$i]['dis_id']);
		//$this->encrypt->encode($user[$i]['sq_id']);
		//$crud->columns('master_item_code','master_item_name','master_item_description','master_item_make','master_item_rating','master_item_part_no','master_item_price','master_item_stock','master_item_created_date','master_item_updated_date');
		$right_status = $this->check_rights('edit');
		if($right_status == false)
		{
			...
			$editstr = '<a title="Edit" href="'.base_url().'Dispatch/other_details/'.$idenc.'" class="btn btn-sm btn-outline green"><i class="fa fa-pencil"></i></a>';
		}
		$right_status = $this->check_rights('delete');
		if($right_status == false)
		{
			...
			$deletestr = '<a title="delete" href="'.base_url().'Dispatch/delete/'.$idenc.'"onclick="return confirm('."'Are you sure you want to Delete this record?'".')" class="btn btn-sm btn-outline red"><i class="fa fa-times"></i></a>';
		}
		$right_status = $this->check_rights('add');
		if($right_status == false)
		{
		...
		if($right_status == false)
		{
			...
			$viewpdfstr = '<a title="View PDF" href="'.base_url().'uploads/invoice_file/'.$user[$i]['dis_invoice_file'].'" target="_BLANK" class="btn btn-sm btn-outline blue"><i class="fa fa-file"></i></a>';
		}
		if($right_status == false)
		{
			$viewdrwaing = '';
		}else{
			$viewdrwaing = '<a title="View PDF" href="'.base_url().'uploads/woi_product_drawing/'.$user[$i]['woi_product_drawing'].'" target="_BLANK" class="btn btn-sm btn-outline blue"><i class="fa fa-picture-o"></i></a>';
		}
		if($right_status == false )
		{
			$emailsend = '';
		}else{
			if($user[$i]['dis_issendmail'] == 0)
			{
				$emailsend = '<a title="Email Send" href="'.base_url().'Dispatch/mail/'.$idenc.'" class="btn btn-sm btn-outline blue"><i class="fa fa-envelope-square"></i></a>';
			}
			else{
				$emailsend = '';
			}
		...
		if($user[$i]['dis_wostatus'] == 1 && $user[$i]['dis_issendmail'] == 0)
		{
			 $sstt = '<span class="label label-primary">Pending</span>';
		}else 
			{
				 $sstt = '<span class="label label-warning">Completed</span>';
			}
		...
		$records["data"][] = array(
				...
				''.$user[$i]['wo_po_no'],
				''.$user[$i]['wo_po_date'],
				''.$user[$i]['dis_docket_no'],
				''.$user[$i]['wo_wo_no'],
				''.$user[$i]['wo_wo_date'],
				''.$user[$i]['dis_courier_name'],
				''.$user[$i]['dis_courier_type'],
				''.$user[$i]['dis_remark'].' '.$user[$i]['woi_item_remark_production'],
				''.$user[$i]['au_fname'],
				''.$user[$i]['disi_partno']." | ".$user[$i]['disi_itm_price']." | ".$user[$i]['disi_itm_discount']." | ".$user[$i]['disi_qty'],
				''.$user[$i]['otw_invno'],
				''.$user[$i]['dis_docket_date'],			
				''.$user[$i]['dis_cdate'],
				''.$sstt,
			  ''.$editstr.''.$deletestr.''.$emailsend.''.$viewpdfstr.''.$viewdrwaing.'',
		);
		}
	...
	public function ajax_demoreturn()
	{
		$count = $this->Dispatch_model->get_demoreturn_count();
		$iTotalRecords = $count;
		$iDisplayLength = intval($_REQUEST['length']);
		$iDisplayLength = $iDisplayLength < 0 ? $iTotalRecords : $iDisplayLength; 
		$iDisplayStart = intval($_REQUEST['start']);
		...
		$end = $end > $iTotalRecords ? $iTotalRecords : $end;
		//echo 'Start = '.$iDisplayStart.' End = '.$iDisplayLength.'<br/>';die;
		$user = $this->Dispatch_model->demoreturn_report($iDisplayStart,$iDisplayLength);
		...
		if($right_status == false)
		{
			...
			$editstr = '<a title="Edit" href="'.base_url().'Dashboard_workorder_final/demo_add_details?wo_id='.$this->encrypt_decrypt('encrypt',$user[$i]['dis_woid']).'&otw_id='.$user[$i]['dis_otwid'].'&dis_id='.$user[$i]['dis_id'].'&type=edit" class="btn btn-sm btn-outline green"><i class="fa fa-pencil"></i></a>';
		}
		$right_status = $this->check_rights('delete');
		if($right_status == false)
		{
		...
		if($right_status == false)
		{
			...
			$viewpdfstr = '<a title="View PDF" href="'.base_url().'pdf/dis_demowo/dis_demowo'.$idenc.'.pdf" class="btn btn-sm btn-outline blue"><i class="fa fa-eye"></i></a>';
		}
		if($right_status == false )
		{
			$emailsend = '';
		...
		if($right_status == false )
		{
			...
			if($user[$i]['dis_demoreturn'] == 0)
			{
				$action_return = '<a title="" href="'.base_url().'Dispatch/return_itemtodemo/'.$idenc.'"onclick="return confirm('."'Are you sure you want to Return Item?'".')" class="btn btn-sm btn-outline red"><i class="fa fa-plus-circle"></i></a>';
			}
			else{
				$action_return = '';
			}
		...
		$records["data"][] = array(
				''.$user[$i]['dis_no'],
				''.$user[$i]['dis_return_return_no'],
				''.$user[$i]['dis_vendor'],
				''.$user[$i]['dis_return_challan_no'],
				''.$user[$i]['dis_return_challan_date'],
				''.$user[$i]['dis_wo_no'],
				''.$user[$i]['disi_partno'],
				''.$user[$i]['disi_qty'],
				''.$user[$i]['die_demoreturn_jobno'],
				''.$user[$i]['die_demoreturn_remark'],
				//''.$user[$i]['sq_ref_by'],
			    '',
		);
		}
	...
	public function ajax_demo_report()
	{
		$count = $this->Dispatch_model->get_disptchdemo_count();
		$iTotalRecords = $count;
		$iDisplayLength = intval($_REQUEST['length']);
		$iDisplayLength = $iDisplayLength < 0 ? $iTotalRecords : $iDisplayLength; 
		$iDisplayStart = intval($_REQUEST['start']);
		...
		$end = $end > $iTotalRecords ? $iTotalRecords : $end;
		//echo 'Start = '.$iDisplayStart.' End = '.$iDisplayLength.'<br/>';die;
		$user = $this->Dispatch_model->demo_dispatch_report($iDisplayStart,$iDisplayLength);
		...
		$status = $status_list[rand(0, 2)];
		$id = ($i + 1);
		$idenc = $this->encrypt_decrypt('encrypt',$user[$i]['disi_id']);
		$dis_id = $this->encrypt_decrypt('encrypt',$user[$i]['dis_id']);
		//$this->encrypt->encode($user[$i]['sq_id']);
		//$crud->columns('master_item_code','master_item_name','master_item_description','master_item_make','master_item_rating','master_item_part_no','master_item_price','master_item_stock','master_item_created_date','master_item_updated_date');
		$right_status = $this->check_rights('edit');
		if($right_status == false)
		...
		if($right_status == false)
		{
			$deletestr = '';$idenc = $this->encrypt_decrypt('encrypt',$user[$i]['disi_id']);
		}else{
			$deletestr = '<a title="delete" href="'.base_url().'Dispatch/delete/'.$idenc.'"onclick="return confirm('."'Are you sure you want to Delete this record?'".')" class="btn btn-sm btn-outline red"><i class="fa fa-times"></i></a>';
		}
		$right_status = $this->check_rights('add');
		...
		if($right_status == false)
		{
			...
			$viewpdfstr = '<a title="View PDF" href="'.base_url().'pdf/dis_demowo/dis_demowo'.$dis_id.'.pdf" class="btn btn-sm btn-outline blue"><i class="fa fa-eye"></i></a>';
		}
		if($right_status == false )
		{
			$emailsend = '';
		...
		if($right_status == false )
		{
			...
			if($user[$i]['disi_demoreturn'] == 0 && ($this->encrypt_decrypt('decrypt',$this->session->userdata['miconlogin']['userid']) == 42))
			{
				$action_return = '<a title="" href="'.base_url().'Dispatch/return_itemtodemo/'.$idenc.'"onclick="return confirm('."'Are you sure you want to Return Item?'".')" class="btn btn-sm btn-outline red"><i class="fa fa-plus-circle"></i></a>';
			}
			else{
		...
		$records["data"][] = array(
				''.$user[$i]['dis_no'],
				''.$user[$i]['dis_vendor'],
				''.$user[$i]['dis_challan_no'],
				''.$user[$i]['dis_challan_date'],
				''.$user[$i]['dis_wo_no'],
				''.$user[$i]['disi_partno'],
				''.$user[$i]['disi_qty'],
				''.$user[$i]['dis_remark'],
				//''.$user[$i]['sq_ref_by'],
			  ''.$editstr.''.$deletestr.''.$viewpdfstr.''.''.$action_return.'',
		);
		}
	...
	public function return_itemtodemo()
	{
		//die;
		$enid = $this->uri->segment(3) ? $this->uri->segment(3) : '';
		$idencr = $this->uri->segment(3) ? $this->encrypt_decrypt('decrypt', $enid) : '';
		//echo '<pre>';print_r($idencr);die;
		//$data['action'] = "Dispatch/return_itemtodemo";
		$data['main_content'] = 'Demo_return_view';
	...
	public function create_invoice()
	{
		...
		$idencr = $this->uri->segment(3) ? $this->encrypt_decrypt('decrypt', $enid) : '';
		//echo '<pre>';print_r($idencr);die;
		$this->Dispatch_model->get_create_invoice($idencr);
		redirect(base_url('Dispatch/dispatch_demoreport'));
	}
	public function free_return_item()
	{
		$enid = $this->uri->segment(3) ? $this->uri->segment(3) : '';
		$idencr = $this->uri->segment(3) ? $this->encrypt_decrypt('decrypt', $enid) : '';
		$value = array();
		...
		if($value)
		{
			...
			$this->Dispatch_model->get_freereturn_stock($idencr,$value);
			redirect(base_url('Dispatch/dispatch_demoreport'));
		}else{
			
			//echo '<pre>';print_r($idencr);die;
			$data['demo_no'] = $this->Dispatch_model->demo_no_get($idencr);
			$data['action'] = "Dispatch/free_return_item/".$enid;
			$data['main_content'] = 'Demo_freereturn_view';
			$this->load->view('includes/template', $data);
			//$this->Dispatch_model->get_return_stock($idencr);
			//redirect(base_url('Dispatch/dispatch_demoreport'));
		}
	}
	public function return_stock()
	{
		...
		{
			//echo '<pre>';print_r($value);die;
			$this->Dispatch_model->get_return_stock($idencr,$value);
			redirect(base_url('Dispatch/dispatch_demoreport'));
		}else{
			
			//echo '<pre>';print_r($idencr);die;
			$data['demo_no'] = $this->Dispatch_model->demo_no_get($idencr);
			//echo '<pre>';print_r($data['demo_no']);die;
			$data['action'] = "Dispatch/return_stock/".$enid;
			$data['main_content'] = 'Demo_return_stock_view';
			$this->load->view('includes/template', $data);
			//$this->Dispatch_model->get_return_stock($idencr);
			//redirect(base_url('Dispatch/dispatch_demoreport'));
		}
	...
	public function mail()
	{
		...
		$idencr = $this->uri->segment(3) ? $this->encrypt_decrypt('decrypt', $enid) : '';
		$lists = $this->Dispatch_model->get_pdfdata($idencr);
		$uid = $this->encrypt_decrypt('decrypt',$this->session->userdata['miconlogin']['userid']);
				;
		$mailerdata = $this->Dispatch_model->get_mailer_detail($uid);
		...
		//$this->email->to("chandan@miconindia.com");
		$this->email->to($lists['inv']['dis_cust_email'],$lists['inv']['au_email']);
		$message=$this->load->view('dispatch_email_form_view',$lists,true);
		//echo $lists['inv']['dis_no'];die;
		$this->email->subject("Aavad Dispatch Detail : #".$lists['inv']['dis_no']);
		$this->email->message($message);

		$this->email->attach($path."uploads/invoice_file/".$lists['inv']['dis_invoice_file']);

      	if($this->email->send())
     	{
		      echo 'Email sent.';
		      $this->Dispatch_model->set_mailbit($idencr);
		      $this->session->set_flashdata('success', 'Mail sent successfully.');
			  redirect(base_url('Dispatch/dispatch_report'));
	     }
	     else
	    {
	...
	public function add()
	{
		...
		if($right_status == false)
		{
			$this->session->set_flashdata('rights_error', "You Don't have rights to access Dispatch Add functionality");
			redirect(base_url());
		}
		$success = $this->validation();
		...
		if($success == TRUE)
		{
			...
			{	
				...
				$value = $this->input->post(NULL,FALSE);
				$value = $this->security->xss_clean($value);
				$value['dis_cdate'] = date('Y-m-d H:i:s');
				$value['dis_udate'] = date('Y-m-d H:i:s');
				//$this->session->set_userdata('tabno',2);
				$lid = $this->Dispatch_model->add($value);
				//***********PDF File Code Start********************
				// $pdfdata = $this->Dispatch_model->get_pdfdata($lid,'Dispatch');
				// //echo '<pre>';print_r($pdfdata);die;
				// $html = $this->load->view('Dispatch/Dispatch_pdf_view',$pdfdata,TRUE);
				...
				if($lid)
				{
					...
					redirect(base_url('Dispatch/other_details/'.$this->encrypt_decrypt('encrypt',$lid)), 'refresh');
				}else{
					$this->session->set_flashdata('error', 'Dispatch not added successfully!!');
					redirect(base_url('Dispatch/add'), 'refresh');
				}
			 	redirect(base_url('Dispatch'), 'refresh');
			}
		}
		if($success == FALSE)
		{
	...
	public function other_add()
	{
		...
		if($right_status == false)
		{
			...
			{	//echo '<pre>';print_r($this->input->post(NULL,FALSE));die;
				...
				$lid = $this->Dispatch_model->other_add($value);
				//***********PDF File Code Start********************
				$pdfdata = $this->Dispatch_model->get_pdfdata($lid,'Dispatch');
				//echo '<pre>';print_r($pdfdata);die;
				$html = $this->load->view('Dispatch/Dispatch_pdf_view',$pdfdata,TRUE);
	...
	public function item_add()
	{
			...
			//echo '<pre>';print_r($this->input->post(NULL,FALSE));die;
				...
				$value = $this->input->post(NULL,FALSE);
				$value = $this->security->xss_clean($value);
				$value['disi_cdate'] = date('Y-m-d H:i:s');
				$value['disi_udate'] = date('Y-m-d H:i:s');
				$this->session->set_userdata('tabno',2);
					if(isset($_FILES['master_item_img']['name']) && ($_FILES['master_item_img']['name'] != '')){
					$result = $this->Dispatch_model->item_img($value);
					//if(count($result) > 0)
					//{
				...
				$lid = $this->Dispatch_model->item_add($value);
	...
	public function item_edit()
	{
		ini_set('memory_limit', '-1');
			...
			if($right_status == false)
			{
				$this->session->set_flashdata('rights_error', "You Don't have rights to access Dispatch Edit functionality");
				redirect(base_url());
			}
			//echo "<pre>"; print_r($_FILES); die;
			...
			//echo '<pre>';print_r($this->input->post(NULL,FALSE));die;
				...
				$lid = $this->Dispatch_model->item_edit($value,$sqiitemid);
				...
				{
					$this->session->set_flashdata('success', 'Details of item added successfully.');
					redirect(base_url('Dispatch/other_details/'.$this->uri->segment(3).'?acttype=edit&itemid='.$sqiitemid), 'refresh');
				}else{
					$this->session->set_flashdata('error', 'Details of item not added successfully!!');
					redirect(base_url('Dispatch/add'), 'refresh');
				}
	...
	public function folup_add()
	{
		...
		if($right_status == false)
		{
			...
			{	//echo '<pre>';print_r($this->input->post(NULL,FALSE));die;
				...
				$lid = $this->Dispatch_model->folup_add($value);
				//***********PDF File Code Start********************
				$pdfdata = $this->Dispatch_model->get_pdfdata($lid,'Dispatch');
				//echo '<pre>';print_r($pdfdata);die;
				$html = $this->load->view('Dispatch/Dispatch_pdf_view',$pdfdata,TRUE);
	...
	{
		$idencr = $this->uri->segment(3) ? $this->encrypt_decrypt('decrypt', $this->uri->segment(3)) : '';
		//$this->data['autosearch_items'] = $this->Dispatch_model->get_customertype();
		// $this->data['custometyps'] = $this->Dispatch_model->get_customertype();
		 $this->data['list'] = $this->Dispatch_model->get($idencr);
		// //echo '<pre>';print_r($this->data['list']);die;
		 $this->data['items'] = $this->Dispatch_model->get_items($idencr);
		 $this->data['dis_items'] = $this->Dispatch_model->get_dis_items();
		 //echo '<pre>';print_r($this->data['dis_items']);die;
		if($this->input->get('acttype') && $this->input->get('itemid') && ($this->input->get('acttype') == 'edit'))
		{
			$eitemid = $this->input->get('itemid');
			$this->data['edit_items'] = $this->Dispatch_model->get_edit_inqitems($idencr,$eitemid);
			//echo '<pre>';print_r($this->data['edit_items']);die;
			$this->data['action_itm'] = "Dispatch/item_edit/".$this->uri->segment(3).'?acttype=edit&itemid='.$eitemid;
		}else{
			$this->data['action_itm'] = "Dispatch/item_add/".$this->uri->segment(3);
		}
		$this->session->set_userdata('tabno',1);
		//echo "<pre>"; print_r($this->data['items']); die;
		$this->data['custometyps'] = $this->Dispatch_model->get_customertype();
		$this->data['countries'] = $this->Dispatch_model->get_country();
		$this->data['dis_no'] = $this->Dispatch_model->dis_no_get();
		$this->data['vendors'] = $this->Dispatch_model->get_masterparty();
		$this->data['admins'] = $this->Dispatch_model->get_admin();
		$this->data['main_content'] = 'Sales_basic_details_tab';
		$this->data['action_bd'] = "Dispatch/add/".$this->uri->segment(3);
		$this->data['action_othr'] = "Dispatch/other_add/".$this->uri->segment(3);
		$this->data['all_hsns'] = array();
		//$this->data['action_itm'] = "Dispatch/item_add/".$this->uri->segment(3);
		$this->data['action_fup'] = "Dispatch/folup_add/".$this->uri->segment(3);
		$this->load->view('includes/template',$this->data);
	}
	...
	public function get_form()
	{
		...
		$this->data['vendors'] = $this->Dispatch_model->get_masterparty();
		$this->data['admins'] = $this->Dispatch_model->get_admin();
		$this->data['main_content'] = 'Dispatch_form_view';
		$this->data['action'] = "Dispatch/add";
		$this->load->view('includes/template',$this->data);
	}
	...
	public function delete($id=false)
	{
		...
		if($enid && ($enid != ''))
		{
			...
			if(isset($id) && $id!= ''){
				$this->data['list'] = $this->Dispatch_model->get($idencr);
				if(!empty($this->data['list'])){
					$lid = $this->Dispatch_model->delete($idencr);
						if ($lid) {
						$this->session->set_flashdata('success', 'Dispatch deleted successfully.');
						redirect('Dispatch/dispatch_report', 'refresh');
						} else {
							$this->session->set_flashdata('error', 'Dispatch not deleted successfully!!.');
							redirect('Dispatch/dispatch_report', 'refresh'); 
						}						
				}else{
					$this->session->set_flashdata('error', 'Dispatch not Available!!');
			  		redirect('Dispatch/dispatch_report', 'refresh'); 
			  	}
			}
			else{
			...
			$this->session->set_flashdata('error', 'Something went wrong');
			redirect(base_url('Dispatch/dispatch_report'), 'refresh'); 
		}
	}
	...
	public function delete_all()
	{
		...
		if($right_status == false)
		{
			$this->session->set_flashdata('rights_error', "You Don't have rights to access Dispatch Delete functionality");
			redirect(base_url());
		}
		...
		if($myid && is_array($myid) && !empty($myid))
		{
			foreach($this->input->get('delid') as $enid)
			{
				if($enid && ($enid != ''))
				{
					...
					if(isset($idencr) && $idencr != ''){
						$this->data['list'] = $this->Dispatch_model->get($idencr);//die;
						if(!empty($this->data['list'])){
							$lid = $this->Dispatch_model->delete($idencr);
								if ($lid) {
								$this->session->set_flashdata('success', 'Dispatch inquiry deleted successfully.');
								} else {
									$this->session->set_flashdata('error', 'Dispatch inquiry not deleted successfully!!.');
								}						
						}else{
							$this->session->set_flashdata('error', 'Dispatch inquiry not Available!!');
						}
					}
					else{
							$this->session->set_flashdata('error', 'Dispatch inquiry not Available!!');
  	...
  	{		
		if($this->input->post() && isset($_FILES['userfile']['name']) && ($_FILES['userfile']['name'] != ''))
		{
			...
			if (!$this->upload->do_upload()) {
				...
				$this->data['action'] = "Dispatch/importcsv";
				...
				if ($this->csvimport->get_array($file_path)) {
					...
					if(is_array($csv_array) && !empty($csv_array))
					{
						foreach ($csv_array as $row)
						{
							...
							if(isset($row['Dispatch']) && ($row['Latitude'] != '') && (isset($row['Longitude'])) && (isset($row['DispatchCode'])))
							{
								$this->Dispatch_model->importcsv($row);
							}
						}
	...
	public function check_rights($type)
	{
		...
		if(isset($this->session->userdata['miconlogin']['rightsid']) && ($this->session->userdata['miconlogin']['rightsid'] != '') && isset($this->session->userdata['miconlogin']['typeid']) && ($this->session->userdata['miconlogin']['typeid'] != ''))
		{
			...
			if($typeid != 3)
			{
				...
				$finalrights = $this->global_model->get_rights($rightsid,$moduleid = 34,$type);
	...
	public function setbit_Dispatch()
	{
		$this->Dispatch_model->setbit_Dispatch();
	}
	...
	public function getsub_category()
	{
		...
		$this->data['sub_catlists'] = $this->Dispatch_model->get_subsource($this->abc);
	...
	public function item_description()
	{
		if($this->input->post() && $this->input->post('master_item_id'))
		{
			...
			$array = $this->Dispatch_model->get_item_description($id);
	...
	public function get_customer_information()
	{
		...
		{
			$this->Dispatch_model->get_customer_information($value['term']);
		}
	...
	public function get_hc()
	{
		...
		if(isset($id) && ($id != ''))
		{
			$value = $this->Dispatch_model->get_hcs($id);
			echo json_encode($value);
		}
	}

	public function delete_dispatch_item($id=false)
	{
		$right_status = $this->check_rights('delete');
		if($right_status == false)
		{
			$this->session->set_flashdata('rights_error', "You Don't have rights to access Dispatch inquiry Delete functionality");
			redirect(base_url());
		}
		$enid = $this->uri->segment(3) ? $this->uri->segment(3) : '';
		...
		if($enid && ($enid != ''))
		{
			...
			if(isset($id) && $id!= ''){
				$this->session->set_userdata('tabno',2);
					$lid = $this->Dispatch_model->delete_dispatch_item($idencr,$sa_id);
					//***********PDF File Code Start********************
					$autoid = $this->uri->segment(3) ? $this->encrypt_decrypt('decrypt', $this->uri->segment(4)) : '';
					// $pdfdata = $this->Dispatch_model->get_pdfdata($autoid,'Dispatch');
					// //echo '<pre>';print_r($pdfdata);die;
					// $html = $this->load->view('Dispatch/Dispatch_pdf_view',$pdfdata,TRUE);
					// //$html=$this->data['result_view'];
					// $header='';
					...
					//***********PDF File Code End********************
						if ($lid) {
						$this->session->set_flashdata('success', 'Dispatch inquiry deleted successfully.');
						redirect('Dispatch/other_details/'.$this->uri->segment(4), 'refresh');
						} else {
							$this->session->set_flashdata('error', 'Dispatch inquiry not deleted successfully!!.');
							redirect('Dispatch/other_details/'.$this->uri->segment(4), 'refresh'); 
						}						
			...
			$this->session->set_flashdata('error', 'Something went wrong');
			redirect(base_url('Dispatch/other_details/'.$this->uri->segment(4)), 'refresh'); 
		}
	}
	...
	public function status_act($id=false)
	{
		...
		if($enid && ($enid != ''))
		{
			...
			if(isset($id) && $id!= ''){
				$this->session->set_userdata('tabno',3);
					$lid = $this->Dispatch_model->status_act($idencr);
						if ($lid) {
	...
	public function status_deact($id=false)
	{
		...
		if($enid && ($enid != ''))
		{
			...
			if(isset($id) && $id!= ''){
				$this->session->set_userdata('tabno',3);
					$lid = $this->Dispatch_model->status_deact($idencr);
						if ($lid) {
						$this->session->set_flashdata('success', 'Followup status changed successfully.');
						redirect('inq-followup', 'refresh');
						} else {
	...
	public function delete_fup($id=false)
	{
		...
		if($enid && ($enid != ''))
		{
			...
			if(isset($id) && $id!= ''){
				$this->session->set_userdata('tabno',3);
					$lid = $this->Dispatch_model->delete_fup($idencr);
					//***********PDF File Code Start********************
					$autoid = $this->uri->segment(3) ? $this->encrypt_decrypt('decrypt', $this->uri->segment(4)) : '';
					$pdfdata = $this->Dispatch_model->get_pdfdata($autoid,'Dispatch');
					//echo '<pre>';print_r($pdfdata);die;
	...
	public function get_item_details()
	{
		...
		if(isset($value['term']) && !empty($value['term']))
		{
			$this->Dispatch_model->get_item_details($value['term']);
		}
	}
	...
	public function create_quote()
	{
		...
		$lid = $this->Dispatch_model->create_qoute($id);
...
ini_set('memory_limit','3222222222222222222222222222222222222222M');
        ...
        $finalar = $this->Dispatch_model->get_excel_certificate();
...
//echo '<pre>';print_r($finalar);die;
	...
	public function get_country_to_state()
	{
		...
		{
			$statelists = $this->Dispatch_model->get_country_to_state($countyid);
		}else{
			$statelists = array();
		}
	...
	public function get_state_to_city()
	{
		...
		{
			$citylists = $this->Dispatch_model->get_state_to_city($stateID);
		}else{
			$citylists = array();
		}
	...
	//    $this->Dispatch_model->old_dis_to_new_dis();
