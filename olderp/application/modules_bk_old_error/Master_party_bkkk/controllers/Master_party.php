...
class Master_party extends CI_Controller {
	...
	public function ajax()
	{
		...
		$count = $this->master_party_model->get_master_party_count();
		...
		$user = $this->master_party_model->get_master_party($iDisplayStart,$iDisplayLength);
		...
		$records["data"][] = array(
			  ...
			  ''.$user[$i]['master_party_contact_person'].",".$user[$i]['conparty_name'],
			  ''.$user[$i]['master_party_gst'],
			  ''.$user[$i]['conparty_mobile'].",".$user[$i]['conparty_mobile'],
			  //''.$user[$i]['master_party_fax'],
			  ''.$user[$i]['master_party_contact_no'].",".$user[$i]['master_party_phone'],
			  ''.$user[$i]['master_party_email_address'].",".$user[$i]['conparty_emailid'],
			  ''.$user[$i]['state_name'],
			  ''.$user[$i]['city_name'],
			  '<a href="'.base_url().'Master_party/edit/'.$idenc.'" class="btn btn-sm btn-outline green"><i class="fa fa-pencil"></i></a><a href="'.base_url().'Master_party/delete/'.$idenc.'"onclick="return confirm('."'Are you sure you want to Delete this record?'".')" class="btn btn-sm btn-outline red"><i class="fa fa-times"></i></a>',
		);
		}
