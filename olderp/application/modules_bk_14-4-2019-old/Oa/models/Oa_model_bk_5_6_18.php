...
class Oa_model extends CI_Model {
  ...
  //       if($this->session->userdata('miconlogin') && is_array($this->session->userdata['miconlogin']) && !empty($this->session->userdata['miconlogin']) && isset($this->session->userdata['miconlogin']['typeid']) && ($this->encrypt_decrypt('decrypt',$this->session->userdata['miconlogin']['typeid']) != 3) &&  isset($this->session->userdata['miconlogin']['userid']) && ($this->session->userdata['miconlogin']['userid'] != ''))
	...
	public function oa_no_get()
	{
		...
		$code = $query->row_array();
		$autoid['oa_id'] = isset($autoid['oa_id']) ? $autoid['oa_id'] : '';
		return $code['pre_enquiry'].''.($autoid['oa_id']+1);
	}
	...
	public function oatopi()
	{
		...
		if($this->input->get('delid') && is_array($get_array) && !empty($get_array))
		{

			$this->db->select('*');
			$this->db->from('tbl_oa_item');
			//$this->db->join('tbl_booking_order','tbl_indend_status.ind_booking_id = tbl_booking_order.bo_id');
			$this->db->where_in('oai_id',$get_array);
			$query = $this->db->get();
			$inditems = $query->result_array();
			//echo "<pre>";print_r($inditems);die;
				...
				$item = array(
					...
					'pi_cid' => $userid,
					// 'vendor' => $autoid['vendor'],
					// 'pi_email' => $autoid['oa_email'],
					// 'pi_phone' => $autoid['oa_phone'],
					// 'pi_city' => $autoid['oa_city'],
					'pi_cdate' => date("Y-m-d"),
					'pi_udate' => date("Y-m-d")
					);
				//echo '<pre>';print_r($item);die;
				...
				foreach($inditems as $inditem)
				{
					...
					$this->db->from('tbl_oa_item');
					//$this->db->join('tbl_booking_order','tbl_indend_status.ind_booking_id = tbl_booking_order.bo_id');
					$this->db->where('oai_id',$inditem['oai_id']);
					$query = $this->db->get();
					$inddata = $query->row_array();
					//echo "<pre>"; print_r($inddata); die;

					 $item = array(
							'pii_oale_quotation_id' => $lid,
							'pii_itm_name' => $inddata['oai_itm_name'],
							'pii_itm' => $inddata['oai_itm'],
							//'pi_itm_currency' => $data['item_currency'][$key],
							'pii_itm_qty' => $inddata['oai_itm_qty'],
							...
							'pii_itm_stock' => $inddata['oai_itm_stock'],
							'pii_itm_opn_qty' => $inddata['oai_itm_opn_qty'],
							'pii_itm_price' => $inddata['oai_itm_price'],
							'pii_itm_discount' => $inddata['oai_itm_discount'],
							'pii_itm_total' => $inddata['oai_itm_total'],
							'pii_hsncode' => $inddata['oai_itm_hsncode'],
							//'pii_itm_cid' => $userid,
