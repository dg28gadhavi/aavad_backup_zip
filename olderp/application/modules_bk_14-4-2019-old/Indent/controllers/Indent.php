...
class Indent extends CI_Controller {
	...
	public function index()
	{
		...
		{
			$this->session->set_flashdata('rights_error', "You Don't have rights to access Indent VIew functionality");
			redirect(base_url());
		}
		$this->data['main_content'] = 'Indent_grid_view';
		$this->load->view('includes/template',$this->data);
	}

	public function ajax()
	{
		$user = $this->Indent_model->Indent_report();
		...
		if($right_status == false)
		{
			...
			$editstr = '<a href="'.base_url().'Indent/edit/'.$idenc.'" class="btn btn-sm btn-outline green"><i class="fa fa-pencil"></i></a>';
		}
		...
		if($right_status == false)
		{
			...
			$deletestr = '<a href="'.base_url().'Indent/delete/'.$idenc.'"onclick="return confirm('."'Are you sure you want to Delete this record?'".')" class="btn btn-sm btn-outline red"><i class="fa fa-times"></i></a>';
		}
		$records["data"][] = array(
	...
	public function ajax_wo()
	{
		$user = $this->Indent_model->wo_indent_report();
		$iTotalRecords = count($user);
		$iDisplayLength = intval($_REQUEST['length']);
		$iDisplayLength = $iDisplayLength < 0 ? $iTotalRecords : $iDisplayLength; 
		$iDisplayStart = intval($_REQUEST['start']);
	...
	public function Indent_report()
	{
		...
		$this->data['admins'] = $this->Indent_model->get_admin();
		//$this->data['custometyps'] = $this->Indent_model->get_customertype();
		//$this->data['Indent'] = $this->Indent_model->get_Indent();
		$this->data['main_content'] = 'Indent_report';
		$this->load->view('includes/template',$this->data);
	}
	public function Wo_indent_report()
	{
		...
		//$this->data['custometyps'] = $this->Indent_model->get_customertype();
		//$this->data['Indent'] = $this->Indent_model->get_Indent();
		$this->data['main_content'] = 'wo_indent_report';
		$this->load->view('includes/template',$this->data);
	}
	...
	{
		$this->data['custometyps'] = $this->Indent_model->get_customertype();
		$this->data['cities'] = $this->Indent_model->get_city();
		$this->data['countries'] = $this->Indent_model->get_country();
		$this->data['wo_no'] = $this->Indent_model->wo_no_get();
		$this->data['vendors'] = $this->Indent_model->get_masterparty();
		$this->data['admins'] = $this->Indent_model->get_admin();
		$this->data['main_content'] = 'Indent_form_view';
		$this->data['action'] = "Indent/add";
		$this->load->view('includes/template',$this->data);
	}
	...
	public function indenttopo()
	{
		...
		$lid = $this->indend_model->indenttopo();
		if(isset($lid))
		{
	...
	public function delete($id=false)
	{
		...
		if($enid && ($enid != ''))
		{
			$idencr = $this->uri->segment(3) ? $this->encrypt_decrypt('decrypt', $enid) : '';
			if(isset($id) && $id!= ''){
				$this->data['list'] = $this->Indent_model->get($idencr);
				if(!empty($this->data['list'])){
					$lid = $this->Indent_model->delete($idencr);
						if ($lid) {
						$this->session->set_flashdata('success', 'Indent deleted successfully.');
						redirect('Indent/Indent_report', 'refresh');
						} else {
							$this->session->set_flashdata('error', 'Indent not deleted successfully!!.');
							redirect('Indent/Indent_report', 'refresh'); 
						}						
				}else{
					$this->session->set_flashdata('error', 'Indent not Available!!');
			  		redirect('Indent/Indent_report', 'refresh'); 
			  	}
			}
			else{
			...
			redirect(base_url('Indent/Indent_report'), 'refresh'); 
		}
	}
	...
	public function delete_all()
	{
		...
		if($right_status == false)
		{
			$this->session->set_flashdata('rights_error', "You Don't have rights to access Indent Delete functionality");
			redirect(base_url());
		}
			...
			foreach($this->input->get('delid') as $enid)
			{
				if($enid && ($enid != ''))
				{
					...
					if(isset($idencr) && $idencr != ''){
						$this->data['list'] = $this->Indent_model->get($idencr);//die;
						if(!empty($this->data['list'])){
							$lid = $this->Indent_model->delete($idencr);
								if ($lid) {
								$this->session->set_flashdata('success', 'Indent inquiry deleted successfully.');
								} else {
									$this->session->set_flashdata('error', 'Indent inquiry not deleted successfully!!.');
								}						
						}else{
							$this->session->set_flashdata('error', 'Indent inquiry not Available!!');
						}
					}
					else{
							$this->session->set_flashdata('error', 'Indent inquiry not Available!!');
		...
		redirect(base_url('Indent'), 'refresh');
	...
	public function setbit_Indent()
	{
		$this->Indent_model->setbit_Indent();
	}
	...
	public function getsub_category()
	{
		...
		$this->data['sub_catlists'] = $this->Indent_model->get_subsource($this->abc);
	...
	public function item_description()
	{
		if($this->input->post() && $this->input->post('master_item_id'))
		{
			...
			$array = $this->Indent_model->get_item_description($id);
	...
	public function get_customer_information()
	{
		...
		{
			$this->Indent_model->get_customer_information($value['term']);
		}
	...
	public function get_hc()
	{
		...
		if(isset($id) && ($id != ''))
		{
			$value = $this->Indent_model->get_hcs($id);
			echo json_encode($value);
		}
	}

	public function delete_Indent_item($id=false)
	{
		$right_status = $this->check_rights('delete');
		if($right_status == false)
		{
			$this->session->set_flashdata('rights_error', "You Don't have rights to access Indent inquiry Delete functionality");
			redirect(base_url());
		}
		$enid = $this->uri->segment(3) ? $this->uri->segment(3) : '';
		...
		if($enid && ($enid != ''))
		{
			...
			if(isset($id) && $id!= ''){
				$this->session->set_userdata('tabno',2);
					$lid = $this->Indent_model->delete_Indent_item($idencr,$sa_id);
					//***********PDF File Code Start********************
					$autoid = $this->uri->segment(3) ? $this->encrypt_decrypt('decrypt', $this->uri->segment(4)) : '';
					// $pdfdata = $this->Indent_model->get_pdfdata($autoid,'Indent');
					// //echo '<pre>';print_r($pdfdata);die;
					// $html = $this->load->view('Indent/Indent_pdf_view',$pdfdata,TRUE);
					// //$html=$this->data['result_view'];
					// $header='';
					// $footer='';
					...
					//***********PDF File Code End********************
						if ($lid) {
						$this->session->set_flashdata('success', 'Indent inquiry deleted successfully.');
						redirect('Indent/other_details/'.$this->uri->segment(4), 'refresh');
						} else {
							$this->session->set_flashdata('error', 'Indent inquiry not deleted successfully!!.');
							redirect('Indent/other_details/'.$this->uri->segment(4), 'refresh'); 
						}						
			...
			$this->session->set_flashdata('error', 'Something went wrong');
			redirect(base_url('Indent/other_details/'.$this->uri->segment(4)), 'refresh'); 
		}
	}
		...
		if($enid && ($enid != ''))
		{
			...
			if(isset($id) && $id!= ''){
				$this->session->set_userdata('tabno',3);
					$lid = $this->Indent_model->status_act($idencr);
						if ($lid) {
	...
	public function status_deact($id=false)
	{
		...
		if($enid && ($enid != ''))
		{
			...
			if(isset($id) && $id!= ''){
				$this->session->set_userdata('tabno',3);
					$lid = $this->Indent_model->status_deact($idencr);
						if ($lid) {
						$this->session->set_flashdata('success', 'Followup status changed successfully.');
						redirect('inq-followup', 'refresh');
						} else {
	...
	public function delete_fup($id=false)
	{
		...
		if($enid && ($enid != ''))
		{
			...
			if(isset($id) && $id!= ''){
				$this->session->set_userdata('tabno',3);
					$lid = $this->Indent_model->delete_fup($idencr);
					//***********PDF File Code Start********************
					$autoid = $this->uri->segment(3) ? $this->encrypt_decrypt('decrypt', $this->uri->segment(4)) : '';
					$pdfdata = $this->Indent_model->get_pdfdata($autoid,'Indent');
					//echo '<pre>';print_r($pdfdata);die;
	...
	public function get_item_details()
	{
		...
		if(isset($value['term']) && !empty($value['term']))
		{
			$this->Indent_model->get_item_details($value['term']);
		}
	}
	...
	public function create_quote()
	{
		...
		$lid = $this->Indent_model->create_qoute($id);
...
ini_set('memory_limit','3222222222222222222222222222222222222222M');
        ...
        $finalar = $this->Indent_model->get_excel_certificate();
...
//echo '<pre>';print_r($finalar);die;
	...
	public function get_country_to_state()
	{
		...
		{
			$statelists = $this->Indent_model->get_country_to_state($countyid);
		}else{
			$statelists = array();
		}
	...
	public function get_state_to_city()
	{
		...
		{
			$citylists = $this->Indent_model->get_state_to_city($stateID);
		}else{
			$citylists = array();
		}
