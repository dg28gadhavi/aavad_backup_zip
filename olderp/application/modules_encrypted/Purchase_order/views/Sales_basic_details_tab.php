...
<div class="row">
    ...
    <div class="breadcrumbs breadcrumbs-header">
        <h1>Purchase Order Description</h1>
        <?php $id = $this->uri->segment(3); ?>
            ...
            <?php $id = $this->uri->segment(3); ?>
            <a class="btn green" href="<?php echo base_url(); ?>Purchase_order/mail/<?php echo $id; ?>">Mail Sent</a>
     </div> 
     </div>      
    <div class="col-md-6 breadcrumbs breadcrumbs-header">
        <ol class="breadcrumb">
            <li>
                ...
                <a class="bread-space" href="<?php echo base_url(); ?>Purchase_order/Purchase_order_report">Purchase Order Report</a>
                <a class="bread-space" href="<?php echo base_url(); ?>Purchase_order/add">Create New Purchase Order</a>
            </li>
        </ol>
    </div>
                    ...
                    <?php //echo '<pre>';print_r($emails);//die; ?>
                        ...
                        <div class="portlet box blue">     
                             <div class="portlet-title">
                                    <div class="caption">
                                        <i class="fa fa-cogs"></i>Purchase Order Process </div>
                                    <div class="tools">
                            ...
                            <div class="portlet-body">
                                ...
                                <?php //$tabono = $this->session->userdata('tabno') ? $this->session->userdata('tabno') : 0; ?>
                                           ...
                                           <?php /* <li class="<?php if(isset($tabno) && ($tabno == 3 || $tabno == 0)) { ?> active <?php } ?>">
                                                <a href="#tab_2_3"  data-toggle="tab" aria-expanded="false"> Work Order Open Item </a>
                                            </li> */ ?>
                                            <!-- <li class="<?php if(isset($tabno) && ($tabno == 3)) { ?> active <?php } ?>">
                                                <a href="#tab_2_3" data-toggle="tab" aria-expanded="false">Indend Item</a>
                                            </li> -->
                                            ...
                                            <div class="tab-content">
                                                    <div class="tab-pane fade <?php if(isset($tabno) && ($tabno == 1)) { ?> active in <?php } ?>" id="tab_2_1">
                                                           <?php $this->load->view('Purchase_order_bd_view_tab');?>    
                                                    </div>
                                            <div class="tab-pane fade <?php if(isset($tabno) && ($tabno == 2 || $tabno == 0)) { ?> active in <?php } ?>" id="tab_2_2">
                                                         <?php $this->load->view('Purchase_order_item_details_tab');?>     
                                            </div>   
                                            <?php /*<div class="tab-pane fade <?php if(isset($tabno) && ($tabno == 3 || $tabno == 3)) { ?> active in <?php } ?>" id="tab_2_3">
                                                         <?php $this->load->view('Purchase_order_details_tab');?>     
                                            </div>   */ ?>
                </div>
            </div>
                <!-- END CONTENT BODY -->
 ...
 <script>
  $( function() {
    $("#sqi_itm_pno").autocomplete({
            source: base_url+"Purchase_order/get_item_details",
            minLength: 1,
            html: true,
            select: function( event, ui )
            {
                ...
                $('#poi_stock').val(ui.item.stock);
                $('#sqi_itm_hsncode').val(ui.item.hsncode);
                $('#poi_itm_desc').val(ui.item.description);
                $('#poi_price').val(ui.item.rate);
                $('#poi_last_price').val(ui.item.purchase_price);
...
<script type="text/javascript">
$(document).on('blur', '.poi_qty,.poi_price,.poi_discount', function() {
        var idd = $(this).attr('id');
        //ert(idd);
        var total =($("#poi_qty").val() * $("#poi_price").val());
        var totaldisc = ((($("#poi_qty").val() * $("#poi_price").val()) * $("#poi_discount").val()) / 100);
        var ftotal = (($("#poi_qty").val() * $("#poi_price").val()) - totaldisc);
        $("#poi_total").val(parseFloat(total).toFixed(2));
        $("#poi_ftotal").val(parseFloat(ftotal).toFixed(2));
        //ert(totaldisc);
...
$( function() {
    ...
    $("#sq_con_person").autocomplete({
            ...
            source: function(request, response) {
                $.getJSON(base_url+"Purchase_order/get_contactperson_information", { vendor_id : $('#vendor_id').val() }, 
                          response);
              },
            ...
            select: function( event, ui )
            {
                ...
                var str = '';
               $("#sq_con_person").val(ui.item.cperson);
               $("#po_contactno").val(ui.item.contact_mobile);
