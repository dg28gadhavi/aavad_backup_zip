...
class Inquiry extends CI_Controller {
	...
	public function inq_status_view()
	{
		...
		// $this->pagination->initialize($config); 
		// $this->data['pagi'] = $this->pagination->create_links();
		// $this->data['inq_lists'] = $this->inquiry_model->get_inq_status($page,$config['per_page']);
		// //***************************************************************************
		// $this->data['datass'] = $this->inquiry_model->get_all_masters();
		// $this->data['total_rows'] = $config['total_rows'];
		...
		if($vals['inq_date'] == '1970-01-01')
			{
				...
				{
					 $sstt = date("m-d-Y", strtotime($vals['inq_date']));
				}
...
//			  $id,
	...
	public function ajax_inq_status()
	{
		$count = $this->inquiry_model->get_inquiry_status_count();
		$iTotalRecords = $count;
		$iDisplayLength = intval($_REQUEST['length']);
		$iDisplayLength = $iDisplayLength < 0 ? $iTotalRecords : $iDisplayLength; 
		$iDisplayStart = intval($_REQUEST['start']);
		...
		$end = $iDisplayStart + $iDisplayLength;
		$end = $end > $iTotalRecords ? $iTotalRecords : $end;
		$user = $this->inquiry_model->get_inquiry_status($iDisplayStart,$iDisplayLength);
		$status_list = array(
			array("success" => "Pending"),
			array("info" => "Closed"),
			array("danger" => "On Hold"),
				...
				if ($this->csvimport->get_array($file_path)) {
					...
					if(is_array($csv_array) && !empty($csv_array))
					{
						foreach ($csv_array as $row)
						{
							...
							if((isset($row['inqid'])) && (isset($row['userid'])) && (isset($row['dateinq'])) && (isset($row['calldate'])) && (isset($row['fudate'])) && (isset($row['compnayname'])) && (isset($row['clientname'])) && (isset($row['contactno'])) && (isset($row['contactno2'])) && (isset($row['country'])) && (isset($row['state'])) && (isset($row['city'])) && (isset($row['areaname'])) && (isset($row['proid'])) && (isset($row['producttype'])) && (isset($row['category'])) && (isset($row['status'])) && (isset($row['executive'])) && (isset($row['source'])) && (isset($row['email'])) && (isset($row['band'])) && (isset($row['age'])) && (isset($row['education'])) && (isset($row['experience'])) && (isset($row['expfiled'])) && (isset($row['relative_in_foreign'])) && (isset($row['countryeligible'])) && (isset($row['refusal'])) && (isset($row['spouseage'])) && (isset($row['spouseageedu'])) && (isset($row['spouseexp'])) && (isset($row['spouseexpfiled'])) && (isset($row['kids'])) && (isset($row['reference'])) && (isset($row['inqtype'])) && (isset($row['remarks'])))
							{
								//echo 'hiiiiiiiii';die;
								//$this->inquiry_model->importcsv($row);
