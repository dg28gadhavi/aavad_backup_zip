...
class Master_item extends CI_Controller {
	...
	{
		$user = $this->master_item_model->get_master_items();
		...
		if($right_status == false)
		{
			...
			$editstr = '<a href="'.base_url().'Master_item/edit/'.$idenc.'" class="btn btn-sm btn-outline green"><i class="fa fa-search"></i>Edit</a>';
		}
		...
		if($right_status == false)
		{
			...
			$deletestr = '<a href="'.base_url().'Master_item/delete/'.$idenc.'"onclick="return confirm('."'Are you sure you want to Delete this record?'".')" class="btn btn-sm btn-outline red"><i class=""></i> Delete</a>';
		}
		$records["data"][] = array(
			  '<input type="checkbox" name="delid[]" value="'.$idenc.'">',
			  $id,
			''.$user[$i]['master_item_code'],
			''.$user[$i]['master_item_hsncode'],
			''.$user[$i]['master_item_name'],
			''.$user[$i]['master_item_description'],
			''.$user[$i]['master_item_rate'],
			// ''.$user[$i]['master_item_unit_name'],
			''.$user[$i]['master_item_stock'],
				...
				if ($this->csvimport->get_array($file_path)) {
					...
					if(is_array($csv_array) && !empty($csv_array))
					{
						foreach ($csv_array as $row)
						{
							...
							if(isset($row['Master_item']) && ($row['Latitude'] != '') && (isset($row['Longitude'])) && (isset($row['master_itemCode'])))
							{
