...
class Brand extends CI_Controller {
	...
	public function index()
	{
		...
		{
			$this->session->set_flashdata('rights_error', "You Don't have rights to access Brand VIew functionality");
			redirect(base_url());
		}
		$this->data['main_content'] = 'Brand_grid_view';
		$this->load->view('includes/template',$this->data);
	}

	public function ajax()
	{
		$user = $this->brand_model->get_brand();
		...
		$id = ($i + 1);
		$idenc = $this->encrypt_decrypt('encrypt',$user[$i]['brand_id']);
		//$this->encrypt->encode($user[$i]['brand_id']);
		...
		if($right_status == false)
		{
			...
			$editstr = '<a href="'.base_url().'Brand/edit/'.$idenc.'" class="btn btn-sm btn-outline green"><i class="fa fa-search"></i> Edit</a>';
		}
		...
		if($right_status == false)
		{
			...
			$deletestr = '<a href="'.base_url().'Brand/delete/'.$idenc.'"onclick="return confirm('."'Are you sure you want to Delete this record?'".')" class="btn btn-sm btn-outline red"><i class=""></i> Delete</a>';
		}
		$records["data"][] = array(
			  '<input type="checkbox" name="delid[]" value="'.$user[$i]['brand_id'].'">',
			  $id,
			  ''.$user[$i]['brand_name'],
			  //''.$user[$i]['brand_roe'],
			  ''.date("m-d-Y", strtotime($user[$i]['brand_cdate'])),
			  ''.date("m-d-Y", strtotime($user[$i]['brand_udate'])),
			  ''.$editstr.''.$deletestr.'',
		);
		}
	...
	public function add()
	{
		...
		if($right_status == false)
		{
			$this->session->set_flashdata('rights_error', "You Don't have rights to access Brand Add functionality");
			redirect(base_url());
		}
		...
		if($success == TRUE)
		{
			if($this->input->post(NULL,FALSE))
			{
				...
				$value['brand_cdate'] = date('Y-m-d H:i:s');
				$value['brand_udate'] = date('Y-m-d H:i:s');
				
				$lid = $this->brand_model->add($value);
				if($lid)
				{
					$this->session->set_flashdata('success', 'Brand added successfully.');
					redirect(base_url('Brand'), 'refresh');
				}else{
					$this->session->set_flashdata('error', 'Brand not added successfully!!');
					redirect(base_url('Brand/add'), 'refresh');
				}
			 	redirect(base_url('Brand'), 'refresh');
			}
		}
	...
	public function edit($id = FALSE)
	{
		...
		if($right_status == false)
		{
			$this->session->set_flashdata('rights_error', "You Don't have rights to access Brand Edit functionality");
			redirect(base_url());
		}
		//require 'Zebra_Image.php';
		...
		if($enid && ($enid != ''))
		{
			...
			if($success == TRUE)
			{
				if($this->input->post(NULL,FALSE))
				{
					...
					$lid = $this->brand_model->edit($value,$idencr);
					if($lid)
					{
						$this->session->set_flashdata('success', 'Brand edited successfully.');
						redirect(base_url('Brand'), 'refresh');
					}else{
						$this->session->set_flashdata('error', 'Brand not edited successfully!!');
					}
				 	redirect(base_url('Brand'), 'refresh');
				}
			}
			if($success == FALSE)
			{
				if(isset($idencr) && $idencr != ''){
					$this->data['list'] = $this->brand_model->get($idencr);
					//echo "<pre>"; print_r($this->data['list']); die;
					if(!empty($this->data['list']))
					{
						//echo "hi"; die;
						$this->data['action'] = "Brand/edit/".$enid;
						$this->data['main_content'] = 'Brand_form_view';
						$this->load->view('includes/template',$this->data);
						//parent::load_view('admin/master/brand/Brand_form_view',$this->data);
					}
					else
					{
						 $this->session->set_flashdata('error', 'Brand not Available!!');
						 redirect(base_url('Brand'), 'refresh'); 
					}
				}
				else{
	...
	public function validation() 
	{
		if($this->input->post(NULL,TRUE))
		{
			...
			if($this->uri->segment(2) == 'add'){
				...
				$enid = $this->uri->segment(3) ? $this->uri->segment(3) : '';
				$idencr = $this->uri->segment(3) ? $this->encrypt_decrypt('decrypt', $enid) : '';
				$this->form_validation->set_rules('brand_name', 'brand name', 'trim|required|edit_unique[tbl_brand.brand_name.'.$idencr.'.brand_id]');  
			}
	...
	{
		//$this->data['getbrand'] = $this->brand_model->addtbrand();
		$this->data['main_content'] = 'Brand_form_view';
		$this->data['action'] = "Brand/add";
		$this->load->view('includes/template',$this->data);
	}
	...
	public function delete($id=false)
	{
		...
		if($right_status == false)
		{
			$this->session->set_flashdata('rights_error', "You Don't have rights to access Brand Delete functionality");
			redirect(base_url());
		}
		$enid = $this->uri->segment(3) ? $this->uri->segment(3) : '';
		if($enid && ($enid != ''))
		{
			...
			if(isset($id) && $id!= ''){
				$this->data['list'] = $this->brand_model->get($idencr);
				if(!empty($this->data['list'])){
					$lid = $this->brand_model->delete($idencr);
						if ($lid) {
						$this->session->set_flashdata('success', 'Brand deleted successfully.');
						redirect('Brand', 'refresh');
						} else {
							$this->session->set_flashdata('error', 'Brand not deleted successfully!!.');
							redirect('Brand', 'refresh'); 
						}						
				}else{
					$this->session->set_flashdata('error', 'Brand not Available!!');
		...
		if($this->input->get('delid') && is_array($this->input->get('delid')) && !empty($this->input->get('delid')))
		{
			foreach($this->input->get('delid') as $enid)
			{
				if($enid && ($enid != ''))
				{
					...
					if(isset($idencr) && $idencr != ''){
						$this->data['list'] = $this->brand_model->get($idencr);//die;
						if(!empty($this->data['list'])){
							$lid = $this->brand_model->delete($idencr);
								if ($lid) {
								$this->session->set_flashdata('success', 'Brand deleted successfully.');
  	...
  	{		
		if($this->input->post() && isset($_FILES['userfile']['name']) && ($_FILES['userfile']['name'] != ''))
		{
			...
			if (!$this->upload->do_upload()) {
				...
				$this->data['action'] = "brand/importcsv";
				...
				if ($this->csvimport->get_array($file_path)) {
					...
					if(is_array($csv_array) && !empty($csv_array))
					{
						foreach ($csv_array as $row)
						{
							...
							if(isset($row['Brand']) && ($row['Latitude'] != '') && (isset($row['Longitude'])) && (isset($row['brandCode'])))
							{
								$this->brand_model->importcsv($row);
							}
						}
	...
	public function setbit_brand()
	{
		$this->brand_model->setbit_brand();
	}
