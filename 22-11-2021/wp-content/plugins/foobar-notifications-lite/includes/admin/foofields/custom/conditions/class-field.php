...
namespace FooPlugins\FooBar\Admin\FooFields\Custom\Conditions;

use FooPlugins\FooBar\Admin\FooFields\Custom\Conditions\Group\Field as ConditionsGroup;
use FooPlugins\FooBar\Admin\FooFields\Container;
use FooPlugins\FooBar\Pro\Visibility\Engine;
use WP_User;

if ( ! class_exists( __NAMESPACE__ . '\Field' ) ) {

	class Field extends \FooPlugins\FooBar\Admin\FooFields\Fields\Field {
		...
		function ajax_handle_conditions() {
			if ( $this->verify_nonce() ) {
				...
				wp_send_json_error( __( 'Invalid condition!', 'foobar' ), 400 );
			}
		}
