...
class CFF_Notifications {
	...
	public function hooks() {
		...
		add_action( 'cff_admin_notices', array( $this, 'output' ) );
	...
	public function header_notices() {
		...
		$output .= sprintf(
			...
			__('You\'re using Custom Facebook Feed Lite. To unlock more features consider', 'custom-faceboook-feed'),
			$upgrade_url,
			__('upgrading to Pro', 'custom-facebook-feed')
		);

		$output .= sprintf(
			'<button type="button" class="cff-dismiss" id="cff-dismiss-header-notice" title="%s" data-page="overview">%s</button>',
			__('Dismiss this message', 'custom-facebook-feed'),
	...
	public function verify( $notifications ) { // phpcs:ignore Generic.Metrics.CyclomaticComplexity.TooHigh
		...
		foreach ( $notifications as $notification ) {
			...
			$license = CFF_Utils::cff_is_pro_version() ? 'pro' : 'free';
	...
	public function enqueues() {
		...
		wp_enqueue_style(
			...
			CFF_PLUGIN_URL . "admin/assets/css/admin-notifications{$min}.css",
			array(),
			CFFVER
		);
		...
		wp_enqueue_script(
			'cff-admin-notifications',
			CFF_PLUGIN_URL . "admin/assets/js/admin-notifications{$min}.js",
			array( 'jquery' ),
			CFFVER,
			true
		);
	...
	public function output() {
		...
		foreach ( $notifications as $notification ) {
			...
			if ( ! empty( $notification['btns'] ) && is_array( $notification['btns'] ) ) {
				foreach ( $notification['btns'] as $btn_type => $btn ) {
					if ( $type == 'review' || $type == 'discount' ) {
						$btn_class = $btn_type === 'primary' ? 'cff-btn-blue' : 'cff-btn-grey';
					} else {
						$btn_class = $btn_type === 'primary' ? 'cff-btn-orange' : 'cff-btn-grey';
					}
					if ( is_array( $btn['url'] ) ) {
			...
			if ( $type == 'review' ) {
				$step1_img = CFF_PLUGIN_URL . 'admin/assets/img/' . sanitize_text_field( $notification['image'] );
				...
				if ( ! $review_consent ) {
					...
					$notifications_html .= sprintf(
						'<div class="cff_review_step1_notice" data-message-id="%3$s">' . $step1_img_html . '
							<h3 class="title">%1$s</h3>
							<div class="review-step-1-btns">%2$s</div>
						</div>',
						__( 'Are you enjoying the Custom Facebook Feed Plugin?', 'custom-facebook-feed' ),
						$step1_btns,
			...
			if ( $type == 'review' ) {
				$notifications_html .= sprintf(
					...
					__( 'Glad to hear you are enjoying it. Would you consider leaving a positive review?', 'custom-facebook-feed' ),
					__( 'It really helps to support the plugin and help others to discover it too!', 'custom-facebook-feed' ),
					$buttons_html,
					! empty( $notification['id'] ) ? esc_attr( sanitize_text_field( $notification['id'] ) ) : 0,
					$current_class,
				...
				$notifications_html .= sprintf(
					...
					__( 'Exclusive Offer! 60% OFF', 'custom-facebook-feed' ),
					__( 'We don’t run promotions very often, but for a limited time we’re offering 60% Off our Pro version to all users of our free Facebook Feed.', 'custom-facebook-feed' ),
					$buttons_html,
					! empty( $notification['id'] ) ? esc_attr( sanitize_text_field( $notification['id'] ) ) : 0,
					$current_class,
					( $notification['id'] == 'review' && ! empty( $review_step2_style ) ) ? $review_step2_style : '',
