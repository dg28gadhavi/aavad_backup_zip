...
class Purchase_order extends CI_Controller {
	...
	public function index()
	{
		...
		{
			$this->session->set_flashdata('rights_error', "You Don't have rights to access Purchase_order VIew functionality");
			redirect(base_url());
		}
		//$this->data['admins'] = $this->Purchase_order_model->get_admin();
		//$this->data['report_result'] = $this->Purchase_order_model->get_totalreport();
		//$this->data['custometyps'] = $this->Purchase_order_model->get_customertype();
		//$this->data['Purchase_order'] = $this->Purchase_order_model->get_Purchase_order();
		$this->data['main_content'] = 'Purchase_order_report';
		$this->load->view('includes/template',$this->data);
	}

	public function ajax()
	{
		$user = $this->Purchase_order_model->get_Purchase_order();
		...
		if($right_status == false)
		{
			...
			$editstr = '<a href="'.base_url().'Purchase_order/edit/'.$idenc.'" class="btn btn-sm btn-outline green"><i class="fa fa-pencil"></i></a>';
		}
		...
		if($right_status == false)
		{
			...
			$deletestr = '<a href="'.base_url().'Purchase_order/delete/'.$idenc.'"onclick="return confirm('."'Are you sure you want to Delete this record?'".')" class="btn btn-sm btn-outline red"><i class="fa fa-times"></i></a>';
		}
		if($right_status == false)
		{
			$viewpdfstr = '';
		}else{
			$viewpdfstr = '<a href="'.base_url().'pdf/po/po'.$idenc.'.pdf" class="btn btn-sm btn-outline blue" target="_blank"><i class="fa fa-search"></i> View PDF</a>';
		}
	...
	public function sales_b2b_inq_report()
	{
		...
		$b2b_info=$this->Purchase_order_model->b2b_type_info();
		...
		$enddate=date('d-F-Y');
										$url='https://www.tradeindia.com/utils/my_inquiry.html?userid='.$b2b_info['b2b_trad_uid'].'&profile_id='.$b2b_info['b2b_trad_pid'].'&key='.$b2b_info['b2b_trad_key'].'&from_date='.$sdate_trade."&to_date=".$edate_trade.'';
								           ...
								           $inq_trade=$this->Purchase_order_model->insert_trade_btob_inq($json_trade);
		                 ...
		                 //                   $ch = curl_init($ind_url);
								           ...
								           $inq_india=$this->Purchase_order_model->insert_india_btob_inq($jsonresult);
		...
		//$this->data['b2b_inq'] = $this->Purchase_order_model->get_b2binq();
	...
	{
		ini_set('memory_limit', '-1');
		$user = $this->Purchase_order_model->get_b2binq();
		...
		{
			$editstr = '<a title="Edit" href="'.base_url().'Purchase_order/add_b2b_comments/'.$idenc.'" class="btn btn-sm btn-outline green"><i class="fa fa-comments"></i></a>';
		}
		...
		{
			$assignstr = '<a title="Assign To Yourself" href="'.base_url().'Purchase_order/assign_b2b_inq/'.$idenc.'"onclick="return confirm('."'Are you sure you want to Assign this Inquiry?'".')" class="btn btn-sm btn-outline green"><i class="fa fa-check-circle"></i></a>';
		...
		if($right_status == false)
		{
			...
			$quotestr = '<a title="Create Quotation" href="'.base_url().'Purchase_order/create_quote/'.$idenc.'"onclick="return confirm('."'Are you sure you want to Create Quotation?'".')" class="btn btn-sm btn-outline red"><i class="fa fa-plus-circle"></i></a>';
		}
		$right_status = $this->check_rights('view');
		if($right_status == false)
		{
		...
		if($enid && ($enid != ''))
		{
			...
			if($success == TRUE)
			{
				if($this->input->post(NULL,FALSE))
				{
					...
					$lid = $this->Purchase_order_model->edit_b2b($value,$idencr);
					if($lid)
					{	
						$this->session->set_flashdata('success', 'sales_b2b_inq_report edited successfully.');
						redirect(base_url('Purchase_order/add_b2b_comments/'.$enid), 'refresh');
					}else{
			...
			if($success == FALSE)
			{
				...
				{
					$this->data['list'] = $this->Purchase_order_model->get_b2binq_id($idencr);
					$this->data['chat_list'] = $this->Purchase_order_model->get_b2biq_chat($idencr);
					//echo "<pre>"; print_r($this->data['list']); die;
					if(!empty($this->data['list']))
					{
						//echo "hi"; die;
						$this->data['action'] = "Purchase_order/add_b2b_comments/".$enid;
					...
					else
					{
						 ...
						 redirect(base_url('Purchase_order/sales_b2b_inq_report'), 'refresh'); 
					}
				}
				else{
					$this->session->set_flashdata('error', 'Sales_b2b_enq_form_view not Available!!');
	...
	public function delete_chat()
	{
				...
				$lid = $this->Purchase_order_model->delete_chat($value,$chat_id);
				if($lid)
				{
					...
					redirect(base_url('Purchase_order/add_b2b_comments/'.$idencr), 'refresh');
				}
			 	redirect(base_url('Purchase_order/add_b2b_comments/'.$idencr), 'refresh');
	}
	public function assign_b2b_inq()
	{
		...
		$value = array();
				...
				$value = $this->security->xss_clean($value);
				$value['b2bu_udate'] = date('Y-m-d H:i:s');
				$lid = $this->Purchase_order_model->assign_b2b_inq($idencr);
				if($lid)
				{
	...
	public function followup()
	{
		...
		$this->data['total'] = $this->Purchase_order_model->count();
		$this->data['listfolloup'] = $this->Purchase_order_model->get_listofollow();
	...
	{
		$user = $this->Purchase_order_model->get_followup();
		//echo "<pre>"; print_r($user); die;
		$iTotalRecords = count($user);
		$iDisplayLength = intval($_REQUEST['length']);
		$iDisplayLength = $iDisplayLength < 0 ? $iTotalRecords : $iDisplayLength; 
		...
		if($right_status == false)
		{
			...
			$editstr = '<a href="'.base_url().'Purchase_order/other_details/'.$idenc.'" class="btn btn-sm btn-outline green"><i class="fa fa-search"></i> Edit</a>';
		}
		$right_status = $this->check_rights('delete');
		if($right_status == false)
		{
			...
			$deletestr = '<a href="'.base_url().'Purchase_order/delete_fup_sec/'.$fuidenc.'"onclick="return confirm('."'Are you sure you want to Delete this record?'".')" class="btn btn-sm btn-outline red"><i class=""></i> Delete</a>';
		}
		$actstr = '';
		if($user[$i]['folst'] == 6){
			$actstr = '<a href="'.base_url().'Purchase_order/status_act/'.$fuidenc.'"onclick="return confirm('."'Are you sure you want to Active this record?'".')" class="btn btn-sm btn-outline red"><i class=""></i> Active</a>';
		}
		if($user[$i]['folst'] == 5){
		$actstr = '<a href="'.base_url().'Purchase_order/status_deact/'.$fuidenc.'"onclick="return confirm('."'Are you sure you want to Deactive this record?'".')" class="btn btn-sm btn-outline red"><i class=""></i> Deactivate</a>';
		}
	...
	{
		$user = $this->Purchase_order_model->sales_inq_report();
		$iTotalRecords = count($user);
		$iDisplayLength = intval($_REQUEST['length']);
		$iDisplayLength = $iDisplayLength < 0 ? $iTotalRecords : $iDisplayLength; 
		$iDisplayStart = intval($_REQUEST['start']);
		...
		$status = $status_list[rand(0, 2)];
		$id = ($i + 1);
		$idenc = $this->encrypt_decrypt('encrypt',$user[$i]['po_id']);
		//$this->encrypt->encode($user[$i]['sq_id']);
		//$crud->columns('master_item_code','master_item_name','master_item_description','master_item_make','master_item_rating','master_item_part_no','master_item_price','master_item_stock','master_item_created_date','master_item_updated_date');
		$right_status = $this->check_rights('edit');
		if($right_status == false)
		{
			...
			$editstr = '<a title="Edit" href="'.base_url().'Purchase_order/other_details/'.$idenc.'" class="btn btn-sm btn-outline green"><i class="fa fa-pencil"></i></a>';
		}
		$right_status = $this->check_rights('delete');
		if($right_status == false)
		{
			...
			$deletestr = '<a title="delete" href="'.base_url().'Purchase_order/delete/'.$idenc.'"onclick="return confirm('."'Are you sure you want to Delete this record?'".')" class="btn btn-sm btn-outline red"><i class="fa fa-times"></i></a>';
		}
		$right_status = $this->check_rights('add');
		if($right_status == false)
		{
		...
		if($right_status == false)
		{
			...
			$viewpdfstr = '<a title="View PDF" href="'.base_url().'Purchase_order/load_pdf/'.$idenc.'.pdf" class="btn btn-sm btn-outline blue" target="_blank"><i class="fa fa-eye"></i></a>';
		}
		if($right_status == false)
		{
			$complet_po = '';
		}else{
			$complet_po = '<a title="" href="'.base_url().'Purchase_order/complet_po/'.$idenc.'"onclick="return confirm('."'Are you sure you want to Complet this record?'".')" class="btn btn-sm btn-outline red"><i class="fa fa-check"></i></a>';
		}
		if($right_status == false)
		{
			$complet_po = '';
		}else{
			if($user[$i]['po_completed'] == 0)
			{
				$complet_po = '<a title="" href="'.base_url().'Purchase_order/complet_po/'.$idenc.'"onclick="return confirm('."'Are you sure you want to Complet this record?'".')" class="btn btn-sm btn-outline red"><i class="fa fa-check"></i></a>';
			}
			else{
		...
		if($user[$i]['po_completed'] == 1)
		{
			$complet = '<span class="label label-success">Completed</span>';
		}
		else{
			$complet = '';
		}
		...
		if($right_status == false)
		{
			...
			$emailsend = '<a title="Email Send" href="'.base_url().'Purchase_order/mail/'.$idenc.'" class="btn btn-sm btn-outline blue" target="_blank"><i class="fa fa-envelope-square"></i></a>';
		}
		...
		$records["data"][] = array(
				...
				''.$complet,
				//''.$user[$i]['sq_ref_by'],
			  ''.$editstr.''.$deletestr.''.$viewpdfstr.''.$complet_po.''.$emailsend,
		);
		}
	...
	public function add()
	{
		...
		if($right_status == false)
		{
			$this->session->set_flashdata('rights_error', "You Don't have rights to access Purchase_order Add functionality");
			redirect(base_url());
		}
		$success = $this->validation();
		...
		if($success == TRUE)
		{
			...
			{	
				...
				$value = $this->input->post(NULL,FALSE);
				$value = $this->security->xss_clean($value);
				$value['po_cdate'] = date('Y-m-d H:i:s');
				$value['po_udate'] = date('Y-m-d H:i:s');
				//$this->session->set_userdata('tabno',2);
				$lid = $this->Purchase_order_model->add($value);
				...
				{
					//die;
					$this->session->set_flashdata('success', 'Purchase_order added successfully.');
					redirect(base_url('Purchase_order/other_details/'.$this->encrypt_decrypt('encrypt',$lid)), 'refresh');
				}else{
					$this->session->set_flashdata('error', 'Purchase_order not added successfully!!');
					redirect(base_url('Purchase_order/add'), 'refresh');
				}
			 	redirect(base_url('Purchase_order'), 'refresh');
			}
		}
		if($success == FALSE)
		{
	...
	{
		$enid = $this->uri->segment(3) ? $this->uri->segment(3) : '';
		$files = glob(FCPATH.'pdf/po/*.pdf'); //get all file names
		...
		if($enid && ($enid != ''))
		{
				...
				$pdfdata = $this->Purchase_order_model->get_pdfdata($lid,'sale_quotation');
				//echo '<pre>';print_r($pdfdata);die;
				$html = $this->load->view('Purchase_order/Purchase_order_pdf_view',$pdfdata,TRUE);
				...
				$pdfFilePath = FCPATH.'/pdf/po/po'.$this->encrypt_decrypt('encrypt',$lid).'.pdf';
				...
				if($id == false)
				{
					...
					redirect(base_url().'pdf/po/po'.$this->encrypt_decrypt('encrypt',$lid).'.pdf');
				}
	...
	public function other_add()
	{
		...
		if($right_status == false)
		{
			...
			{	//echo '<pre>';print_r($this->input->post(NULL,FALSE));die;
				...
				$lid = $this->Purchase_order_model->other_add($value);
				/***********PDF File Code Start********************/
				$pdfdata = $this->Purchase_order_model->get_pdfdata($lid,'Purchase_order');
				//echo '<pre>';print_r($pdfdata);die;
				$html = $this->load->view('Purchase_order/Purchase_order_pdf_view',$pdfdata,TRUE);
				//$html=$this->data['result_view'];
				$header='';
	...
	public function item_add()
	{
			...
			if($right_status == false)
			{
				...
				$value = $this->input->post(NULL,FALSE);
				$value = $this->security->xss_clean($value);
				$value['poi_cdate'] = date('Y-m-d H:i:s');
				$value['poi_udate'] = date('Y-m-d H:i:s');
				$this->session->set_userdata('tabno',2);
					if(isset($_FILES['master_item_img']['name']) && ($_FILES['master_item_img']['name'] != '')){
					$result = $this->Purchase_order_model->item_img($value);
					//if(count($result) > 0)
					//{
				...
				$response = $this->Purchase_order_model->item_add($value);
				...
				if(isset($response['status']) && ($response['status'] == true))
				{
					...
					redirect(base_url('Purchase_order/other_details/'.$this->uri->segment(3)), 'refresh');
				}else{
	...
	public function item_edit()
	{
		ini_set('memory_limit', '-1');
			...
			if($right_status == false)
			{
				$this->session->set_flashdata('rights_error', "You Don't have rights to access Purchase_order Edit functionality");
				redirect(base_url());
			}
			...
			//echo '<pre>';print_r($this->input->post(NULL,FALSE));die;
				...
				$response = $this->Purchase_order_model->item_edit($value,$sqiitemid);
				
				/***********PDF File Code Start********************/
				$pdfdata = $this->Purchase_order_model->get_pdfdata($response['last_insert_id'],'Purchase_order');
				//echo '<pre>';print_r($pdfdata);die;
				$html = $this->load->view('Purchase_order/Purchase_order_pdf_view',$pdfdata,TRUE);
				//$html=$this->data['result_view'];
				$header='';
				$footer='';
				$pdfFilePath = FCPATH.'/pdf/po/po'.$this->encrypt_decrypt('encrypt',$response['last_insert_id']).'.pdf';
				$data['page_title'] = 'Hello world';
				ini_set('memory_limit','32222222222222222222222222M');
				$this->load->library('pdf');
				$pdf = $this->pdf->load();
				...
				{
					$this->session->set_flashdata('success_item', 'Details of item added successfully.');
					redirect(base_url('Purchase_order/other_details/'.$this->uri->segment(3).'?acttype=edit&itemid='.$sqiitemid), 'refresh');
				}else{
					$this->session->set_flashdata('error_item', $response['msg']);
					redirect(base_url('Purchase_order/other_details/'.$this->uri->segment(3).'?acttype=edit&itemid='.$sqiitemid), 'refresh');
				}
	...
	public function folup_add()
	{
		...
		if($right_status == false)
		{
			...
			{	//echo '<pre>';print_r($this->input->post(NULL,FALSE));die;
				...
				$lid = $this->Purchase_order_model->folup_add($value);
				//***********PDF File Code Start********************
				$pdfdata = $this->Purchase_order_model->get_pdfdata($lid,'Purchase_order');
				//echo '<pre>';print_r($pdfdata);die;
				$html = $this->load->view('Purchase_order/Purchase_order_pdf_view',$pdfdata,TRUE);
	...
	{
		$idencr = $this->uri->segment(3) ? $this->encrypt_decrypt('decrypt', $this->uri->segment(3)) : '';
		//$this->data['autosearch_items'] = $this->Purchase_order_model->get_customertype();
		// $this->data['custometyps'] = $this->Purchase_order_model->get_customertype();
		 $this->data['list'] = $this->Purchase_order_model->get($idencr);
		//echo '<pre>';print_r($this->data['list']);die;
		$this->data['items'] = $this->Purchase_order_model->get_items($idencr);
		$this->data['wo_items'] = $this->Purchase_order_model->wo_open_report();
		...
		if($this->input->get('acttype') && $this->input->get('itemid') && ($this->input->get('acttype') == 'edit'))
		{
			...
			$this->data['edit_items'] = $this->Purchase_order_model->get_edit_inqitems($idencr,$eitemid);
			//echo '<pre>';print_r($this->data['edit_items']);die;
			$this->data['action_itm'] = "Purchase_order/item_edit/".$this->uri->segment(3).'?acttype=edit&itemid='.$eitemid;
		}else{
			$this->data['action_itm'] = "Purchase_order/item_add/".$this->uri->segment(3);
		}
		//echo "<pre>"; print_r($this->data['items']); die;
		$this->data['currency'] = $this->Purchase_order_model->get_currency();
		$this->data['delivary'] = $this->Purchase_order_model->get_delivary();
		$this->data['po_no'] = $this->Purchase_order_model->po_no_get();
		$this->data['admins'] = $this->Purchase_order_model->get_admin();
		$this->data['main_content'] = 'Sales_basic_details_tab';
		$this->data['action_bd'] = "Purchase_order/add/".$this->uri->segment(3);
		$this->data['action_othr'] = "Purchase_order/other_add/".$this->uri->segment(3);
		//$this->data['action_itm'] = "Purchase_order/item_add/".$this->uri->segment(3);
		$this->data['action_fup'] = "Purchase_order/folup_add/".$this->uri->segment(3);
		$this->load->view('includes/template',$this->data);
	}

	public function woitemtopo()
	{
		$itemid=$this->Purchase_order_model->woitemtopo();
		$this->session->set_userdata('tabno',2);
		$this->session->set_flashdata('success_item', 'Details of item added successfully.');
		redirect(base_url('Purchase_order/other_details/'.$this->uri->segment(3).'?acttype=edit&itemid='.$itemid), 'refresh');
	}

	public function indenttopo()
	{

		$itemid=$this->Purchase_order_model->indenttopo();
		$this->session->set_userdata('tabno',3);
		$this->session->set_flashdata('success_item', 'Details of item added successfully.');
		redirect(base_url('Purchase_order/other_details/'.$this->uri->segment(3).'?acttype=edit&itemid='.$itemid), 'refresh');
	...
	public function validation() 
	{
		...
		if($this->input->post(NULL,TRUE))
		{
			...
			{
				$this->form_validation->set_rules('po_supplier', 'po_supplier', 'trim|required');
				...
				$this->form_validation->set_rules('po_supplier', 'Email', 'trim|required');   
			}
			else 
			{
				//echo "hiiixsds";die;
	...
	public function get_form()
	{
		...
		$this->data['custometyps'] = $this->Purchase_order_model->get_customertype();
		$this->data['po_no'] = $this->Purchase_order_model->po_no_get();
		$this->data['po_code'] = $this->Purchase_order_model->po_code_get();
		$this->data['vendors'] = $this->Purchase_order_model->get_masterparty();
		$this->data['admins'] = $this->Purchase_order_model->get_admin();
		$this->data['main_content'] = 'Purchase_order_form_view';
		$this->data['action'] = "Purchase_order/add";
		$this->load->view('includes/template',$this->data);
	}
	...
	public function complet_po()
	{
		...
		if($enid && ($enid != ''))
		{
			...
			if(isset($idencr) && $idencr!= '')
			{
				...
				{
					//die;
					$lid = $this->Purchase_order_model->complet_po($idencr);
						if ($lid) 
						{
						$this->session->set_flashdata('success', 'Purchase_order Completed successfully.');
						redirect('Purchase_order/Purchase_order_report', 'refresh');
						} else {
							$this->session->set_flashdata('error', 'Purchase_order not Completed successfully!!.');
							redirect('Purchase_order/Purchase_order_report', 'refresh'); 
						}						
				}else{
					$this->session->set_flashdata('error', 'Purchase_order not Available!!');
			  		redirect('Purchase_order/Purchase_order_report', 'refresh'); 
			  	}
			}
			else{
			...
			$this->session->set_flashdata('error', 'Something went wrong');
			redirect(base_url('Purchase_order/Purchase_order_report'), 'refresh'); 
		}
	}
	public function delete($id=false)
	{
		...
		if($enid && ($enid != ''))
		{
			...
			if(isset($id) && $id!= ''){
				$this->data['list'] = $this->Purchase_order_model->get($idencr);
				if(!empty($this->data['list'])){
					$lid = $this->Purchase_order_model->delete($idencr);
						if ($lid) {
						$this->session->set_flashdata('success', 'Purchase_order deleted successfully.');
						redirect('Purchase_order/Purchase_order_report', 'refresh');
						} else {
							$this->session->set_flashdata('error', 'Purchase_order not deleted successfully!!.');
							redirect('Purchase_order/Purchase_order_report', 'refresh'); 
						}						
				}else{
					$this->session->set_flashdata('error', 'Purchase_order not Available!!');
	...
	public function delete_all()
	{
		...
		if($right_status == false)
		{
			$this->session->set_flashdata('rights_error', "You Don't have rights to access Purchase_order Delete functionality");
			redirect(base_url());
		}
		...
		if($myid && is_array($myid) && !empty($myid))
		{
			foreach($this->input->get('delid') as $enid)
			{
				if($enid && ($enid != ''))
				{
					...
					if(isset($idencr) && $idencr != ''){
						$this->data['list'] = $this->Purchase_order_model->get($idencr);//die;
						if(!empty($this->data['list'])){
							$lid = $this->Purchase_order_model->delete($idencr);
								if ($lid) {
								$this->session->set_flashdata('success', 'Purchase_order inquiry deleted successfully.');
								} else {
									$this->session->set_flashdata('error', 'Purchase_order inquiry not deleted successfully!!.');
								}						
						}else{
							$this->session->set_flashdata('error', 'Purchase_order inquiry not Available!!');
						}
					}
					else{
							$this->session->set_flashdata('error', 'Purchase_order inquiry not Available!!');
  	...
  	{		
		if($this->input->post() && isset($_FILES['userfile']['name']) && ($_FILES['userfile']['name'] != ''))
		{
			...
			if (!$this->upload->do_upload()) {
				...
				$this->data['action'] = "Purchase_order/importcsv";
				...
				if ($this->csvimport->get_array($file_path)) {
					...
					if(is_array($csv_array) && !empty($csv_array))
					{
						foreach ($csv_array as $row)
						{
							...
							if(isset($row['Purchase_order']) && ($row['Latitude'] != '') && (isset($row['Longitude'])) && (isset($row['Purchase_orderCode'])))
							{
								$this->Purchase_order_model->importcsv($row);
							}
						}
	...
	public function setbit_Purchase_order()
	{
		$this->Purchase_order_model->setbit_Purchase_order();
	}
	...
	public function getsub_category()
	{
		...
		$this->data['sub_catlists'] = $this->Purchase_order_model->get_subsource($this->abc);
	...
	public function item_description()
	{
		if($this->input->post() && $this->input->post('master_item_id'))
		{
			...
			$array = $this->Purchase_order_model->get_item_description($id);
	...
	public function get_customer_information()
	{
		...
		{
			$this->Purchase_order_model->get_customer_information($value['term']);
		}
	...
	public function get_contactperson_information()
	{
		...
		{
			$this->Purchase_order_model->get_contactperson_information($value['vendor_id']);
		}
	...
	public function get_hc()
	{
		...
		if(isset($id) && ($id != ''))
		{
			$value = $this->Purchase_order_model->get_hcs($id);
			echo json_encode($value);
		}
	}

	public function delete_Purchase_order_item($id=false)
	{
		$right_status = $this->check_rights('delete');
		if($right_status == false)
		{
		...
		if($enid && ($enid != ''))
		{
			...
			if(isset($id) && $id!= ''){
				$this->session->set_userdata('tabno',2);
					$lid = $this->Purchase_order_model->delete_Purchase_order_item($idencr,$sa_id);
					//***********PDF File Code Start********************
					$autoid = $this->uri->segment(3) ? $this->encrypt_decrypt('decrypt', $this->uri->segment(4)) : '';
					// $pdfdata = $this->Purchase_order_model->get_pdfdata($autoid,'Purchase_order');
					// //echo '<pre>';print_r($pdfdata);die;
					// $html = $this->load->view('Purchase_order/Purchase_order_pdf_view',$pdfdata,TRUE);
					// //$html=$this->data['result_view'];
					// $header='';
					...
					//***********PDF File Code End********************
						if ($lid) {
						$this->session->set_flashdata('success', 'Purchase_order inquiry deleted successfully.');
						redirect('Purchase_order/other_details/'.$this->uri->segment(4), 'refresh');
						} else {
							$this->session->set_flashdata('error', 'Purchase_order inquiry not deleted successfully!!.');
							redirect('Purchase_order/other_details/'.$this->uri->segment(4), 'refresh'); 
						}						
			...
			$this->session->set_flashdata('error', 'Something went wrong');
			redirect(base_url('Purchase_order/other_details/'.$this->uri->segment(4)), 'refresh'); 
		}
	}
	...
	public function status_act($id=false)
	{
		...
		if($right_status == false)
		{
			$this->session->set_flashdata('rights_error', "You Don't have rights to access Purchase_order inquiry Delete functionality");
			redirect(base_url());
		}
		$enid = $this->uri->segment(3) ? $this->uri->segment(3) : '';
		if($enid && ($enid != ''))
		{
			...
			if(isset($id) && $id!= ''){
				$this->session->set_userdata('tabno',3);
					$lid = $this->Purchase_order_model->status_act($idencr);
						if ($lid) {
	...
	public function status_deact($id=false)
	{
		...
		if($enid && ($enid != ''))
		{
			...
			if(isset($id) && $id!= ''){
				$this->session->set_userdata('tabno',3);
					$lid = $this->Purchase_order_model->status_deact($idencr);
						if ($lid) {
						$this->session->set_flashdata('success', 'Followup status changed successfully.');
						redirect('inq-followup', 'refresh');
						} else {
	...
	public function delete_fup($id=false)
	{
		...
		if($enid && ($enid != ''))
		{
			...
			if(isset($id) && $id!= ''){
				$this->session->set_userdata('tabno',3);
					$lid = $this->Purchase_order_model->delete_fup($idencr);
					//***********PDF File Code Start********************
					$autoid = $this->uri->segment(3) ? $this->encrypt_decrypt('decrypt', $this->uri->segment(4)) : '';
					$pdfdata = $this->Purchase_order_model->get_pdfdata($autoid,'Purchase_order');
					//echo '<pre>';print_r($pdfdata);die;
	...
	public function get_item_details()
	{
		...
		if(isset($value['term']) && !empty($value['term']))
		{
			$this->Purchase_order_model->get_item_details($value['term']);
		}
	}
	...
	public function create_quote()
	{
		...
		$lid = $this->Purchase_order_model->create_qoute($id);
...
ini_set('memory_limit','3222222222222222222222222222222222222222M');
        ...
        $finalar = $this->Purchase_order_model->get_excel_certificate();
...
//echo '<pre>';print_r($finalar);die;
	...
	public function get_country_to_state()
	{
		...
		{
			$statelists = $this->Purchase_order_model->get_country_to_state($countyid);
		}else{
			$statelists = array();
		}
	...
	public function get_state_to_city()
	{
		...
		{
			$citylists = $this->Purchase_order_model->get_state_to_city($stateID);
		}else{
			$citylists = array();
		}
	...
	public function mail()
	{
		...
		$uid = $this->encrypt_decrypt('decrypt',$this->session->userdata['miconlogin']['userid']);
				;
		$this->data['maildata'] = $this->Purchase_order_model->get_mailer_detail($uid);
		$this->data['lists'] = $this->Purchase_order_model->get($idencr);
		$this->data['main_content'] = 'purchase_qoutation_mail_view';
		$this->data['action_mail'] = "Purchase_order/send_mail/".$this->uri->segment(3);
		$this->load->view('includes/template',$this->data);
	}
	...
	public function send_mail()
	{
		...
		if($right_status == false)
		{
			$this->session->set_flashdata('rights_error', "You Don't have rights to access functionality");
			redirect(base_url());
		}
		$enid = $this->uri->segment(3) ? $this->uri->segment(3) : '';
			if($this->input->post(NULL,FALSE))
			{	
				$this->load_pdf(false);
				$idencr = $this->uri->segment(3) ? $this->encrypt_decrypt('decrypt', $enid) : '';
				$lists = $this->Purchase_order_model->get($idencr);
				$success = $this->validation();
				$value = array();
				$value = $this->input->post(NULL,FALSE);
				$value = $this->security->xss_clean($value);
				...
				$uid = $this->encrypt_decrypt('decrypt',$this->session->userdata['miconlogin']['userid']);
				;
				$mailerdata = $this->Purchase_order_model->get_mailer_detail($uid);
				...
				$this->email->to($value['sqm_to']);
				if(isset($value['sqm_to_cc']) && ($value['sqm_to_cc'] != ''))
				{
					$this->email->cc($value['sqm_to_cc']);
				}
				$this->email->subject($value['sqm_sub']);
				$this->email->message($value['sqm_body']);
				$this->email->attach($path."pdf/po/po".$this->uri->segment(3).".pdf", 'attachment',''.$lists[0]['po_no'].'.pdf');
				$this->email->message("Please find attached Purchase Order");
				if($this->email->send())
				{
				echo 'Email sent.';
				}
				...
				redirect(base_url('Purchase_order/other_details/'.$enid), 'refresh');
		}
		else
		{
			$this->mail();
	...
	public function mails()
	{
		...
		$this->email->cc($mailerdata['au_gmail_email']);// change it to yours
		$this->email->subject("Aavad Purchase Order Detail : #".$lists[0]['po_no']);
		//$this->email->message("Purchase Order");
		$this->email->attach($path."pdf/po/po".$this->uri->segment(3).".pdf", 'attachment',''.$lists[0]['po_no'].'.pdf');
		$this->email->message("Please find attached Purchase Order");
		if($this->email->send())
		{
