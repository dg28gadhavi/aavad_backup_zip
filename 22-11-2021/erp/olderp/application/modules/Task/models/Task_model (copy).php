...
class Task_model extends CI_Model {
	...
	public function add($data)
	{
	    ...
	    if(isset($data['add_new']) && $data['add_new'] == 1)
	    {
		    	$item = array(
					...
					'master_party_pastcomment'=> isset($data['task_pastcomment']) ? $data['task_pastcomment'] : '',
	    ...
	    if(isset($data['task_iscompleted']) && $data['task_iscompleted'] == 1)
	    {
			...
			$update = array(
					'tr_task_id' => $lid,
					'tr_ticket_no' => $data['task_ticketno'],
					'tr_given_by' => $this->encrypt_decrypt('decrypt',$this->session->userdata['miconlogin']['userid']),
					'tr_alloted_to' => $data['task_attendedby'],
					'tr_complete_date' => date("Y-m-d H:i:s"),
					'tr_complete_remark' => $data['task_details'],
					'tr_details' => $data['task_rdetails'],
					'tr_is_completed' => 2,
					'tr_udate' => date("Y-m-d H:i:s"),
					'tr_ip' => $this->input->ip_address()
					);
			$this->db->insert('tbl_task_remainders',$update);
			return $lid;
	    }else{
			$insert = array(
				'task_subject' => $data['task_subject'],
	...
	{
		$sql = "SELECT * from tbl_master_party WHERE UPPER(master_party_com_name) LIKE '%".$this->db->escape_like_str(strtoupper($q))."%'";
