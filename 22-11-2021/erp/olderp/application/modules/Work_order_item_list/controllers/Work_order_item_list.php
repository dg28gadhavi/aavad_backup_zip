...
class Work_order_item_list extends CI_Controller {
	...
	public function __construct()
	{
		...
		$this->load->model('Work_order_item_list_model');
	...
	public function index()
	{
		...
		{
			$this->session->set_flashdata('rights_error', "You Don't have rights to access Work_order_item_list VIew functionality");
			redirect(base_url());
		}
		$this->data['main_content'] = 'Work_order_item_list_grid_view';
		$this->load->view('includes/template',$this->data);
	}

	public function ajax()
	{
		$user = $this->Work_order_item_list_model->get_Work_order_item_list();
		...
		if($right_status == false)
		{
			...
			$editstr = '<a href="'.base_url().'Work_order_item_list/edit/'.$idenc.'" class="btn btn-sm btn-outline green"><i class="fa fa-pencil"></i></a>';
		}
		...
		if($right_status == false)
		{
			...
			$deletestr = '<a href="'.base_url().'Work_order_item_list/delete/'.$idenc.'"onclick="return confirm('."'Are you sure you want to Delete this record?'".')" class="btn btn-sm btn-outline red"><i class="fa fa-times"></i></a>';
		}
		if($right_status == false)
		{
			$viewpdfstr = '';
	...
	public function Work_order_item_list_report()
	{
		...
		$this->data['admins'] = $this->Work_order_item_list_model->get_admin();
		//$this->data['custometyps'] = $this->Work_order_item_list_model->get_customertype();
		$this->data['executive'] = $this->Work_order_item_list_model->get_Executive();
		$this->data['total_counts'] = $this->Work_order_item_list_model->get_total_counts();
		$this->data['main_content'] = 'Work_order_item_list_report';
		$this->load->view('includes/template',$this->data);
	}

	public function pdf_workorder()
	{
		//$alldata = $this->Work_order_item_list_model->wo_pdfdata();
	...
	{
		$this->Work_order_item_list_model->wo_confirm();
		$this->session->set_flashdata('success', 'confirm successfully.');
					redirect(base_url('Work_order_item_list/other_details/'.$this->uri->segment(3)), 'refresh');
	}
	public function ajax_salesinq_report()
	{
		$count = $this->Work_order_item_list_model->wo_report_count();
		...
		$user = $this->Work_order_item_list_model->wo_report($iDisplayStart,$iDisplayLength);
		//echo '<pre>';print_r($user);die;
		foreach($user as $i => $vals){
		//$status = $status_list[rand(0, 2)];
		//$id = ($i + 1);
		$idenc = $this->encrypt_decrypt('encrypt',$vals['wo_id']);
		//$this->encrypt->encode($user[$i]['sq_id']);
		//$crud->columns('master_item_code','master_item_name','master_item_description','master_item_make','master_item_rating','master_item_part_no','master_item_price','master_item_stock','master_item_created_date','master_item_updated_date');
		$right_status = $this->check_rights('edit');
		if($right_status == false)
		...
		$right_status = $this->check_rights('view');
		if($user[$i]['woi_product_drawing'] == "")
		{
			$viewdrwaing = '';
		}else{
			$dep_id =  $this->encrypt_decrypt('decrypt',$this->session->userdata['miconlogin']['dep_id']);
			if($dep_id == 9 || $dep_id == 5)
			{
				$viewdrwaing = '<a title="View PDF" href="'.base_url().'uploads/woi_product_drawing/'.$user[$i]['woi_product_drawing'].'" class="btn btn-sm btn-outline blue " target="_blank"><i class="fa fa-picture-o"></i></a>';
			}else{
				$viewdrwaing = '';
			}
		...
		if($type_id == 3)
		{
			if($vals['woi_admin_approve'] == '0' && $this->input->get('adminapproved')){
				$linkstr = '<a href="'.base_url().'Dashboard_workorder_final/check_qty?wo_itemid='.$vals['woi_id'].'&itemid='.$vals['woi_item_id'].'&wo_id='.$vals['woi_wo_id'].'" class="btn btn-success"><i class="fa fa-check"></i></a>
											  <a href="'.base_url().'Dashboard_workorder_final/edit_qty?wo_itemid='.$vals['woi_id'].'&itemid='.$vals['woi_item_id'].'&wo_id='.$vals['woi_wo_id'].'" target="_BLANK" class="btn btn-danger"><i class="fa fa-pencil"></i></a>
											  <a href="'.base_url().'Dashboard_workorder_final/delete_itm?wo_itemid='.$vals['woi_id'].'&itemid='.$vals['woi_item_id'].'&wo_id='.$vals['woi_wo_id'].'"  onclick="return confirm('."'Are you sure you want to delete this records?'".')" class="btn btn-success"><i class="fa fa-close"></i></a>';
			}
		}else{
			$dep_id =  $this->encrypt_decrypt('decrypt',$this->session->userdata['miconlogin']['dep_id']);
			if($dep_id == 10)
			{
				if($vals['woi_manager_approve'] == '0' && $this->input->get('smapproved')){
				$linkstr = '<a href="'.base_url().'Dashboard_workorder_final/check_qty?wo_itemid='.$vals['woi_id'].'&itemid='.$vals['woi_item_id'].'&wo_id='.$vals['woi_wo_id'].'" class="btn btn-success"><i class="fa fa-check"></i></a>
											  <a href="'.base_url().'Dashboard_workorder_final/edit_qty?wo_itemid='.$vals['woi_id'].'&itemid='.$vals['woi_item_id'].'&wo_id='.$vals['woi_wo_id'].'" target="_BLANK" class="btn btn-danger"><i class="fa fa-pencil"></i></a>
											  <a href="'.base_url().'Dashboard_workorder_final/delete_itm?wo_itemid='.$vals['woi_id'].'&itemid='.$vals['woi_item_id'].'&wo_id='.$vals['woi_wo_id'].'"  onclick="return confirm('."'Are you sure you want to delete this records?'".')" class="btn btn-success"><i class="fa fa-close"></i></a>';
											}
			}else if($dep_id == 11){
				if($vals['woi_promanager_approve'] == '0' && $this->input->get('pmapproved')){
				$linkstr = '<a href="'.base_url().'Dashboard_workorder_final/check_qty?wo_itemid='.$vals['woi_id'].'&itemid='.$vals['woi_item_id'].'&wo_id='.$vals['woi_wo_id'].'" class="btn btn-success"><i class="fa fa-check"></i></a>
											  <a href="'.base_url().'Dashboard_workorder_final/edit_qty?wo_itemid='.$vals['woi_id'].'&itemid='.$vals['woi_item_id'].'&wo_id='.$vals['woi_wo_id'].'" target="_BLANK" class="btn btn-danger"><i class="fa fa-pencil"></i></a>
											  <a href="'.base_url().'Dashboard_workorder_final/delete_itm?wo_itemid='.$vals['woi_id'].'&itemid='.$vals['woi_item_id'].'&wo_id='.$vals['woi_wo_id'].'"  onclick="return confirm('."'Are you sure you want to delete this records?'".')" class="btn btn-success"><i class="fa fa-close"></i></a>';
											}
		...
		$vals['wo_preparedby'] = $vals['au_fname'].' '. $vals['au_lname'];
		$records["data"][] = array(
				''.$vals['wo_wo_no'],
				''.$vals['wo_wo_date'],
				''.$vals['wo_customer_name'],
				''.$vals['woi_part_no'],
				''.$vals['master_party_name'],
				''.$vals['woi_qty'],
				''.$vals['woi_price'],
				''.$vals['woi_discount'],
				''.$vals['woi_final_price'],
				''.$vals['wo_preparedby'],

				''.$vals['woi_productweight'],
				''.$vals['woi_comment'],
				
				''.$vals['wo_remark'].' '.$vals['woi_item_remark_production'],
				''.$vals['wo_udate'],
				//''.$user[$i]['sq_ref_by'],
			    $linkstr.''.$viewdrwaing
		);
	...
	public function add()
	{
		...
		if($right_status == false)
		{
			$this->session->set_flashdata('rights_error', "You Don't have rights to access Work_order_item_list Add functionality");
			redirect(base_url());
		}
		...
		if($success == TRUE)
		{
			...
			{	
				...
				$lid = $this->Work_order_item_list_model->add($value);
				$this->session->set_userdata('tabno',1);
				//***********PDF File Code Start********************
				$pdfdata = $this->Work_order_item_list_model->get_pdfdata($lid,'Work_order_item_list');
				//echo '<pre>';print_r($pdfdata);die;
				$html = $this->load->view('Work_order_item_list/Work_order_item_list_pdf_view',$pdfdata,TRUE);
				...
				if($lid)
				{
					//die;
					$this->session->set_userdata('tabno',1);
					$this->session->set_flashdata('success', 'Work_order_item_list added successfully.');
					redirect(base_url('Work_order_item_list/other_details/'.$this->encrypt_decrypt('encrypt',$lid)), 'refresh');
				}else{
					$this->session->set_flashdata('error', 'Work_order_item_list not added successfully!!');
					redirect(base_url('Work_order_item_list/add'), 'refresh');
				}
			 	redirect(base_url('Work_order_item_list'), 'refresh');
			}
		}
		if($success == FALSE)
		{
	...
	public function other_add()
	{
		...
		if($right_status == false)
		{
			...
			{	//echo '<pre>';print_r($this->input->post(NULL,FALSE));die;
				...
				$lid = $this->Work_order_item_list_model->other_add($value);
				//***********PDF File Code Start********************
				$pdfdata = $this->Work_order_item_list_model->get_pdfdata($lid,'Work_order_item_list');
				//echo '<pre>';print_r($pdfdata);die;
				$html = $this->load->view('Work_order_item_list/Work_order_item_list_pdf_view',$pdfdata,TRUE);
	...
	public function item_add()
	{
			...
			if($right_status == false)
			{
				...
				$this->session->set_userdata('tabno',2);
					...
					$result = $this->Work_order_item_list_model->item_img($value);
					//if(count($result) > 0)
					//{
				...
				$lid = $this->Work_order_item_list_model->item_add($value);
	...
	public function item_edit()
	{
		ini_set('memory_limit', '-1');
			...
			if($right_status == false)
			{
				$this->session->set_flashdata('rights_error', "You Don't have rights to access Work_order_item_list Edit functionality");
				redirect(base_url());
			}
				$value = array();
				$value = $this->input->post(NULL,FALSE);
				...
				$lid = $this->Work_order_item_list_model->item_edit($value,$sqiitemid);
				...
				{
					$this->session->set_flashdata('success', 'Details of item added successfully.');
					redirect(base_url('Work_order_item_list/other_details/'.$this->uri->segment(3).'?acttype=edit&itemid='.$sqiitemid), 'refresh');
				}else{
					$this->session->set_flashdata('error', 'Details of item not added successfully!!');
					redirect(base_url('Work_order_item_list/add'), 'refresh');
				}
	...
	public function folup_add()
	{
		...
		if($right_status == false)
		{
			...
			{	//echo '<pre>';print_r($this->input->post(NULL,FALSE));die;
				...
				$value['sq_udate'] = date('Y-m-d H:i:s');
				$this->session->set_userdata('tabno',3);
				$lid = $this->Work_order_item_list_model->folup_add($value);
				//***********PDF File Code Start********************
				$pdfdata = $this->Work_order_item_list_model->get_pdfdata($lid,'Work_order_item_list');
				//echo '<pre>';print_r($pdfdata);die;
				$html = $this->load->view('Work_order_item_list/Work_order_item_list_pdf_view',$pdfdata,TRUE);
	...
	public function other_details()
	{
		...
		//$this->data['autosearch_items'] = $this->Work_order_item_list_model->get_customertype();
		// $this->data['custometyps'] = $this->Work_order_item_list_model->get_customertype();
		 $this->data['list'] = $this->Work_order_item_list_model->get($idencr);
		// //echo '<pre>';print_r($this->data['list']);die;
		 $this->data['items'] = $this->Work_order_item_list_model->get_items($idencr);
		...
		if($this->input->get('acttype') && $this->input->get('itemid') && ($this->input->get('acttype') == 'edit'))
		{
			...
			$this->data['edit_items'] = $this->Work_order_item_list_model->get_edit_inqitems($idencr,$eitemid);
			//echo '<pre>';print_r($this->data['edit_items']);die;
			$this->data['action_itm'] = "Work_order_item_list/item_edit/".$this->uri->segment(3).'?acttype=edit&itemid='.$eitemid;
		}else{
			$this->data['action_itm'] = "Work_order_item_list/item_add/".$this->uri->segment(3);
		}
		//echo "<pre>"; print_r($this->data['items']); die;
		$this->data['confirm_or_not'] = $this->Work_order_item_list_model->get_confirm_or_not($idencr);
		$this->data['custometyps'] = $this->Work_order_item_list_model->get_customertype();
		$this->data['countries'] = $this->Work_order_item_list_model->get_country();
		$this->data['wo_no'] = $this->Work_order_item_list_model->wo_no_get();
		$this->data['vendors'] = $this->Work_order_item_list_model->get_masterparty();
		$this->data['admins'] = $this->Work_order_item_list_model->get_admin();
		$this->data['main_content'] = 'Sales_basic_details_tab';
		$this->data['action_bd'] = "Work_order_item_list/add/".$this->uri->segment(3);
		$this->data['action_othr'] = "Work_order_item_list/other_add/".$this->uri->segment(3);
		//$this->data['action_itm'] = "Work_order_item_list/item_add/".$this->uri->segment(3);
		$this->data['action_fup'] = "Work_order_item_list/folup_add/".$this->uri->segment(3);
		$this->load->view('includes/template',$this->data);
	}
	...
	{
		$this->data['custometyps'] = $this->Work_order_item_list_model->get_customertype();
		$this->data['cities'] = $this->Work_order_item_list_model->get_city();
		$this->data['countries'] = $this->Work_order_item_list_model->get_country();
		$this->data['wo_no'] = $this->Work_order_item_list_model->wo_no_get();
		$this->data['vendors'] = $this->Work_order_item_list_model->get_masterparty();
		$this->data['admins'] = $this->Work_order_item_list_model->get_admin();
		$this->data['main_content'] = 'Work_order_item_list_form_view';
		$this->data['action'] = "Work_order_item_list/add";
		$this->load->view('includes/template',$this->data);
	}
	...
	public function admin_aprove()
	{
		...
		{
			$this->Work_order_item_list_model->admin_qty_aprove($value);
			$this->session->set_userdata('tabno',3);
	...
	public function manager_aprove()
	{
		...
		$value=$this->input->post();
			...
			if($value['staff_ori_qty'] >= $value['staff_qty'])
			{
				$response=$this->Work_order_item_list_model->manager_qty_aprove($value);
	...
	public function production_aprove()
	{
		...
		if($value['staff_ori_qty'] >= $value['staff_qty'])
		{
			$response=$this->Work_order_item_list_model->production_qty_aprove($value);
			if($response['status'] == true)
			{
				$this->session->set_flashdata('success', 'successfully.');
			   redirect(base_url('Work_order_item_list/other_details/'.$this->uri->segment(3)), 'refresh');
	...
	public function store_aprove()
	{
		...
		if($value['staff_ori_qty'] >= $value['staff_qty'])
		{
			$this->Work_order_item_list_model->store_qty_aprove($value);
			$this->session->set_flashdata('success', 'successfully.');
			redirect(base_url('Work_order_item_list/other_details/'.$this->uri->segment(3)), 'refresh');
		}
		else{
	...
	public function account_aprove()
	{
		...
		if($value['staff_ori_qty'] >= $value['staff_qty'])
		{
			$this->Work_order_item_list_model->account_qty_aprove($value);
			$this->session->set_flashdata('success', 'successfully.');
			redirect(base_url('Work_order_item_list/other_details/'.$this->uri->segment(3)), 'refresh');
		}
		else{
	...
	public function dispatch_aprove()
	{
		...
		if($value['staff_ori_qty'] >= $value['staff_qty'])
		{
			$this->Work_order_item_list_model->dispatch_qty_aprove($value);
			$this->session->set_flashdata('success', 'successfully.');
			redirect(base_url('Work_order_item_list/other_details/'.$this->uri->segment(3)), 'refresh');
		}
		else{
		...
		if($enid && ($enid != ''))
		{
			...
			if(isset($id) && $id!= ''){
				...
				if(!empty($this->data['list'])){
					$lid = $this->Work_order_item_list_model->delete($idencr);
						if ($lid) {
						$this->session->set_flashdata('success', 'Work_order_item_list deleted successfully.');
						redirect('Work_order_item_list/Work_order_item_list_report', 'refresh');
						} else {
							$this->session->set_flashdata('error', 'Work_order_item_list not deleted successfully!!.');
							redirect('Work_order_item_list/Work_order_item_list_report', 'refresh'); 
						}						
				}else{
					$this->session->set_flashdata('error', 'Work_order_item_list not Available!!');
			  		redirect('Work_order_item_list/Work_order_item_list_report', 'refresh'); 
			  	}
			}
			else{
			...
			redirect(base_url('Work_order_item_list/Work_order_item_list_report'), 'refresh'); 
		}
	}
	...
	public function delete_all()
	{
		...
		if($right_status == false)
		{
			$this->session->set_flashdata('rights_error', "You Don't have rights to access Work_order_item_list Delete functionality");
			redirect(base_url());
		}
			...
			foreach($this->input->get('delid') as $enid)
			{
				if($enid && ($enid != ''))
				{
					...
					if(isset($idencr) && $idencr != ''){
						$this->data['list'] = $this->Work_order_item_list_model->get($idencr);//die;
						if(!empty($this->data['list'])){
							$lid = $this->Work_order_item_list_model->delete($idencr);
								if ($lid) {
								$this->session->set_flashdata('success', 'Work_order_item_list inquiry deleted successfully.');
								} else {
									$this->session->set_flashdata('error', 'Work_order_item_list inquiry not deleted successfully!!.');
								}						
						}else{
							$this->session->set_flashdata('error', 'Work_order_item_list inquiry not Available!!');
						}
					}
					else{
							$this->session->set_flashdata('error', 'Work_order_item_list inquiry not Available!!');
  	...
  	{		
		if($this->input->post() && isset($_FILES['userfile']['name']) && ($_FILES['userfile']['name'] != ''))
		{
			...
			if (!$this->upload->do_upload()) {
				...
				$this->data['action'] = "Work_order_item_list/importcsv";
				...
				if ($this->csvimport->get_array($file_path)) {
					...
					if(is_array($csv_array) && !empty($csv_array))
					{
						foreach ($csv_array as $row)
						{
							...
							if(isset($row['Work_order_item_list']) && ($row['Latitude'] != '') && (isset($row['Longitude'])) && (isset($row['Work_order_item_listCode'])))
							{
								$this->Work_order_item_list_model->importcsv($row);
							}
						}
	...
	public function setbit_Work_order_item_list()
	{
		$this->Work_order_item_list_model->setbit_Work_order_item_list();
	}
	...
	public function getsub_category()
	{
		...
		$this->data['sub_catlists'] = $this->Work_order_item_list_model->get_subsource($this->abc);
	...
	public function item_description()
	{
		if($this->input->post() && $this->input->post('master_item_id'))
		{
			...
			$array = $this->Work_order_item_list_model->get_item_description($id);
	...
	public function get_customer_information()
	{
		...
		{
			$this->Work_order_item_list_model->get_customer_information($value['term']);
		}
	...
	public function get_hc()
	{
		...
		if(isset($id) && ($id != ''))
		{
			$value = $this->Work_order_item_list_model->get_hcs($id);
			echo json_encode($value);
		}
	}

	public function delete_Work_order_item_list_item($id=false)
	{
		$right_status = $this->check_rights('delete');
		if($right_status == false)
		{
			$this->session->set_flashdata('rights_error', "You Don't have rights to access Work_order_item_list inquiry Delete functionality");
			redirect(base_url());
		}
		$enid = $this->uri->segment(3) ? $this->uri->segment(3) : '';
		...
		if($enid && ($enid != ''))
		{
			...
			if(isset($id) && $id!= ''){
				$this->session->set_userdata('tabno',2);
					$lid = $this->Work_order_item_list_model->delete_Work_order_item_list_item($idencr,$sa_id);
					//***********PDF File Code Start********************
					$autoid = $this->uri->segment(3) ? $this->encrypt_decrypt('decrypt', $this->uri->segment(4)) : '';
					//***********PDF File Code Start********************
				$pdfdata = $this->Work_order_item_list_model->get_pdfdata($sa_id,'Work_order_item_list');
				//echo '<pre>';print_r($pdfdata);die;
				$html = $this->load->view('Work_order_item_list/Work_order_item_list_pdf_view',$pdfdata,TRUE);
				//$html=$this->data['result_view'];
				$header='';
				...
				//***********PDF File Code End********************
						if ($sa_id) {
						$this->session->set_flashdata('success', 'Work_order_item_list inquiry deleted successfully.');
						redirect('Work_order_item_list/other_details/'.$this->uri->segment(4), 'refresh');
						} else {
							$this->session->set_flashdata('error', 'Work_order_item_list inquiry not deleted successfully!!.');
							redirect('Work_order_item_list/other_details/'.$this->uri->segment(4), 'refresh'); 
						}						
			...
			$this->session->set_flashdata('error', 'Something went wrong');
			redirect(base_url('Work_order_item_list/other_details/'.$this->uri->segment(4)), 'refresh'); 
		}
	}
	...
	public function status_act($id=false)
	{
		...
		if($enid && ($enid != ''))
		{
			...
			if(isset($id) && $id!= ''){
				$this->session->set_userdata('tabno',3);
					$lid = $this->Work_order_item_list_model->status_act($idencr);
						if ($lid) {
	...
	public function status_deact($id=false)
	{
		...
		if($enid && ($enid != ''))
		{
			...
			if(isset($id) && $id!= ''){
				$this->session->set_userdata('tabno',3);
					$lid = $this->Work_order_item_list_model->status_deact($idencr);
						if ($lid) {
						$this->session->set_flashdata('success', 'Followup status changed successfully.');
						redirect('inq-followup', 'refresh');
						} else {
	...
	public function delete_fup($id=false)
	{
		...
		if($enid && ($enid != ''))
		{
			...
			if(isset($id) && $id!= ''){
				$this->session->set_userdata('tabno',3);
					$lid = $this->Work_order_item_list_model->delete_fup($idencr);
					//***********PDF File Code Start********************
					$autoid = $this->uri->segment(3) ? $this->encrypt_decrypt('decrypt', $this->uri->segment(4)) : '';
					$pdfdata = $this->Work_order_item_list_model->get_pdfdata($autoid,'Work_order_item_list');
					//echo '<pre>';print_r($pdfdata);die;
					$html = $this->load->view('Work_order_item_list/Work_order_item_list_pdf_view',$pdfdata,TRUE);
					//$html=$this->data['result_view'];
					$header='';
	...
	public function get_item_details()
	{
		...
		if(isset($value['term']) && !empty($value['term']))
		{
			$this->Work_order_item_list_model->get_item_details($value['term']);
		}
	}
	...
	public function create_quote()
	{
		...
		$lid = $this->Work_order_item_list_model->create_qoute($id);
...
ini_set('memory_limit','3222222222222222222222222222222222222222M');
        ...
        $finalar = $this->Work_order_item_list_model->get_excel_certificate();
...
//echo '<pre>';print_r($finalar);die;
	...
	public function get_country_to_state()
	{
		...
		{
			$statelists = $this->Work_order_item_list_model->get_country_to_state($countyid);
		}else{
			$statelists = array();
		}
	...
	public function get_state_to_city()
	{
		...
		{
			$citylists = $this->Work_order_item_list_model->get_state_to_city($stateID);
		}else{
			$citylists = array();
		}
