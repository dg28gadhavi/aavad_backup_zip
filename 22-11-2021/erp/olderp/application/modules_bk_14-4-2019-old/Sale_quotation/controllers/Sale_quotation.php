...
class Sale_quotation extends CI_Controller {
	...
	public function ajax_salesqoute_report()
	{
		...
		if($right_status == false)
		{
			...
			$viewpdfstr = '<a title="View PDF" href="'.base_url().'pdf/quot/quote'.$idenc.'.pdf" class="btn btn-sm btn-outline blue" target="_blank"><i class="fa fa-eye"></i></a>';
		}
		if($right_status == false)
		{
			$emailsend = '';
	...
	public function send_mail()
	{
		...
		$enid = $this->uri->segment(3) ? $this->uri->segment(3) : '';
			...
			{	//echo '<pre>';print_r($this->input->post());die;
				...
				$this->email->attach($path."pdf/quot/quote".$this->uri->segment(3).".pdf", 'attachment',''.$lists[0]['sa_no'].'.pdf');
		...
		if($enid && ($enid != ''))
		{
			...
			if(isset($id) && $id!= ''){
				$this->session->set_userdata('qtabno',2);
					...
					$pdfdata = $this->sale_quotation_model->get_pdfdata($autoid,'sale_quotation');
					//echo '<pre>';print_r($pdfdata);die;
					$html = $this->load->view('Sale_quotation/Sale_quotation_pdf_view',$pdfdata,TRUE);
					//$html=$this->data['result_view'];
					$header='';
					$footer='';
					$pdfFilePath = FCPATH.'/pdf/quot/quote'.$this->encrypt_decrypt('encrypt',$autoid).'.pdf';
					$data['page_title'] = 'Hello world';
					ini_set('memory_limit','32222222222222222222222222M');
					$this->load->library('pdf');
					$pdf = $this->pdf->load();
...
ini_set('memory_limit','3222222222222222222222222222222222222222M');
        ...
        $fhead = array('Sr. No','Qoute. Date','Comapany name','client name','contact no','email id','city','Part no','qty','product price','grand total','inq status','source','remark');
        $i=0;
...
//echo '<pre>';print_r($finalar);die;
	...
	public function test_mail()
	{
	   ...
	   $config = array();
        ...
        $config['wordwrap'] = TRUE;
	        ...
	        $this->email->initialize($config);
	      $this->email->set_newline("\r\n");
	      $this->email->from('dd20792@gmail.com'); // change it to yours
	      $this->email->to('developer@miconinfotech.com');// change it to yours
	...
	public function test_mail_second()
	{
		$config = array();
		$config['protocol']    = 'smtp';
		$config['smtp_host']    = 'ssl://smtp.gmail.com';
		$config['smtp_port']    = '465';
		$config['smtp_timeout'] = '7';
		$config['smtp_user']    = 'erp@miconindia.com';
		$config['smtp_pass']    = 'parag@1234';
		$config['charset']    = 'utf-8';
		$config['newline']    = "\r\n";
		$config['mailtype'] = 'html'; // or html
		$config['validation'] = TRUE; // bool whether to validate email or not 
		$this->load->library('email');
		$this->email->initialize($config);
		$this->email->from('erp@miconindia.com'); // change it to yours
	      $this->email->to('erp@miconindia.com');// change it to yours
		//$this->email->cc('janak@nbgroup.in,alpa@nbgroup.in');
		$this->email->subject('Thank you for contacting us');
		//$message=$this->load->view('front/contact_email',$value,TRUE);
		//echo $message; die;
		$this->email->message('hiiiiiiiiiiiiii');
		$this->email->send();
