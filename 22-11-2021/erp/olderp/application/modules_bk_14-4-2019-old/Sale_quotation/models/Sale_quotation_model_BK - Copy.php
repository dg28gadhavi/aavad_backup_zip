...
class Sale_quotation_model extends CI_Model {
  ...
  //       if($this->session->userdata('miconlogin') && is_array($this->session->userdata['miconlogin']) && !empty($this->session->userdata['miconlogin']) && isset($this->session->userdata['miconlogin']['typeid']) && ($this->encrypt_decrypt('decrypt',$this->session->userdata['miconlogin']['typeid']) != 3) &&  isset($this->session->userdata['miconlogin']['userid']) && ($this->session->userdata['miconlogin']['userid'] != ''))
	...
	{//tbl_followup_method
		$this->db->select('tbl_sale_quotation.sa_id as id,tbl_sale_quotation.sa_enq_date as inq_dt,tbl_salesq_followup.fu_followdate as followdate,tbl_sale_quotation.sa_no as no,tbl_sale_quotation.sa_con_person as name,tbl_sale_quotation.sa_mobile as mno,tbl_sale_quotation.sa_inq_st as stname,tbl_admin_users.au_fname as executive');
		$this->db->from('tbl_sale_quotation');
		$this->db->join('tbl_salesq_followup', 'tbl_salesq_followup.fu_inq_id = tbl_sale_quotation.sa_id','left');
		...
		$this->db->join('tbl_admin_users', 'tbl_admin_users.au_id = tbl_salesq_followup.fu_followexe','left');
		...
		$this->db->order_by('tbl_salesq_followup.fu_followdate','desc');
		$this->db->where('tbl_sale_quotation.sa_isdeleted','0');

		if($this->input->post('date') && $this->input->post('date') != ''){
			$this->db->like('tbl_salesq_followup.fu_followdate', date("Y-m-d", strtotime($this->input->post('date'))));
		}
	...
	public function count()
	{
        ...
        $this->db->from('tbl_sale_quotation');
        $this->db->join('tbl_salesq_followup','tbl_salesq_followup.fu_inq_id = tbl_sale_quotation.sa_id');
		if($this->session->userdata('miconlogin') && is_array($this->session->userdata['miconlogin']) && !empty($this->session->userdata['miconlogin']) && isset($this->session->userdata['miconlogin']['typeid']) && ($this->encrypt_decrypt('decrypt',$this->session->userdata['miconlogin']['typeid']) != 3) &&  isset($this->session->userdata['miconlogin']['userid']) && ($this->session->userdata['miconlogin']['userid'] != ''))
		{
	...
	public function get_listofollow()
	{
        ...
        $this->db->select('tbl_sale_quotation.sa_id as id,tbl_sale_quotation.sa_enq_date as inq_dt,tbl_salesq_followup.fu_followdate as followdate,tbl_salesq_followup.fu_remark as fu_remark,tbl_sale_quotation.sa_mobile as mno,tbl_sale_quotation.sa_con_person as name,tbl_sale_quotation.sa_inq_st as stname,tbl_admin_users.au_fname as executive');
        $this->db->from('tbl_sale_quotation');
         $this->db->join('tbl_salesq_followup', 'tbl_salesq_followup.fu_inq_id = tbl_sale_quotation.sa_id','left');
        ...
        $this->db->or_where('tbl_salesq_followup.fu_followupst',5);
         //$this->db->where('tbl_ubasic_details.bd_bit',1);
         $this->db->order_by('tbl_salesq_followup.fu_followdate','desc');
