...
class Dispatch_mgmt extends CI_Controller {
	...
	public function __construct()
	{
		...
		$this->load->model('Dispatch_mgmt_model');
	...
	public function index()
	{
		...
		{
			$this->session->set_flashdata('rights_error', "You Don't have rights to access Dispatch_mgmt VIew functionality");
			redirect(base_url());
		}
		$this->data['main_content'] = 'Dispatch_mgmt_grid_view';
		$this->load->view('includes/template',$this->data);
	}

	public function ajax()
	{
		$user = $this->Dispatch_mgmt_model->Dispatch_mgmt_report();
		...
		$admin_users=$this->Dispatch_mgmt_model->get_admin_users();
		$admin_select='';
	...
	public function Dispatch_mgmt_report()
	{
		...
		$this->data['admins'] = $this->Dispatch_mgmt_model->get_admin();
		//$this->data['custometyps'] = $this->Dispatch_mgmt_model->get_customertype();
		//$this->data['Dispatch_mgmt'] = $this->Dispatch_mgmt_model->get_Dispatch_mgmt();
		$this->data['main_content'] = 'Dispatch_mgmt_report';
		$this->load->view('includes/template',$this->data);
	}
	...
	public function assign_production()
	{
		...
		$this->Dispatch_mgmt_model->assign_production($value,$womid);
	...
	public function store_approve()
	{
		...
		$this->Dispatch_mgmt_model->store_approve($otwiid);
		redirect(base_url('Dispatch_mgmt'), 'refresh');
	...
	{
		$this->data['custometyps'] = $this->Dispatch_mgmt_model->get_customertype();
		$this->data['cities'] = $this->Dispatch_mgmt_model->get_city();
		$this->data['countries'] = $this->Dispatch_mgmt_model->get_country();
		$this->data['wo_no'] = $this->Dispatch_mgmt_model->wo_no_get();
		$this->data['vendors'] = $this->Dispatch_mgmt_model->get_masterparty();
		$this->data['admins'] = $this->Dispatch_mgmt_model->get_admin();
		$this->data['main_content'] = 'Dispatch_mgmt_form_view';
		$this->data['action'] = "Dispatch_mgmt/add";
		$this->load->view('includes/template',$this->data);
	}

	public function Dispatch_mgmttopo()
	{
		echo "<pre>"; print_r($this->input->get()); die;
		$lid = $this->indend_model->Dispatch_mgmttopo();
		if(isset($lid))
		{
	...
	public function delete($id=false)
	{
		...
		if($enid && ($enid != ''))
		{
			$idencr = $this->uri->segment(3) ? $this->encrypt_decrypt('decrypt', $enid) : '';
			if(isset($id) && $id!= ''){
				$this->data['list'] = $this->Dispatch_mgmt_model->get($idencr);
				if(!empty($this->data['list'])){
					$lid = $this->Dispatch_mgmt_model->delete($idencr);
						if ($lid) {
						$this->session->set_flashdata('success', 'Dispatch_mgmt deleted successfully.');
						redirect('Dispatch_mgmt/Dispatch_mgmt_report', 'refresh');
						} else {
							$this->session->set_flashdata('error', 'Dispatch_mgmt not deleted successfully!!.');
							redirect('Dispatch_mgmt/Dispatch_mgmt_report', 'refresh'); 
						}						
				}else{
					$this->session->set_flashdata('error', 'Dispatch_mgmt not Available!!');
			  		redirect('Dispatch_mgmt/Dispatch_mgmt_report', 'refresh'); 
			  	}
			}
			else{
			...
			redirect(base_url('Dispatch_mgmt/Dispatch_mgmt_report'), 'refresh'); 
		}
	}
	...
	public function delete_all()
	{
		...
		if($right_status == false)
		{
			$this->session->set_flashdata('rights_error', "You Don't have rights to access Dispatch_mgmt Delete functionality");
			redirect(base_url());
		}
			...
			foreach($this->input->get('delid') as $enid)
			{
				if($enid && ($enid != ''))
				{
					...
					if(isset($idencr) && $idencr != ''){
						$this->data['list'] = $this->Dispatch_mgmt_model->get($idencr);//die;
						if(!empty($this->data['list'])){
							$lid = $this->Dispatch_mgmt_model->delete($idencr);
								if ($lid) {
								$this->session->set_flashdata('success', 'Dispatch_mgmt inquiry deleted successfully.');
								} else {
									$this->session->set_flashdata('error', 'Dispatch_mgmt inquiry not deleted successfully!!.');
								}						
						}else{
							$this->session->set_flashdata('error', 'Dispatch_mgmt inquiry not Available!!');
						}
					}
					else{
							$this->session->set_flashdata('error', 'Dispatch_mgmt inquiry not Available!!');
	...
	public function setbit_Dispatch_mgmt()
	{
		$this->Dispatch_mgmt_model->setbit_Dispatch_mgmt();
	}
	...
	public function getsub_category()
	{
		...
		$this->data['sub_catlists'] = $this->Dispatch_mgmt_model->get_subsource($this->abc);
	...
	public function item_description()
	{
		if($this->input->post() && $this->input->post('master_item_id'))
		{
			...
			$array = $this->Dispatch_mgmt_model->get_item_description($id);
	...
	public function get_customer_information()
	{
		...
		{
			$this->Dispatch_mgmt_model->get_customer_information($value['term']);
		}
	...
	public function get_hc()
	{
		...
		if(isset($id) && ($id != ''))
		{
			$value = $this->Dispatch_mgmt_model->get_hcs($id);
			echo json_encode($value);
		}
	}

	public function delete_Dispatch_mgmt_item($id=false)
	{
		$right_status = $this->check_rights('delete');
		if($right_status == false)
		{
			$this->session->set_flashdata('rights_error', "You Don't have rights to access Dispatch_mgmt inquiry Delete functionality");
			redirect(base_url());
		}
		$enid = $this->uri->segment(3) ? $this->uri->segment(3) : '';
		...
		if($enid && ($enid != ''))
		{
			...
			if(isset($id) && $id!= ''){
				$this->session->set_userdata('tabno',2);
					$lid = $this->Dispatch_mgmt_model->delete_Dispatch_mgmt_item($idencr,$sa_id);
					//***********PDF File Code Start********************
					$autoid = $this->uri->segment(3) ? $this->encrypt_decrypt('decrypt', $this->uri->segment(4)) : '';
					// $pdfdata = $this->Dispatch_mgmt_model->get_pdfdata($autoid,'Dispatch_mgmt');
					// //echo '<pre>';print_r($pdfdata);die;
					// $html = $this->load->view('Dispatch_mgmt/Dispatch_mgmt_pdf_view',$pdfdata,TRUE);
					// //$html=$this->data['result_view'];
					// $header='';
					// $footer='';
					...
					//***********PDF File Code End********************
						if ($lid) {
						$this->session->set_flashdata('success', 'Dispatch_mgmt inquiry deleted successfully.');
						redirect('Dispatch_mgmt/other_details/'.$this->uri->segment(4), 'refresh');
						} else {
							$this->session->set_flashdata('error', 'Dispatch_mgmt inquiry not deleted successfully!!.');
							redirect('Dispatch_mgmt/other_details/'.$this->uri->segment(4), 'refresh'); 
						}						
			...
			$this->session->set_flashdata('error', 'Something went wrong');
			redirect(base_url('Dispatch_mgmt/other_details/'.$this->uri->segment(4)), 'refresh'); 
		}
	}
		...
		if($enid && ($enid != ''))
		{
			...
			if(isset($id) && $id!= ''){
				$this->session->set_userdata('tabno',3);
					$lid = $this->Dispatch_mgmt_model->status_act($idencr);
						if ($lid) {
	...
	public function status_deact($id=false)
	{
		...
		if($enid && ($enid != ''))
		{
			...
			if(isset($id) && $id!= ''){
				$this->session->set_userdata('tabno',3);
					$lid = $this->Dispatch_mgmt_model->status_deact($idencr);
						if ($lid) {
						$this->session->set_flashdata('success', 'Followup status changed successfully.');
						redirect('inq-followup', 'refresh');
						} else {
	...
	public function delete_fup($id=false)
	{
		...
		if($enid && ($enid != ''))
		{
			...
			if(isset($id) && $id!= ''){
				$this->session->set_userdata('tabno',3);
					$lid = $this->Dispatch_mgmt_model->delete_fup($idencr);
					//***********PDF File Code Start********************
					$autoid = $this->uri->segment(3) ? $this->encrypt_decrypt('decrypt', $this->uri->segment(4)) : '';
					$pdfdata = $this->Dispatch_mgmt_model->get_pdfdata($autoid,'Dispatch_mgmt');
					//echo '<pre>';print_r($pdfdata);die;
	...
	public function get_item_details()
	{
		...
		if(isset($value['term']) && !empty($value['term']))
		{
			$this->Dispatch_mgmt_model->get_item_details($value['term']);
		}
	}
	...
	public function create_quote()
	{
		...
		$lid = $this->Dispatch_mgmt_model->create_qoute($id);
...
ini_set('memory_limit','3222222222222222222222222222222222222222M');
        ...
        $finalar = $this->Dispatch_mgmt_model->get_excel_certificate();
...
//echo '<pre>';print_r($finalar);die;
	...
	public function get_country_to_state()
	{
		...
		{
			$statelists = $this->Dispatch_mgmt_model->get_country_to_state($countyid);
		}else{
			$statelists = array();
		}
	...
	public function get_state_to_city()
	{
		...
		{
			$citylists = $this->Dispatch_mgmt_model->get_state_to_city($stateID);
		}else{
			$citylists = array();
		}
