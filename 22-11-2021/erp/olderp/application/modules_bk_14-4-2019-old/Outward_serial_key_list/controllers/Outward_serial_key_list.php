...
class Outward_serial_key_list extends CI_Controller {
	...
	public function __construct()
	{
		...
		$this->load->model('Outward_serial_key_list_model');
	...
	public function index()
	{
		...
		{
			$this->session->set_flashdata('rights_error', "You Don't have rights to access Outward_serial_key_list VIew functionality");
			redirect(base_url());
		}
		//$this->data['main_content'] = 'Outward_serial_key_list_grid_view';
		$this->load->view('includes/template',$this->data);
	}

	public function ajax()
	{
		$user = $this->Outward_serial_key_list_model->get_Outward_serial_key_list();
		...
		if($right_status == false)
		{
			...
			$editstr = '<a href="'.base_url().'Outward_serial_key_list/edit/'.$idenc.'" class="btn btn-sm btn-outline green"><i class="fa fa-pencil"></i></a>';
		}
		...
		if($right_status == false)
		{
			...
			$deletestr = '<a href="'.base_url().'Outward_serial_key_list/delete/'.$idenc.'"onclick="return confirm('."'Are you sure you want to Delete this record?'".')" class="btn btn-sm btn-outline red"><i class="fa fa-times"></i></a>';
		}
		if($right_status == false)
		{
			$viewpdfstr = '';
	...
	public function Outward_serial_key_list_report()
	{
		...
		$this->data['admins'] = $this->Outward_serial_key_list_model->get_admin();
		$this->data['report_result'] = $this->Outward_serial_key_list_model->get_totalreport();
		//$this->data['custometyps'] = $this->Outward_serial_key_list_model->get_customertype();
		//$this->data['Outward_serial_key_list'] = $this->Outward_serial_key_list_model->get_Outward_serial_key_list();
		$this->data['main_content'] = 'Outward_serial_key_list_report';
		$this->load->view('includes/template',$this->data);
	}
	public function sales_b2b_inq_report()
	{
		...
		$b2b_info=$this->Outward_serial_key_list_model->b2b_type_info();
		...
		$enddate=date('d-F-Y');
										$url='https://www.tradeindia.com/utils/my_inquiry.html?userid='.$b2b_info['b2b_trad_uid'].'&profile_id='.$b2b_info['b2b_trad_pid'].'&key='.$b2b_info['b2b_trad_key'].'&from_date='.$sdate_trade."&to_date=".$edate_trade.'';
								           ...
								           $inq_trade=$this->Outward_serial_key_list_model->insert_trade_btob_inq($json_trade);
		                 ...
		                 //                   $ch = curl_init($ind_url);
								           ...
								           $inq_india=$this->Outward_serial_key_list_model->insert_india_btob_inq($jsonresult);
		...
		//$this->data['b2b_inq'] = $this->Outward_serial_key_list_model->get_b2binq();
	...
	{
		ini_set('memory_limit', '-1');
		$user = $this->Outward_serial_key_list_model->get_b2binq();
		...
		{
			$editstr = '<a title="Edit" href="'.base_url().'Outward_serial_key_list/add_b2b_comments/'.$idenc.'" class="btn btn-sm btn-outline green"><i class="fa fa-comments"></i></a>';
		}
		...
		{
			$assignstr = '<a title="Assign To Yourself" href="'.base_url().'Outward_serial_key_list/assign_b2b_inq/'.$idenc.'"onclick="return confirm('."'Are you sure you want to Assign this Inquiry?'".')" class="btn btn-sm btn-outline green"><i class="fa fa-check-circle"></i></a>';
		...
		if($right_status == false)
		{
			...
			$quotestr = '<a title="Create Quotation" href="'.base_url().'Outward_serial_key_list/create_quote/'.$idenc.'"onclick="return confirm('."'Are you sure you want to Create Quotation?'".')" class="btn btn-sm btn-outline red"><i class="fa fa-plus-circle"></i></a>';
		}
		$right_status = $this->check_rights('view');
		if($right_status == false)
		{
		...
		if($enid && ($enid != ''))
		{
			...
			if($success == TRUE)
			{
				if($this->input->post(NULL,FALSE))
				{
					...
					$lid = $this->Outward_serial_key_list_model->edit_b2b($value,$idencr);
					if($lid)
					{	
						$this->session->set_flashdata('success', 'sales_b2b_inq_report edited successfully.');
						redirect(base_url('Outward_serial_key_list/add_b2b_comments/'.$enid), 'refresh');
					}else{
			...
			if($success == FALSE)
			{
				...
				{
					$this->data['list'] = $this->Outward_serial_key_list_model->get_b2binq_id($idencr);
					$this->data['chat_list'] = $this->Outward_serial_key_list_model->get_b2biq_chat($idencr);
					//echo "<pre>"; print_r($this->data['list']); die;
					if(!empty($this->data['list']))
					{
						//echo "hi"; die;
						$this->data['action'] = "Outward_serial_key_list/add_b2b_comments/".$enid;
					...
					else
					{
						 ...
						 redirect(base_url('Outward_serial_key_list/sales_b2b_inq_report'), 'refresh'); 
					}
				}
				else{
					$this->session->set_flashdata('error', 'Sales_b2b_enq_form_view not Available!!');
	...
	public function delete_chat()
	{
				...
				$lid = $this->Outward_serial_key_list_model->delete_chat($value,$chat_id);
				if($lid)
				{
					...
					redirect(base_url('Outward_serial_key_list/add_b2b_comments/'.$idencr), 'refresh');
				}
			 	redirect(base_url('Outward_serial_key_list/add_b2b_comments/'.$idencr), 'refresh');
	}
	public function assign_b2b_inq()
	{
		...
		$value = array();
				...
				$value = $this->security->xss_clean($value);
				$value['b2bu_udate'] = date('Y-m-d H:i:s');
				$lid = $this->Outward_serial_key_list_model->assign_b2b_inq($idencr);
				if($lid)
				{
	...
	public function followup()
	{
		...
		$this->data['total'] = $this->Outward_serial_key_list_model->count();
		$this->data['listfolloup'] = $this->Outward_serial_key_list_model->get_listofollow();
	...
	{
		$user = $this->Outward_serial_key_list_model->get_followup();
		//echo "<pre>"; print_r($user); die;
		$iTotalRecords = count($user);
		$iDisplayLength = intval($_REQUEST['length']);
		$iDisplayLength = $iDisplayLength < 0 ? $iTotalRecords : $iDisplayLength; 
		...
		if($right_status == false)
		{
			...
			$editstr = '<a href="'.base_url().'Outward_serial_key_list/other_details/'.$idenc.'" class="btn btn-sm btn-outline green"><i class="fa fa-search"></i> Edit</a>';
		}
		$right_status = $this->check_rights('delete');
		if($right_status == false)
		{
			...
			$deletestr = '<a href="'.base_url().'Outward_serial_key_list/delete_fup_sec/'.$fuidenc.'"onclick="return confirm('."'Are you sure you want to Delete this record?'".')" class="btn btn-sm btn-outline red"><i class=""></i> Delete</a>';
		}
		$actstr = '';
		if($user[$i]['folst'] == 6){
			$actstr = '<a href="'.base_url().'Outward_serial_key_list/status_act/'.$fuidenc.'"onclick="return confirm('."'Are you sure you want to Active this record?'".')" class="btn btn-sm btn-outline red"><i class=""></i> Active</a>';
		}
		if($user[$i]['folst'] == 5){
		$actstr = '<a href="'.base_url().'Outward_serial_key_list/status_deact/'.$fuidenc.'"onclick="return confirm('."'Are you sure you want to Deactive this record?'".')" class="btn btn-sm btn-outline red"><i class=""></i> Deactivate</a>';
		}
	...
	{
		$user = $this->Outward_serial_key_list_model->sales_inq_report();
		$iTotalRecords = count($user);
		$iDisplayLength = intval($_REQUEST['length']);
		$iDisplayLength = $iDisplayLength < 0 ? $iTotalRecords : $iDisplayLength; 
		$iDisplayStart = intval($_REQUEST['start']);
		...
		$pdfpath = ''.base_url().'pdf/Outward_serial_key_list/Outward_serial_key_list'.$idenc.'.pdf';
		$pdfpath = base_url().'Outward_serial_key_list/pdf_load?otwpdfid='.$idenc;
		//$this->encrypt->encode($user[$i]['sq_id']);
		//$crud->columns('master_item_code','master_item_name','master_item_description','master_item_make','master_item_rating','master_item_part_no','master_item_price','master_item_stock','master_item_created_date','master_item_updated_date');
		$right_status = $this->check_rights('edit');
		if($right_status == false)
		{
			...
			$editstr = '<a title="Edit" href="'.base_url().'Outward_serial_key_list/other_details/'.$idenc.'" class="btn btn-sm btn-outline green"><i class="fa fa-pencil"></i></a>';
		}
		$right_status = $this->check_rights('delete');
		if($right_status == false)
		{
			...
			$deletestr = '<a title="delete" href="'.base_url().'Outward_serial_key_list/delete/'.$idenc.'"onclick="return confirm('."'Are you sure you want to Delete this record?'".')" class="btn btn-sm btn-outline red"><i class="fa fa-times"></i></a>';
		}
		$right_status = $this->check_rights('add');
		if($right_status == false)
		{
	...
	public function add()
	{
		...
		if($right_status == false)
		{
			$this->session->set_flashdata('rights_error', "You Don't have rights to access Outward_serial_key_list Add functionality");
			redirect(base_url());
		}
		$success = $this->validation();
		...
		if($success == TRUE)
		{
			...
			{	
				...
				$lid = $this->Outward_serial_key_list_model->add($value);
				//***********PDF File Code Start********************
				// $pdfdata = $this->Outward_serial_key_list_model->get_pdfdata($lid,'Outward_serial_key_list');
				// //echo '<pre>';print_r($pdfdata);die;
				// $html = $this->load->view('Outward_serial_key_list/Outward_serial_key_list_pdf_view',$pdfdata,TRUE);
				...
				{
					//die;
					$this->session->set_flashdata('success', 'Outward_serial_key_list added successfully.');
					redirect(base_url('Outward_serial_key_list/other_details/'.$this->encrypt_decrypt('encrypt',$lid)), 'refresh');
				}else{
					$this->session->set_flashdata('error', 'Outward_serial_key_list not added successfully!!');
					redirect(base_url('Outward_serial_key_list/add'), 'refresh');
				}
			 	redirect(base_url('Outward_serial_key_list'), 'refresh');
			}
		}
		if($success == FALSE)
		{
	...
	public function other_add()
	{
		...
		if($right_status == false)
		{
			...
			{	//echo '<pre>';print_r($this->input->post(NULL,FALSE));die;
				...
				$lid = $this->Outward_serial_key_list_model->other_add($value);
				//***********PDF File Code Start********************
				$pdfdata = $this->Outward_serial_key_list_model->get_pdfdata($lid,'Outward_serial_key_list');
				//echo '<pre>';print_r($pdfdata);die;
				$html = $this->load->view('Outward_serial_key_list/Outward_serial_key_list_pdf_view',$pdfdata,TRUE);
	...
	public function item_add()
	{
			...
			//echo '<pre>';print_r($this->input->post(NULL,FALSE));die;
				...
				$this->session->set_userdata('tabno',2);
					...
					$result = $this->Outward_serial_key_list_model->item_img($value);
					//if(count($result) > 0)
					//{
				...
				$response = $this->Outward_serial_key_list_model->item_add($value);
				...
				if(isset($response['status']) && ($response['status'] == true))
				{
					...
					redirect(base_url('Outward_serial_key_list/other_details/'.$this->uri->segment(3)), 'refresh');
				}else{
	...
	public function item_edit()
	{
		ini_set('memory_limit', '-1');
			...
			if($right_status == false)
			{
				$this->session->set_flashdata('rights_error', "You Don't have rights to access Outward_serial_key_list Edit functionality");
				redirect(base_url());
			}
			//echo "<pre>"; print_r($_FILES); die;
			...
			//echo '<pre>';print_r($this->input->post(NULL,FALSE));die;
				...
				$response = $this->Outward_serial_key_list_model->item_edit($value,$sqiitemid);
				...
				{
					$this->session->set_flashdata('success_item', 'Details of item added successfully.');
					redirect(base_url('Outward_serial_key_list/other_details/'.$this->uri->segment(3).'?acttype=edit&itemid='.$sqiitemid), 'refresh');
				}else{
					$this->session->set_flashdata('error_item', $response['msg']);
					redirect(base_url('Outward_serial_key_list/other_details/'.$this->uri->segment(3).'?acttype=edit&itemid='.$sqiitemid), 'refresh');
				}
	...
	public function folup_add()
	{
		...
		if($right_status == false)
		{
			...
			{	//echo '<pre>';print_r($this->input->post(NULL,FALSE));die;
				...
				$lid = $this->Outward_serial_key_list_model->folup_add($value);
				//***********PDF File Code Start********************
				$pdfdata = $this->Outward_serial_key_list_model->get_pdfdata($lid,'Outward_serial_key_list');
				//echo '<pre>';print_r($pdfdata);die;
				$html = $this->load->view('Outward_serial_key_list/Outward_serial_key_list_pdf_view',$pdfdata,TRUE);
	...
	{
		$idencr = $this->uri->segment(3) ? $this->encrypt_decrypt('decrypt', $this->uri->segment(3)) : '';
		//$this->data['autosearch_items'] = $this->Outward_serial_key_list_model->get_customertype();
		// $this->data['custometyps'] = $this->Outward_serial_key_list_model->get_customertype();
		 $this->data['list'] = $this->Outward_serial_key_list_model->get($idencr);
		// //echo '<pre>';print_r($this->data['list']);die;
		 $this->data['items'] = $this->Outward_serial_key_list_model->get_items($idencr);
		 $this->data['wo_items'] = $this->Outward_serial_key_list_model->get_wo_items($idencr);
		...
		if($this->input->get('acttype') && $this->input->get('itemid') && ($this->input->get('acttype') == 'edit'))
		{
			...
			$this->data['edit_items'] = $this->Outward_serial_key_list_model->get_edit_inqitems($idencr,$eitemid);
			//echo '<pre>';print_r($this->data['edit_items']);die;
			$this->data['action_itm'] = "Outward_serial_key_list/item_edit/".$this->uri->segment(3).'?acttype=edit&itemid='.$eitemid;
		}else{
			$this->data['action_itm'] = "Outward_serial_key_list/item_add/".$this->uri->segment(3);
		}
		//echo "<pre>"; print_r($this->data['items']); die;
		$this->data['otw_no'] = $this->Outward_serial_key_list_model->otw_no_get();
		$this->data['admins'] = $this->Outward_serial_key_list_model->get_admin();
		$this->data['main_content'] = 'Sales_basic_details_tab';
		$this->data['action_bd'] = "Outward_serial_key_list/add/".$this->uri->segment(3);
		$this->data['action_othr'] = "Outward_serial_key_list/other_add/".$this->uri->segment(3);
		//$this->data['action_itm'] = "Outward_serial_key_list/item_add/".$this->uri->segment(3);
		$this->data['action_fup'] = "Outward_serial_key_list/folup_add/".$this->uri->segment(3);
		$this->load->view('includes/template',$this->data);
	}
	...
	public function serial_details()
	{
		...
		if($this->input->get('acttype') && $this->input->get('itemid') && ($this->input->get('acttype') == 'serial'))
		{
			...
			$this->data['itemdetails'] = $this->Outward_serial_key_list_model->get_edit_inqitems($idencr,$sitemid);
			$this->data['serialdetails'] = $this->Outward_serial_key_list_model->get_serialdetails($idencr,$sitemid);
			//echo '<pre>';print_r($this->data['edit_items']);die;
			$this->data['action_serial'] = "Outward_serial_key_list/serialkeys_add/".$this->uri->segment(3).'?acttype=serial&itemid='.$sitemid;
			if($this->input->get('pathfrom'))
			{
				$this->data['action_serial'] .= "&pathfrom=wofinal";
			}
			$this->data['action_itm'] = "Outward_serial_key_list/item_edit/".$this->uri->segment(3).'?acttype=edit&itemid='.$sitemid;
			$this->data['action_bd'] = "Outward_serial_key_list/add/".$this->uri->segment(3);
		}
		//echo "<pre>"; print_r($this->data['items']); die;
		$this->data['main_content'] = 'Sales_basic_details_tab';
	...
	public function serialkeys_add()
	{
			...
			{	
				...
				$value['otw_udate'] = date('Y-m-d H:i:s');
				//$this->session->set_userdata('tabno',2);
				$lid = $this->Outward_serial_key_list_model->serialkeys_add($value);
				...
				if($lid)
				{
					...
					redirect(base_url('Outward_serial_key_list/serial_details/'.$this->encrypt_decrypt('encrypt',$lid)), 'refresh');
					
				}else{
					$this->session->set_userdata('tabno',2);
					$sitemid = $this->input->get('itemid');
					redirect(base_url('Outward_serial_key_list/other_details/'.$this->uri->segment(3).'?itemid='.$sitemid), 'refresh');
				}
			 	redirect(base_url('Outward_serial_key_list'), 'refresh');
			}
	...
	{
		$response=$this->Outward_serial_key_list_model->wotopo();
		if(isset($response['status']) && ($response['status'] == true))
		{
			$this->session->set_flashdata('success_item', 'Details of item added successfully.');
		redirect(base_url('Outward_serial_key_list/other_details/'.$this->uri->segment(3).'?acttype=edit&itemid='.$response['last_insert_id']), 'refresh');
		}else{
			$this->session->set_flashdata('error_item', $response['msg']);
			//die;
			redirect(base_url('Outward_serial_key_list/other_details/'.$this->uri->segment(3)), 'refresh');
	...
	public function get_form()
	{
		$this->data['custometyps'] = $this->Outward_serial_key_list_model->get_customertype();
		$this->data['otw_no'] = $this->Outward_serial_key_list_model->otw_no_get();
		$this->data['vendors'] = $this->Outward_serial_key_list_model->get_masterparty();
		$this->data['admins'] = $this->Outward_serial_key_list_model->get_admin();
		$this->data['main_content'] = 'Outward_serial_key_list_form_view';
		$this->data['action'] = "Outward_serial_key_list/add";
		$this->load->view('includes/template',$this->data);
	}
	...
	public function delete($id=false)
	{
		...
		if($enid && ($enid != ''))
		{
			...
			if(isset($id) && $id!= ''){
				$this->data['list'] = $this->Outward_serial_key_list_model->get($idencr);
				if(!empty($this->data['list'])){
					$lid = $this->Outward_serial_key_list_model->delete($idencr);
						if ($lid) {
						$this->session->set_flashdata('success', 'Outward_serial_key_list deleted successfully.');
						redirect('Outward_serial_key_list/Outward_serial_key_list_report', 'refresh');
						} else {
							$this->session->set_flashdata('error', 'Outward_serial_key_list not deleted successfully!!.');
							redirect('Outward_serial_key_list/Outward_serial_key_list_report', 'refresh'); 
						}						
				}else{
					$this->session->set_flashdata('error', 'Outward_serial_key_list not Available!!');
			  		redirect('Outward_serial_key_list/Outward_serial_key_list_report', 'refresh'); 
			  	}
			}
			else{
			...
			$this->session->set_flashdata('error', 'Something went wrong');
			redirect(base_url('Outward_serial_key_list/Outward_serial_key_list_report'), 'refresh'); 
		}
	}
	...
	public function delete_all()
	{
		...
		if($right_status == false)
		{
			$this->session->set_flashdata('rights_error', "You Don't have rights to access Outward_serial_key_list Delete functionality");
			redirect(base_url());
		}
		...
		if($myid && is_array($myid) && !empty($myid))
		{
			foreach($this->input->get('delid') as $enid)
			{
				if($enid && ($enid != ''))
				{
					...
					if(isset($idencr) && $idencr != ''){
						$this->data['list'] = $this->Outward_serial_key_list_model->get($idencr);//die;
						if(!empty($this->data['list'])){
							$lid = $this->Outward_serial_key_list_model->delete($idencr);
								if ($lid) {
								$this->session->set_flashdata('success', 'Outward_serial_key_list inquiry deleted successfully.');
								} else {
									$this->session->set_flashdata('error', 'Outward_serial_key_list inquiry not deleted successfully!!.');
								}						
						}else{
							$this->session->set_flashdata('error', 'Outward_serial_key_list inquiry not Available!!');
						}
					}
					else{
							$this->session->set_flashdata('error', 'Outward_serial_key_list inquiry not Available!!');
  	...
  	{		
		if($this->input->post() && isset($_FILES['userfile']['name']) && ($_FILES['userfile']['name'] != ''))
		{
			...
			if (!$this->upload->do_upload()) {
				...
				$this->data['action'] = "Outward_serial_key_list/importcsv";
				...
				if ($this->csvimport->get_array($file_path)) {
					...
					if(is_array($csv_array) && !empty($csv_array))
					{
						foreach ($csv_array as $row)
						{
							...
							if(isset($row['Outward_serial_key_list']) && ($row['Latitude'] != '') && (isset($row['Longitude'])) && (isset($row['Outward_serial_key_listCode'])))
							{
								$this->Outward_serial_key_list_model->importcsv($row);
							}
						}
	...
	public function pdf_load()
		{
				$files = glob(FCPATH.'pdf/Outward_serial_key_list/*.pdf'); //get all file names
				...
				//echo $lid;
				$pdfdata = $this->Outward_serial_key_list_model->get_pdfdata($lid);
				$html = $this->load->view('Outward_serial_key_list/Outward_serial_key_list_pdf_view',$pdfdata,TRUE);
				$redirectpath = base_url().'/pdf/Outward_serial_key_list/Outward_serial_key_list'.$lid.'.pdf';
				//$html=$this->data['result_view'];
				$header='';
				$footer='';
				$pdfFilePath = FCPATH.'/pdf/Outward_serial_key_list/Outward_serial_key_list'.$lid.'.pdf';
				$data['page_title'] = 'Hello world';
				ini_set('memory_limit','32M');
				$this->load->library('pdf');
				$pdf = $this->pdf->load();
	...
	public function setbit_Outward_serial_key_list()
	{
		$this->Outward_serial_key_list_model->setbit_Outward_serial_key_list();
	}
	...
	public function getsub_category()
	{
		...
		$this->data['sub_catlists'] = $this->Outward_serial_key_list_model->get_subsource($this->abc);
	...
	public function item_description()
	{
		if($this->input->post() && $this->input->post('master_item_id'))
		{
			...
			$array = $this->Outward_serial_key_list_model->get_item_description($id);
	...
	public function get_customer_information()
	{
		...
		{
			$this->Outward_serial_key_list_model->get_customer_information($value['term']);
		}
	...
	public function get_hc()
	{
		...
		if(isset($id) && ($id != ''))
		{
			$value = $this->Outward_serial_key_list_model->get_hcs($id);
			echo json_encode($value);
		}
	}

	public function delete_Outward_serial_key_list_item($id=false)
	{
		$right_status = $this->check_rights('delete');
		if($right_status == false)
		{
		...
		if($enid && ($enid != ''))
		{
			...
			if(isset($id) && $id!= ''){
				$this->session->set_userdata('tabno',2);
					$lid = $this->Outward_serial_key_list_model->delete_Outward_serial_key_list_item($idencr,$sa_id);
					//***********PDF File Code Start********************
					$autoid = $this->uri->segment(3) ? $this->encrypt_decrypt('decrypt', $this->uri->segment(4)) : '';
					// $pdfdata = $this->Outward_serial_key_list_model->get_pdfdata($autoid,'Outward_serial_key_list');
					// //echo '<pre>';print_r($pdfdata);die;
					// $html = $this->load->view('Outward_serial_key_list/Outward_serial_key_list_pdf_view',$pdfdata,TRUE);
					// //$html=$this->data['result_view'];
					// $header='';
					...
					//***********PDF File Code End********************
						if ($lid) {
						$this->session->set_flashdata('success', 'Outward_serial_key_list inquiry deleted successfully.');
						redirect('Outward_serial_key_list/other_details/'.$this->uri->segment(4), 'refresh');
						} else {
							$this->session->set_flashdata('error', 'Outward_serial_key_list inquiry not deleted successfully!!.');
							redirect('Outward_serial_key_list/other_details/'.$this->uri->segment(4), 'refresh'); 
						}						
			...
			$this->session->set_flashdata('error', 'Something went wrong');
			redirect(base_url('Outward_serial_key_list/other_details/'.$this->uri->segment(4)), 'refresh'); 
		}
	}
	...
	public function status_act($id=false)
	{
		...
		if($right_status == false)
		{
			$this->session->set_flashdata('rights_error', "You Don't have rights to access Outward_serial_key_list inquiry Delete functionality");
			redirect(base_url());
		}
		$enid = $this->uri->segment(3) ? $this->uri->segment(3) : '';
		if($enid && ($enid != ''))
		{
			...
			if(isset($id) && $id!= ''){
				$this->session->set_userdata('tabno',3);
					$lid = $this->Outward_serial_key_list_model->status_act($idencr);
						if ($lid) {
	...
	public function status_deact($id=false)
	{
		...
		if($enid && ($enid != ''))
		{
			...
			if(isset($id) && $id!= ''){
				$this->session->set_userdata('tabno',3);
					$lid = $this->Outward_serial_key_list_model->status_deact($idencr);
						if ($lid) {
						$this->session->set_flashdata('success', 'Followup status changed successfully.');
						redirect('inq-followup', 'refresh');
						} else {
	...
	public function delete_fup($id=false)
	{
		...
		if($enid && ($enid != ''))
		{
			...
			if(isset($id) && $id!= ''){
				$this->session->set_userdata('tabno',3);
					$lid = $this->Outward_serial_key_list_model->delete_fup($idencr);
					//***********PDF File Code Start********************
					$autoid = $this->uri->segment(3) ? $this->encrypt_decrypt('decrypt', $this->uri->segment(4)) : '';
					$pdfdata = $this->Outward_serial_key_list_model->get_pdfdata($autoid,'Outward_serial_key_list');
					//echo '<pre>';print_r($pdfdata);die;
	...
	public function get_item_details()
	{
		...
		if(isset($value['term']) && !empty($value['term']))
		{
			$this->Outward_serial_key_list_model->get_item_details($value['term']);
		}
	}
	...
	public function create_quote()
	{
		...
		$lid = $this->Outward_serial_key_list_model->create_qoute($id);
...
ini_set('memory_limit','3222222222222222222222222222222222222222M');
        ...
        $finalar = $this->Outward_serial_key_list_model->get_excel_certificate();
...
//echo '<pre>';print_r($finalar);die;
	...
	public function get_country_to_state()
	{
		...
		{
			$statelists = $this->Outward_serial_key_list_model->get_country_to_state($countyid);
		}else{
			$statelists = array();
		}
	...
	public function get_state_to_city()
	{
		...
		{
			$citylists = $this->Outward_serial_key_list_model->get_state_to_city($stateID);
		}else{
			$citylists = array();
		}
