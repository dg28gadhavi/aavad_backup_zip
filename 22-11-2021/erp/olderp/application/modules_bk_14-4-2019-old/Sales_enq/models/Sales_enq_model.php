...
class Sales_enq_model extends CI_Model {
	...
	{   
		...
		// 		'master_party_contact_person' => $data['sq_con_person'],
		// 		'master_party_address' => $data['sq_address'],
		// 		'master_party_email_address' => $data['sq_email'],
		// 		'master_party_phone' => $data['sq_phone'],
		// 		'master_party_contact_no ' => $data['sq_mobile'],
		// 		'master_party_webpage' => $data['sq_website'],
		// 		'master_party_customertype' => $data['sq_cust_type'],
		// 		'master_party_country' => isset($data['sq_country']) ? $data['sq_country'] : 0,
		// 		'master_party_state' => isset($data['sq_state']) ? $data['sq_state'] : 0,
		// 		'master_party_city' => isset($data['sq_city']) ? $data['sq_city'] : 0,
		// 		'master_party_created_date' => $data['sq_cdate'],
		// 		'master_party_updated_date' => $data['sq_udate']
		// 		);
		...
		if($this->uri->segment(3))
		{
			...
			// 'sq_rem_date' => date("Y-m-d", strtotime($data['sq_rem_date'])),sq_cust_type
			// 'sq_rem_be_date' => $data['sq_rem_be_date'],
	...
	public function get_sales_enq()
	{
		
		$this->db->select('*');//sum(tbl_sales_enq.inv_grand_total) as grand_total_view
		$this->db->from('tbl_sales_enq');
		$this->db->where('sq_isdeleted',0);
		$this->db->join('tbl_mode_inquiry','tbl_sales_enq.sq_mode_inq = tbl_mode_inquiry.mode_inquiry_id');
  ...
  //       if($this->session->userdata('miconlogin') && is_array($this->session->userdata['miconlogin']) && !empty($this->session->userdata['miconlogin']) && isset($this->session->userdata['miconlogin']['typeid']) && ($this->encrypt_decrypt('decrypt',$this->session->userdata['miconlogin']['typeid']) != 3) &&  isset($this->session->userdata['miconlogin']['userid']) && ($this->session->userdata['miconlogin']['userid'] != ''))
		// {
		// 	$this->db->where('tbl_sales_enq.sq_id', $this->encrypt_decrypt('decrypt',$this->session->userdata['miconlogin']['userid']));
		// }
		$query = $this->db->get();
		//echo "<pre>";print_r($query->result_array());die;
		return $query->result_array();
	}
	public function get_sales_enq_report()
	{
		$this->db->select('*');//sum(tbl_sales_enq.inv_grand_total) as grand_total_view
		$this->db->from('tbl_sales_enq');
		$this->db->where('sq_isdeleted',0);
		...
		if($this->input->get('party')){
			$this->db->where('tbl_sales_enq.sq_id', $this->input->get('party'));
		}
	...
	public function get_sales_enqno_wise()
	{
		...
		$this->db->from('tbl_sales_enq');
		//$this->db->join('tbl_master_party','tbl_sales_enq.vendor = tbl_master_party.master_party_id');
		//$this->db->join('tbl_sales_enq_item','tbl_sales_enq.inv_id = tbl_sales_enq_item.invi_inv_id');
		//$this->db->join('tbl_master_item','tbl_sales_enq_item.invi_itm_name = tbl_master_item.master_item_id');
		//$this->db->where('sq_cid',$this->session->userdata['login']['aus_Id']);
	...
	public function sales_enq_report()
	{
		$this->db->select('*');
		$this->db->from('tbl_sales_enq');
		//$this->db->join('tbl_master_party', 'tbl_master_party.master_party_id = tbl_sales_enq.vendor');
		if($this->input->get('party')){
			$this->db->where('tbl_sales_enq.sq_id', $this->input->get('party'));
		}
		if($this->input->get('created_start_date') && ($this->input->get('created_start_date') != ''))
	...
	//**************************************************End customer wise
		// $this->db->select('*');
		// $this->db->from('tbl_sales_enq_item');
		// $this->db->join('tbl_master_item', 'tbl_master_item.master_item_id = tbl_sales_enq_item.sqi_sales_enq_id');
		// $this->db->join('tbl_sales_enq', 'tbl_sales_enq.sq_id = tbl_sales_enq_item.sqi_sales_enq_id');
		// if($this->input->get('items')){
		// 	$this->db->where('tbl_sales_enq_item.sqi_sales_enq_id', $this->input->get('items'));
		// }
		// if($this->input->get('created_start_date') && ($this->input->get('created_start_date') != ''))
		// {
		// 	$stdate = date("Y-m-d", strtotime($this->input->get('created_start_date')));
	...
	public function get_excel_certificate()
	{
		...
		// $this->db->join('tbl_source_cat as subsource','tbl_sales_enq.sa_subsource_cat =  subsource.source_main_cat','left');
		//$this->db->join('tbl_admin_users','tbl_sales_enq.sq_end_st = tbl_admin_users.aus_Id');
		//$this->db->where('tbl_sales_enq_item.sai_oa_completed',0);
		// $this->db->where('tbl_sales_enq_item.sqi_item_isdelete',0);
    ...
    {
    	$this->db->select('sum(tbl_sales_enq.sq_grd_ttl) as grand_total_view');
    	$this->db->from('tbl_sales_enq');
    	$query = $this->db->get();
		//echo'<pre>';print_r($query->result_array());die;
		return $query->result_array();
    ...
    {
    	$this->db->select('sum(tbl_sales_enq_item.sqi_itm_ftotal) as sub_total_view');
    	$this->db->from('tbl_sales_enq_item');
    	$query = $this->db->get();
		//echo'<pre>';print_r($query->result_array());die;
		return $query->result_array();
    ...
    {
    	$this->db->select('sum(tbl_sales_enq_itax.sqit_tax_amount) as grand_tax_amount');
    	$this->db->from('tbl_sales_enq_itax');
    	$query = $this->db->get();
		return $query->result_array();
    }
    public function get_inqno_wise()
    {
    	$this->db->select('*');
		$this->db->from('tbl_sales_enq');
		$this->db->join('tbl_master_party', 'tbl_master_party.master_party_id = tbl_sales_enq.vendor','left');
		$this->db->order_by('tbl_sales_enq.sq_id','DESC');
    	$query = $this->db->get();
		return $query->result_array();
    }
	...
	public function get_usalesdata($idenc)
	{
		...
		$this->db->join('tbl_master_item','tbl_sales_enq_item.sqi_itm_name = tbl_master_item.master_item_id');
	...
	public function create_inq($id)
	{
		...
		//echo "<pre>";print_r($value['salesb2binq']);die;
		...
		// $code = $query->row_array();
		// $autoid['sq_id'] = isset($autoid['sq_id']) ? $autoid['sq_id'] : '';
		// $inq_no = $code['pre_enquiry'].''.($autoid['sq_id']+1);
		$maininq_no = $this->sq_no_get();
		$item = array(
			'sq_no' => $maininq_no,
	...
	public function create_qoute($id)
	{
		...
		$item = array(
			...
			'sa_tc_price' => $value['salesinq']['sales_enq_tc_price'],
			'sa_tc_wrnty' => $value['salesinq']['sales_enq_tc_wrnty'],
			'sa_tc_pf' => $value['salesinq']['sales_enq_tc_pf'],
			'sa_tc_deli' => $value['salesinq']['sales_enq_tc_deli'],
			'sa_tc_paynt' => $value['salesinq']['sales_enq_tc_paynt'],
			'sa_tc_ovali' => $value['salesinq']['sales_enq_tc_ovali'],
			'sa_tc_frght' => $value['salesinq']['sales_enq_tc_frght'],
			'sa_tc_gst' => $value['salesinq']['sales_enq_tc_gst'],
