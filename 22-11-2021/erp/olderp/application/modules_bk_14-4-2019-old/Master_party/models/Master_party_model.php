...
class Master_party_model extends CI_Model {
	...
	public function add($data)
	{
		if($this->uri->segment(3))
		{
			...
			'master_party_area' => $data['master_party_area'],
	...
	public function importcsv($data)
	{
		//echo '<pre>';print_r($data);die;
		$item = array(
			'master_party_com_name' => $data['Name'],
			'master_party_name' => $data['Name'],
			'master_party_email_address' => $data['Email Id'],
			'master_party_mobile_no' => $data['Cell No'],
			'master_party_office_no' => $data['Phone'],
			'master_party_fax_no' => $data['Fax'],
			'master_party_website' => $data['Web Site'],
			'master_party_office_address' => $data['Address'],
			'master_party_billing_address' => $data['Address'],
			'master_party_shipping_address' => $data['Address'],
			'master_party_pincode' => $data['Pin Code'],
	...
	public function get_Master_party_report()
	{
        ...
        {
           $this->db->like('master_party_office_address', $this->input->post('master_party_office_address'));   
        }
         if($this->input->post('master_party_email_address') && ($this->input->post('master_party_email_address') != ''))
        {
         ...
         if($this->input->post('master_party_office_no') && ($this->input->post('master_party_office_no') != ''))
        {
           $this->db->like('master_party_office_no', $this->input->post('master_party_office_no'));   
        }
