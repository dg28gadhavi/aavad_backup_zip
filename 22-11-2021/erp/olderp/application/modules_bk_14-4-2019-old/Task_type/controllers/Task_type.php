...
class Task_type extends CI_Controller {
	...
	{
		$user = $this->Task_type_model->get_Task_type();
	...
	public function add()
	{
		...
		if($success == TRUE)
		{
			if($this->input->post(NULL,FALSE))
			{
				...
				$value['task_adid'] = $this->encrypt_decrypt('decrypt',$this->session->userdata['miconlogin']['userid']);
				$value['task_atype'] = $this->encrypt_decrypt('decrypt',$this->session->userdata['miconlogin']['typeid']);
	...
	public function validation() 
	{
		...
		{
			$this->load->library('form_validation');
			$this->form_validation->set_rules('task_name', 'task_name name', 'trim|required');  
