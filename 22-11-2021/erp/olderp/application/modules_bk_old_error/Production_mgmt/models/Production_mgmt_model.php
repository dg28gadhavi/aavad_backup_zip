...
class Production_mgmt_model extends CI_Model {
	...
	public function get_admin_users($womid)
	{
		$this->db->select('*');
		...
		if($womdata['wom_status']=='1')
		{
			$this->db->select('*');
			$this->db->from('tbl_admin_users');
			$this->db->where_in('au_dep_id',array(5));
			$query = $this->db->get();
			...
			$this->db->select('*');
			$this->db->from('tbl_admin_users');
			$this->db->where_in('au_dep_id',array(2));
			$query = $this->db->get();
			return $query->result_array();
		}
	}

	public function assign_store($data,$womid)
	{
	...
	public function Production_mgmt_report()
	{
		...
		// $this->db->where('otwi_otw_status','1');
		...
		$this->db->where_in('wom_status',array('1','3'));
		$this->db->where('wom_completed',0);
		// $this->db->where('wom_status','1');
		// $this->db->where('wom_status','3');
		$query = $this->db->get();
