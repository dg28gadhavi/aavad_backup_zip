...
class Country extends CI_Controller {
	...
	public function index()
	{
		...
		{
			$this->session->set_flashdata('rights_error', "You Don't have rights to access Country VIew functionality");
			redirect(base_url());
		}
		$this->data['main_content'] = 'Country_grid_view';
		$this->load->view('includes/template',$this->data);
	}

	public function ajax()
	{
		$user = $this->country_model->get_country();
		...
		$id = ($i + 1);
		$idenc = $this->encrypt_decrypt('encrypt',$user[$i]['country_id']);
		//$this->encrypt->encode($user[$i]['country_id']);
		...
		if($right_status == false)
		{
			...
			$editstr = '<a href="'.base_url().'Country/edit/'.$idenc.'" class="btn btn-sm btn-outline green"><i class="fa fa-pencil"></i></a>';
		}
		...
		if($right_status == false)
		{
			...
			$deletestr = '<a href="'.base_url().'Country/delete/'.$idenc.'"onclick="return confirm('."'Are you sure you want to Delete this record?'".')" class="btn btn-sm btn-outline red"><i class="fa fa-times"></i></a>';
		}
		$records["data"][] = array(
			  '<input type="checkbox" name="delid[]" value="'.$user[$i]['country_id'].'">',
			  $id,
			  ''.$user[$i]['country_name'],
			  //''.$user[$i]['country_roe'],
			  ''.date("m-d-Y", strtotime($user[$i]['country_cdate'])),
			  ''.date("m-d-Y", strtotime($user[$i]['country_udate'])),
			  ''.$editstr.''.$deletestr.'',
		);
		}
	...
	public function add()
	{
		...
		if($right_status == false)
		{
			$this->session->set_flashdata('rights_error', "You Don't have rights to access Country Add functionality");
			redirect(base_url());
		}
		...
		if($success == TRUE)
		{
			if($this->input->post(NULL,FALSE))
			{
				...
				$lid = $this->country_model->add($value);
				if($lid)
				{
					$this->session->set_flashdata('success', 'Country added successfully.');
					redirect(base_url('Country'), 'refresh');
				}else{
					$this->session->set_flashdata('error', 'Country not added successfully!!');
	...
	public function edit($id = FALSE)
	{
		...
		if($right_status == false)
		{
			$this->session->set_flashdata('rights_error', "You Don't have rights to access Country Edit functionality");
			redirect(base_url());
		}
		//require 'Zebra_Image.php';
		...
		if($enid && ($enid != ''))
		{
			...
			if($success == TRUE)
			{
				if($this->input->post(NULL,FALSE))
				{
					...
					$lid = $this->country_model->edit($value,$idencr);
					if($lid)
					{
						$this->session->set_flashdata('success', 'Country edited successfully.');
						redirect(base_url('Country'), 'refresh');
					}else{
						$this->session->set_flashdata('error', 'Country not edited successfully!!');
					}
				 	redirect(base_url('Country'), 'refresh');
				}
			}
			if($success == FALSE)
			{
				if(isset($idencr) && $idencr != ''){
					$this->data['list'] = $this->country_model->get($idencr);
					//echo "<pre>"; print_r($this->data['list']); die;
					if(!empty($this->data['list']))
					{
						//echo "hi"; die;
						$this->data['action'] = "Country/edit/".$enid;
						$this->data['main_content'] = 'Country_form_view';
						$this->load->view('includes/template',$this->data);
						//parent::load_view('admin/master/country/Country_form_view',$this->data);
					}
					else
					{
						 $this->session->set_flashdata('error', 'Country not Available!!');
						 redirect(base_url('Country'), 'refresh'); 
					}
				}
				else{
	...
	public function validation() 
	{
		if($this->input->post(NULL,TRUE))
		{
			...
			if($this->uri->segment(2) == 'add'){
				...
				$enid = $this->uri->segment(3) ? $this->uri->segment(3) : '';
				$idencr = $this->uri->segment(3) ? $this->encrypt_decrypt('decrypt', $enid) : '';
				$this->form_validation->set_rules('country_name', 'country name', 'trim|required|edit_unique[tbl_country.country_name.'.$idencr.'.country_id]');  
			}
	...
	{
		//$this->data['getcountry'] = $this->country_model->addtcountry();
		$this->data['main_content'] = 'Country_form_view';
		$this->data['action'] = "Country/add";
		$this->load->view('includes/template',$this->data);
	}
	...
	public function delete($id=false)
	{
		...
		if($right_status == false)
		{
			$this->session->set_flashdata('rights_error', "You Don't have rights to access Country Delete functionality");
			redirect(base_url());
		}
		$enid = $this->uri->segment(3) ? $this->uri->segment(3) : '';
		if($enid && ($enid != ''))
		{
			...
			if(isset($id) && $id!= ''){
				$this->data['list'] = $this->country_model->get($idencr);
				if(!empty($this->data['list'])){
					$lid = $this->country_model->delete($idencr);
						if ($lid) {
						$this->session->set_flashdata('success', 'Country deleted successfully.');
						redirect('Country', 'refresh');
						} else {
							$this->session->set_flashdata('error', 'Country not deleted successfully!!.');
							redirect('Country', 'refresh'); 
						}						
				}else{
					$this->session->set_flashdata('error', 'Country not Available!!');
		...
		if($myid && is_array($myid) && !empty($myid))
		{
			foreach($this->input->get('delid') as $enid)
			{
				if($enid && ($enid != ''))
				{
					...
					if(isset($idencr) && $idencr != ''){
						$this->data['list'] = $this->country_model->get($idencr);//die;
						if(!empty($this->data['list'])){
							$lid = $this->country_model->delete($idencr);
								if ($lid) {
								$this->session->set_flashdata('success', 'Country deleted successfully.');
				...
				if ($this->csvimport->get_array($file_path)) {
					...
					if(is_array($csv_array) && !empty($csv_array))
					{
						foreach ($csv_array as $row)
						{
							...
							if(isset($row['Country']) && ($row['Latitude'] != '') && (isset($row['Longitude'])) && (isset($row['countryCode'])))
							{
								$this->country_model->importcsv($row);
							}
						}
	...
	public function check_rights($type)
	{
		...
		if(isset($this->session->userdata['miconlogin']['rightsid']) && ($this->session->userdata['miconlogin']['rightsid'] != '') && isset($this->session->userdata['miconlogin']['typeid']) && ($this->session->userdata['miconlogin']['typeid'] != ''))
		{
			...
			if($typeid != 3)
			{
				...
				$finalrights = $this->global_model->get_rights($rightsid,$moduleid = 21,$type);
	...
	{
		$this->country_model->setbit_country();
	}
