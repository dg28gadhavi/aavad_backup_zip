...
class master_vendor_model extends CI_Model {
	...
	public function add($data)
	{
		if($this->uri->segment(3))
		{
			...
			$item = array(
			'master_vendor_com_name' => $data['master_vendor_com_name'],
			'master_vendor_contact' => $data['master_vendor_contact'],
			'master_vendor_cust_type' => $data['master_vendor_cust_type'],
			'master_vendor_location' => $data['master_vendor_location'],
			'master_vendor_country' => $data['master_vendor_country'],
			'master_vendor_currency' => $data['master_vendor_currency'],
			'master_vendor_state' => $data['master_vendor_state'],
			'master_vendor_city' => $data['master_vendor_city'],
			'master_vendor_email_address' => $data['master_vendor_email_address'],
			'master_vendor_mobile_no' => $data['master_vendor_mobile_no'],
			'master_vendor_office_no' => $data['master_vendor_office_no'],
			'master_vendor_fax_no' => $data['master_vendor_fax_no'],
			'master_vendor_website' => $data['master_vendor_website'],
			'master_vendor_office_address' => $data['master_vendor_office_address'],
			'master_vendor_billing_address' => $data['master_vendor_billing_address'],
			'master_vendor_shipping_address' => $data['master_vendor_shipping_address'],
			'master_vendor_pincode' => $data['master_vendor_pincode'],
			'master_vendor_gst' => $data['master_vendor_gst'],
			'master_vendor_tax' => $data['master_vendor_tax'],
			'master_vendor_co_ai'=> isset($data['master_vendor_co_ai']) ? $data['master_vendor_co_ai'] : '',
			'master_vendor_remark'=> isset($data['master_vendor_remark']) ? $data['master_vendor_remark'] : '',
			'master_vendor_cdate' =>date("Y-m-d", strtotime($data['master_vendor_cdate'])),
			'master_vendor_udate' => date("Y-m-d", strtotime($data['master_vendor_udate']))
			);
			$this->db->where('master_vendor_id', $id);
			$this->db->update('tbl_master_vendor',$item); 
			//die;
			$log = array(
					...
					'adlog_module' => 'Master Vendor Edit',
					'adlog_edit' => 1,
		...
		//echo '<pre>';print_r($item);die;
		$this->db->insert('tbl_master_vendor',$item);
		...
		$log = array(
					...
					'adlog_datetime' => date('Y-m-d H:i:s'),
					'adlog_ip' =>$_SERVER['REMOTE_ADDR'],
					'adlog_module' => 'Master Vendor Add',
					'adlog_add' => 1,
					'adlog_userdetails' => $_SERVER['HTTP_USER_AGENT']
				);
			$this->db->insert('tbl_adminlogs',$log);
	...
	public function importcsv($data)
	{
		...
		$this->db->where('master_vendor_com_name',$data['COMPANY NAME']);
		$query = $this->db->get();
		$c_data = $query->row_array();
		if($c_data['master_vendor_id'] == 0 || $c_data['master_vendor_id'] == '')
		{
			$item = array(
				'master_vendor_com_name' => isset($data['COMPANY NAME']) ? $data['COMPANY NAME'] : '',
				'master_vendor_contact' => isset($data['CONTACT PERSON']) ? $data['CONTACT PERSON'] : '',
				'master_vendor_contact_department' => isset($data['DEPARMANT']) ? $data['DEPARMANT'] : '',
				'master_vendor_email_address' => isset($data['EMAIL']) ? $data['EMAIL'] : '',
				'master_vendor_mobile_no' => isset($data['MO NO']) ? $data['MO NO'] : '',
				'master_vendor_office_no' => isset($data['LANDLINE NO']) ? $data['LANDLINE NO'] : '',
				'master_vendor_office_address' => isset($data['ADDRESS']) ? $data['ADDRESS'] : '',
				'master_vendor_billing_address' => isset($data['ADDRESS']) ? $data['ADDRESS'] : '',
				'master_vendor_shipping_address' => isset($data['ADDRESS']) ? $data['ADDRESS'] : '',
				'master_vendor_cdate' => date('Y-m-d H:i:s'),
				'master_vendor_udate' => date('Y-m-d H:i:s')
				);
			//echo '<pre>';print_r($item);die;
			$this->db->insert('tbl_master_vendor',$item);
			$lid = $this->db->insert_id();
			...
			$item = array(
				...
				//echo '<pre>';print_r($item);die;
			$this->db->insert('tbl_master_vendor_contactperson',$item);
			$lid = $this->db->insert_id();
		}else if($c_data['master_vendor_id'] != '')
		{
			//echo '<pre>';print_r($c_data);die;
			$item = array(
						'contact_master_id' => $c_data['master_vendor_id'],
						'contact_pname' => isset($data['CONTACT PERSON']) ? $data['CONTACT PERSON'] : '',
						'contact_mobile'=> isset($data['MOBILE NUMBER']) ? $data['MOBILE NUMBER'] : '',
						'contact_email'=> isset($data['EMAIL']) ? $data['EMAIL'] : '',
						'contact_cdate' => date('Y-m-d H:i:s'),
	...
	public function get($id)
	{
		$this->db->select('*');
		$this->db->from('tbl_master_vendor');
		//$this->db->join('tbl_master_vendor','tbl_sale_quotation.vendor = tbl_master_vendor.master_vendor_id');
		$this->db->where('master_vendor_id',$id);
		$query = $this->db->get();
		return $query->result_array();
	}
	public function get_master_vendor_report()
	{
		$this->db->select('*');
		$this->db->from('tbl_master_vendor');
		$this->db->where('master_vendor_isdelete',0);
		$this->db->order_by('master_vendor_id','DESC');
		if($this->input->post('master_vendor_code') && ($this->input->post('master_vendor_code') != ''))
        {
           $this->db->like('master_vendor_code', $this->input->post('master_vendor_code'));   
        }
        if($this->input->post('master_vendor_com_name') && ($this->input->post('master_vendor_com_name') != ''))
        {
           $this->db->like('master_vendor_com_name', $this->input->post('master_vendor_com_name'));   
        }
         if($this->input->post('master_vendor_office_address') && ($this->input->post('master_vendor_office_address') != ''))
        {
           $this->db->like('master_vendor_office_address', $this->input->post('master_vendor_office_address'));   
        }
         if($this->input->post('master_vendor_email_address') && ($this->input->post('master_vendor_email_address') != ''))
        {
           $this->db->like('master_vendor_email_address', $this->input->post('master_vendor_email_address'));   
        }
         if($this->input->post('master_vendor_office_no') && ($this->input->post('master_vendor_office_no') != ''))
        {
           $this->db->like('master_vendor_office_no', $this->input->post('master_vendor_office_no'));   
        }
         if($this->input->post('master_vendor_mobile_no') && ($this->input->post('master_vendor_mobile_no') != ''))
        {
           $this->db->like('master_vendor_mobile_no', $this->input->post('master_vendor_mobile_no'));   
        }
		$query = $this->db->get();
	...
	public function get_master_vendorno_wise()
	{
		$this->db->select('*');
		$this->db->from('tbl_sale_quotation');
		//$this->db->join('tbl_master_vendor','tbl_sale_quotation.vendor = tbl_master_vendor.master_vendor_id');
	...
	public function get_ajaxpartyAdd($id)
	{
		$this->db->select('*');
		$this->db->from('tbl_master_vendor');
		//$this->db->where('master_vendor_cid',$this->session->userdata['login']['aus_Id']);
		$this->db->where('master_vendor_id',$id);
		$query = $this->db->get();
		return $query->row_array();
	}
	...
	{
		$this->db->select('*');
		$this->db->from('tbl_master_vendor_contactperson');
		$this->db->join('tbl_master_vendor','tbl_master_vendor.master_vendor_id = tbl_master_vendor_contactperson.contact_master_id');
	...
	public function master_vendor_report()
	{
		$this->db->select('*');
		$this->db->from('tbl_sale_quotation');
		//$this->db->join('tbl_master_vendor', 'tbl_master_vendor.master_vendor_id = tbl_sale_quotation.vendor');
    ...
    public function get_inqno_wise()
    {
    	$this->db->select('*');
		$this->db->from('tbl_sale_quotation');
		$this->db->join('tbl_master_vendor', 'tbl_master_vendor.master_vendor_id = tbl_sale_quotation.vendor','left');
		$this->db->order_by('tbl_sale_quotation.sa_id','DESC');
    	$query = $this->db->get();
		return $query->result_array();
    }
    ...
    {
		$this->db->set('master_vendor_isdelete',1);
		$this->db->where('master_vendor_id', $id);
		$this->db->update('tbl_master_vendor'); 
		$log = array(
					'adlog_name' => $this->session->userdata['miconlogin']['email'],
					'adlog_datetime' => date('Y-m-d H:i:s'),
					'adlog_ip' =>$_SERVER['REMOTE_ADDR'],
					'adlog_module' => 'Master Vendor',
					'adlog_delete' => 1
				);
			$this->db->insert('tbl_adminlogs',$log);
			return $id;
	...
	public function get_cust($id)
	{
		...
		$this->db->select('*');
		$this->db->from('tbl_master_vendor');
		$this->db->where('master_vendor_name',$id);
		$query = $this->db->get();
		return $query->row_array();
	}
	...
	{
		$sql = "SELECT * from tbl_master_vendor WHERE UPPER(master_vendor_name) LIKE '%".$this->db->escape_like_str(strtoupper($q))."%'";
		...
		if($query->num_rows > 0)
		{
			...
			{
				$new_row['label']=htmlentities(stripslashes($row['master_vendor_name']));
				$new_row['value']=htmlentities(stripslashes($row['master_vendor_name']));
	...
	public function delete_itms($id,$sid)
    {
		...
		$this->db->update('tbl_master_vendor_contactperson'); 

		$log = array(
					'adlog_name' => $this->session->userdata['miconlogin']['email'],
					'adlog_datetime' => date('Y-m-d H:i:s'),
					'adlog_ip' =>$_SERVER['REMOTE_ADDR'],
					'adlog_module' => 'Master Vendor Delete',
					'adlog_delete' => 1
				);
			$this->db->insert('tbl_adminlogs',$log);
		return $id;
	...
	public function get_customertype()
	{
		...
		//$this->db->join('tbl_master_vendor','tbl_sales_enq.vendor = tbl_master_vendor.master_vendor_id');
	...
	public function item_edit($data,$sqiitemid)
	{
		...
		$item = array(
				...
				$this->db->update('tbl_master_vendor_contactperson',$item);
		...
		$log = array(
					...
					'adlog_datetime' => date('Y-m-d H:i:s'),
					'adlog_ip' =>$_SERVER['REMOTE_ADDR'],
					'adlog_module' => 'Master Vendor Other Edit',
					'adlog_add' => 1,
					'adlog_userdetails' => $_SERVER['HTTP_USER_AGENT']
				);
			$this->db->insert('tbl_adminlogs',$log);
