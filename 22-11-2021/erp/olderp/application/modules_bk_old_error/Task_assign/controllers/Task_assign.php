...
class Task_assign extends CI_Controller {
	...
	{
		$this->data['total'] = $this->task_assign_model->count();
	...
	public function ajax()
	{
		...
		$idenc = $this->encrypt_decrypt('encrypt',$user[$i]['task_id']);//$this->encrypt->encode($vals['edu_is_delete']);
		...
		if(isset($user[$i]['task_date']) && $user[$i]['task_date'] == '1970-01-01')
		{
			$user[$i]['task_date'] = '';
		}else{
			$user[$i]['task_date'] = date('d-m-Y', strtotime($user[$i]['task_date']));
		}
		if(isset($user[$i]['task_when']) && $user[$i]['task_when'] == '1970-01-01')
		{
			$user[$i]['task_when'] = '';
		}else{
			$user[$i]['task_when'] = date('d-m-Y', strtotime($user[$i]['task_when']));
		}
		if(isset($user[$i]['task_comp_dt']) && $user[$i]['task_comp_dt'] == '1970-01-01')
		{
			$user[$i]['task_comp_dt'] = '';
		}else{
			$user[$i]['task_comp_dt'] = date('d-m-Y', strtotime($user[$i]['task_comp_dt']));
		}
		if(isset($user[$i]['task_cdate']) && ($user[$i]['task_cdate'] == '1970-01-01' || $user[$i]['task_cdate'] == '0000-00-00'))
		{
			$user[$i]['task_cdate'] = '';
		}else{
			$user[$i]['task_cdate'] = date('d-m-Y', strtotime($user[$i]['task_cdate']));
		}
		if(isset($user[$i]['task_udate']) && ($user[$i]['task_udate'] == '1970-01-01' || $user[$i]['task_udate'] == '0000-00-00'))
		{
			$user[$i]['task_udate'] = '';
		}else{
			$user[$i]['task_udate'] = date('d-m-Y', strtotime($user[$i]['task_udate']));
		}
		if(isset($user[$i]['task_priority']) && $user[$i]['task_priority'] == 0)
		{
			$user[$i]['task_priority'] = '';
		}
		if($user[$i]['status'] == 1)
		{
			...
			$edit = '<a href="'.base_url().'Task_assign/edit/'.$idenc.'" class="btn btn-sm btn-outline green"><i class="fa fa-search"></i> Edit</a>'; 		
		}
		$delete = '<a href="'.base_url().'Task_assign/delete/'.$idenc.'"onclick="return confirm('."'Are you sure you want to Delete this record?'".')" class="btn btn-sm btn-outline red"><i class=""></i> Delete</a>';
		$records["data"][] = array(
