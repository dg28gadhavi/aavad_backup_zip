...
class Store_aprove extends CI_Controller {
	...
	public function __construct()
	{
		...
		$this->load->model('Store_aprove_model');
	...
	public function index()
	{
		...
		{
			$this->session->set_flashdata('rights_error', "You Don't have rights to access Store_aprove VIew functionality");
			redirect(base_url());
		}
		//$this->data['main_content'] = 'Store_aprove_grid_view';
		$this->load->view('includes/template',$this->data);
	}

	public function ajax()
	{
		$user = $this->Store_aprove_model->get_Store_aprove();
		...
		if($right_status == false)
		{
			...
			$editstr = '<a href="'.base_url().'Store_aprove/edit/'.$idenc.'" class="btn btn-sm btn-outline green"><i class="fa fa-pencil"></i></a>';
		}
		...
		if($right_status == false)
		{
			...
			$deletestr = '<a href="'.base_url().'Store_aprove/delete/'.$idenc.'"onclick="return confirm('."'Are you sure you want to Delete this record?'".')" class="btn btn-sm btn-outline red"><i class="fa fa-times"></i></a>';
		}
		if($right_status == false)
		{
			$viewpdfstr = '';
	...
	public function Store_aprove_report()
	{
		...
		$this->data['admins'] = $this->Store_aprove_model->get_admin();
		//$this->data['custometyps'] = $this->Store_aprove_model->get_customertype();
		$this->data['executive'] = $this->Store_aprove_model->get_Executive();
		$this->data['main_content'] = 'Store_aprove_report';
		$this->load->view('includes/template',$this->data);
	}

	public function pdf_workorder()
	{
		//$alldata = $this->Store_aprove_model->wo_pdfdata();
	...
	{
		$this->Store_aprove_model->wo_confirm();
		$this->session->set_flashdata('success', 'confirm successfully.');
					redirect(base_url('Store_aprove/other_details/'.$this->uri->segment(3)), 'refresh');
	}
	public function ajax_salesinq_report()
	{
		$user = $this->Store_aprove_model->wo_report();
		$iTotalRecords = count($user);
		$iDisplayLength = intval($_REQUEST['length']);
		$iDisplayLength = $iDisplayLength < 0 ? $iTotalRecords : $iDisplayLength; 
		$iDisplayStart = intval($_REQUEST['start']);
		...
		$status = $status_list[rand(0, 2)];
		$id = ($i + 1);
		$idenc = $this->encrypt_decrypt('encrypt',$user[$i]['woi_id']);
		//$this->encrypt->encode($user[$i]['sq_id']);
		//$crud->columns('master_item_code','master_item_name','master_item_description','master_item_make','master_item_rating','master_item_part_no','master_item_price','master_item_stock','master_item_created_date','master_item_updated_date');
		$right_status = $this->check_rights('edit');
		if($right_status == false)
		{
			...
			$editstr = '<a title="Edit" href="'.base_url().'Store_aprove/other_details/'.$idenc.'" class="btn btn-sm btn-outline green"><i class="fa fa-pencil"></i></a>';
		}
		$right_status = $this->check_rights('delete');
		if($right_status == false)
		{
			...
			{
				$deletestr = '<a title="delete" href="'.base_url().'Store_aprove/delete/'.$idenc.'"onclick="return confirm('."'Are you sure you want to Delete this record?'".')" class="btn btn-sm btn-outline red"><i class="fa fa-times"></i></a>';
			}	
			else{
				$deletestr = '';
			}
		...
		if($right_status == false)
		{
			...
			$quotestr = '<a title="Create Quotation" href="'.base_url().'Store_aprove/create_quote/'.$idenc.'"onclick="return confirm('."'Are you sure you want to Create Quotation?'".')" class="btn btn-sm btn-outline red"><i class="fa fa-plus-circle"></i></a>';
		}
		$right_status = $this->check_rights('view');
		if($right_status == false)
		{
		...
		$user[$i]['woi_itm_desc'] = $user[$i]['woi_part_no'].' '. $user[$i]['woi_itm_title'];
		$records["data"][] = array(
			$id,
				''.$user[$i]['wo_wo_no'],
				''.$user[$i]['wo_wo_date'],
	...
	public function add()
	{
		...
		if($right_status == false)
		{
			$this->session->set_flashdata('rights_error', "You Don't have rights to access Store_aprove Add functionality");
			redirect(base_url());
		}
		...
		if($success == TRUE)
		{
			...
			{	
				...
				$lid = $this->Store_aprove_model->add($value);
				$this->session->set_userdata('tabno',1);
				//***********PDF File Code Start********************
				$pdfdata = $this->Store_aprove_model->get_pdfdata($lid,'Store_aprove');
				//echo '<pre>';print_r($pdfdata);die;
				$html = $this->load->view('Store_aprove/Store_aprove_pdf_view',$pdfdata,TRUE);
				...
				if($lid)
				{
					//die;
					$this->session->set_userdata('tabno',1);
					$this->session->set_flashdata('success', 'Store_aprove added successfully.');
					redirect(base_url('Store_aprove/other_details/'.$this->encrypt_decrypt('encrypt',$lid)), 'refresh');
				}else{
					$this->session->set_flashdata('error', 'Store_aprove not added successfully!!');
					redirect(base_url('Store_aprove/add'), 'refresh');
				}
			 	redirect(base_url('Store_aprove'), 'refresh');
			}
		}
		if($success == FALSE)
		{
	...
	public function other_add()
	{
		...
		if($right_status == false)
		{
			...
			{	//echo '<pre>';print_r($this->input->post(NULL,FALSE));die;
				...
				$lid = $this->Store_aprove_model->other_add($value);
				//***********PDF File Code Start********************
				$pdfdata = $this->Store_aprove_model->get_pdfdata($lid,'Store_aprove');
				//echo '<pre>';print_r($pdfdata);die;
				$html = $this->load->view('Store_aprove/Store_aprove_pdf_view',$pdfdata,TRUE);
	...
	public function item_add()
	{
			...
			if($right_status == false)
			{
				...
				$this->session->set_userdata('tabno',2);
					...
					$result = $this->Store_aprove_model->item_img($value);
					//if(count($result) > 0)
					//{
				...
				$lid = $this->Store_aprove_model->item_add($value);
	...
	public function item_edit()
	{
		ini_set('memory_limit', '-1');
			...
			if($right_status == false)
			{
				$this->session->set_flashdata('rights_error', "You Don't have rights to access Store_aprove Edit functionality");
				redirect(base_url());
			}
				$value = array();
				$value = $this->input->post(NULL,FALSE);
				...
				$lid = $this->Store_aprove_model->item_edit($value,$sqiitemid);
				...
				{
					$this->session->set_flashdata('success', 'Details of item added successfully.');
					redirect(base_url('Store_aprove/other_details/'.$this->uri->segment(3).'?acttype=edit&itemid='.$sqiitemid), 'refresh');
				}else{
					$this->session->set_flashdata('error', 'Details of item not added successfully!!');
					redirect(base_url('Store_aprove/add'), 'refresh');
				}
	...
	public function folup_add()
	{
		...
		if($right_status == false)
		{
			...
			{	//echo '<pre>';print_r($this->input->post(NULL,FALSE));die;
				...
				$value['sq_udate'] = date('Y-m-d H:i:s');
				$this->session->set_userdata('tabno',3);
				$lid = $this->Store_aprove_model->folup_add($value);
				//***********PDF File Code Start********************
				$pdfdata = $this->Store_aprove_model->get_pdfdata($lid,'Store_aprove');
				//echo '<pre>';print_r($pdfdata);die;
				$html = $this->load->view('Store_aprove/Store_aprove_pdf_view',$pdfdata,TRUE);
	...
	public function other_details()
	{
		...
		//$this->data['autosearch_items'] = $this->Store_aprove_model->get_customertype();
		// $this->data['custometyps'] = $this->Store_aprove_model->get_customertype();
		 $this->data['list'] = $this->Store_aprove_model->get($idencr);
		// //echo '<pre>';print_r($this->data['list']);die;
		 $this->data['items'] = $this->Store_aprove_model->get_items($idencr);
		...
		if($this->input->get('acttype') && $this->input->get('itemid') && ($this->input->get('acttype') == 'edit'))
		{
			...
			$this->data['edit_items'] = $this->Store_aprove_model->get_edit_inqitems($idencr,$eitemid);
			//echo '<pre>';print_r($this->data['edit_items']);die;
			$this->data['action_itm'] = "Store_aprove/item_edit/".$this->uri->segment(3).'?acttype=edit&itemid='.$eitemid;
		}else{
			$this->data['action_itm'] = "Store_aprove/item_add/".$this->uri->segment(3);
		}
		//echo "<pre>"; print_r($this->data['items']); die;
		$this->data['confirm_or_not'] = $this->Store_aprove_model->get_confirm_or_not($idencr);
		$this->data['custometyps'] = $this->Store_aprove_model->get_customertype();
		$this->data['countries'] = $this->Store_aprove_model->get_country();
		$this->data['wo_no'] = $this->Store_aprove_model->wo_no_get();
		$this->data['vendors'] = $this->Store_aprove_model->get_masterparty();
		$this->data['admins'] = $this->Store_aprove_model->get_admin();
		$this->data['main_content'] = 'Sales_basic_details_tab';
		$this->data['action_bd'] = "Store_aprove/add/".$this->uri->segment(3);
		$this->data['action_othr'] = "Store_aprove/other_add/".$this->uri->segment(3);
		//$this->data['action_itm'] = "Store_aprove/item_add/".$this->uri->segment(3);
		$this->data['action_fup'] = "Store_aprove/folup_add/".$this->uri->segment(3);
		$this->load->view('includes/template',$this->data);
	}
	...
	{
		$this->data['custometyps'] = $this->Store_aprove_model->get_customertype();
		$this->data['cities'] = $this->Store_aprove_model->get_city();
		$this->data['countries'] = $this->Store_aprove_model->get_country();
		$this->data['wo_no'] = $this->Store_aprove_model->wo_no_get();
		$this->data['vendors'] = $this->Store_aprove_model->get_masterparty();
		$this->data['admins'] = $this->Store_aprove_model->get_admin();
		$this->data['main_content'] = 'Store_aprove_form_view';
		$this->data['action'] = "Store_aprove/add";
		$this->load->view('includes/template',$this->data);
	}
	...
	public function admin_aprove()
	{
		...
		{
			$this->Store_aprove_model->admin_qty_aprove($value);
			$this->session->set_userdata('tabno',3);
	...
	public function manager_aprove()
	{
		...
		$value=$this->input->post();
			...
			if($value['staff_ori_qty'] >= $value['staff_qty'])
			{
				$response=$this->Store_aprove_model->manager_qty_aprove($value);
	...
	public function production_aprove()
	{
		...
		if($value['staff_ori_qty'] >= $value['staff_qty'])
		{
			$response=$this->Store_aprove_model->production_qty_aprove($value);
			if($response['status'] == true)
			{
				$this->session->set_flashdata('success', 'successfully.');
			   redirect(base_url('Store_aprove/other_details/'.$this->uri->segment(3)), 'refresh');
	...
	public function store_aprove()
	{
		...
		if($value['staff_ori_qty'] >= $value['staff_qty'])
		{
			$this->Store_aprove_model->store_qty_aprove($value);
			$this->session->set_flashdata('success', 'successfully.');
			redirect(base_url('Store_aprove/other_details/'.$this->uri->segment(3)), 'refresh');
		}
		else{
	...
	public function account_aprove()
	{
		...
		if($value['staff_ori_qty'] >= $value['staff_qty'])
		{
			$this->Store_aprove_model->account_qty_aprove($value);
			$this->session->set_flashdata('success', 'successfully.');
			redirect(base_url('Store_aprove/other_details/'.$this->uri->segment(3)), 'refresh');
		}
		else{
	...
	public function dispatch_aprove()
	{
		...
		if($value['staff_ori_qty'] >= $value['staff_qty'])
		{
			$this->Store_aprove_model->dispatch_qty_aprove($value);
			$this->session->set_flashdata('success', 'successfully.');
			redirect(base_url('Store_aprove/other_details/'.$this->uri->segment(3)), 'refresh');
		}
		else{
		...
		if($enid && ($enid != ''))
		{
			...
			if(isset($id) && $id!= ''){
				...
				if(!empty($this->data['list'])){
					$lid = $this->Store_aprove_model->delete($idencr);
						if ($lid) {
						$this->session->set_flashdata('success', 'Store_aprove deleted successfully.');
						redirect('Store_aprove/Store_aprove_report', 'refresh');
						} else {
							$this->session->set_flashdata('error', 'Store_aprove not deleted successfully!!.');
							redirect('Store_aprove/Store_aprove_report', 'refresh'); 
						}						
				}else{
					$this->session->set_flashdata('error', 'Store_aprove not Available!!');
			  		redirect('Store_aprove/Store_aprove_report', 'refresh'); 
			  	}
			}
			else{
			...
			redirect(base_url('Store_aprove/Store_aprove_report'), 'refresh'); 
		}
	}
	...
	public function delete_all()
	{
		...
		if($right_status == false)
		{
			$this->session->set_flashdata('rights_error', "You Don't have rights to access Store_aprove Delete functionality");
			redirect(base_url());
		}
			...
			foreach($this->input->get('delid') as $enid)
			{
				if($enid && ($enid != ''))
				{
					...
					if(isset($idencr) && $idencr != ''){
						$this->data['list'] = $this->Store_aprove_model->get($idencr);//die;
						if(!empty($this->data['list'])){
							$lid = $this->Store_aprove_model->delete($idencr);
								if ($lid) {
								$this->session->set_flashdata('success', 'Store_aprove inquiry deleted successfully.');
								} else {
									$this->session->set_flashdata('error', 'Store_aprove inquiry not deleted successfully!!.');
								}						
						}else{
							$this->session->set_flashdata('error', 'Store_aprove inquiry not Available!!');
						}
					}
					else{
							$this->session->set_flashdata('error', 'Store_aprove inquiry not Available!!');
  	...
  	{		
		if($this->input->post() && isset($_FILES['userfile']['name']) && ($_FILES['userfile']['name'] != ''))
		{
			...
			if (!$this->upload->do_upload()) {
				...
				$this->data['action'] = "Store_aprove/importcsv";
				...
				if ($this->csvimport->get_array($file_path)) {
					...
					if(is_array($csv_array) && !empty($csv_array))
					{
						foreach ($csv_array as $row)
						{
							...
							if(isset($row['Store_aprove']) && ($row['Latitude'] != '') && (isset($row['Longitude'])) && (isset($row['Store_aproveCode'])))
							{
								$this->Store_aprove_model->importcsv($row);
							}
						}
	...
	public function setbit_Store_aprove()
	{
		$this->Store_aprove_model->setbit_Store_aprove();
	}
	...
	public function getsub_category()
	{
		...
		$this->data['sub_catlists'] = $this->Store_aprove_model->get_subsource($this->abc);
	...
	public function item_description()
	{
		if($this->input->post() && $this->input->post('master_item_id'))
		{
			...
			$array = $this->Store_aprove_model->get_item_description($id);
	...
	public function get_customer_information()
	{
		...
		{
			$this->Store_aprove_model->get_customer_information($value['term']);
		}
	...
	public function get_hc()
	{
		...
		if(isset($id) && ($id != ''))
		{
			$value = $this->Store_aprove_model->get_hcs($id);
			echo json_encode($value);
		}
	}

	public function delete_Store_aprove_item($id=false)
	{
		$right_status = $this->check_rights('delete');
		if($right_status == false)
		{
			$this->session->set_flashdata('rights_error', "You Don't have rights to access Store_aprove inquiry Delete functionality");
			redirect(base_url());
		}
		$enid = $this->uri->segment(3) ? $this->uri->segment(3) : '';
		...
		if($enid && ($enid != ''))
		{
			...
			if(isset($id) && $id!= ''){
				$this->session->set_userdata('tabno',2);
					$lid = $this->Store_aprove_model->delete_Store_aprove_item($idencr,$sa_id);
					//***********PDF File Code Start********************
					$autoid = $this->uri->segment(3) ? $this->encrypt_decrypt('decrypt', $this->uri->segment(4)) : '';
					//***********PDF File Code Start********************
				$pdfdata = $this->Store_aprove_model->get_pdfdata($sa_id,'Store_aprove');
				//echo '<pre>';print_r($pdfdata);die;
				$html = $this->load->view('Store_aprove/Store_aprove_pdf_view',$pdfdata,TRUE);
				//$html=$this->data['result_view'];
				$header='';
				...
				//***********PDF File Code End********************
						if ($sa_id) {
						$this->session->set_flashdata('success', 'Store_aprove inquiry deleted successfully.');
						redirect('Store_aprove/other_details/'.$this->uri->segment(4), 'refresh');
						} else {
							$this->session->set_flashdata('error', 'Store_aprove inquiry not deleted successfully!!.');
							redirect('Store_aprove/other_details/'.$this->uri->segment(4), 'refresh'); 
						}						
			...
			$this->session->set_flashdata('error', 'Something went wrong');
			redirect(base_url('Store_aprove/other_details/'.$this->uri->segment(4)), 'refresh'); 
		}
	}
	...
	public function status_act($id=false)
	{
		...
		if($enid && ($enid != ''))
		{
			...
			if(isset($id) && $id!= ''){
				$this->session->set_userdata('tabno',3);
					$lid = $this->Store_aprove_model->status_act($idencr);
						if ($lid) {
	...
	public function status_deact($id=false)
	{
		...
		if($enid && ($enid != ''))
		{
			...
			if(isset($id) && $id!= ''){
				$this->session->set_userdata('tabno',3);
					$lid = $this->Store_aprove_model->status_deact($idencr);
						if ($lid) {
						$this->session->set_flashdata('success', 'Followup status changed successfully.');
						redirect('inq-followup', 'refresh');
						} else {
	...
	public function delete_fup($id=false)
	{
		...
		if($enid && ($enid != ''))
		{
			...
			if(isset($id) && $id!= ''){
				$this->session->set_userdata('tabno',3);
					$lid = $this->Store_aprove_model->delete_fup($idencr);
					//***********PDF File Code Start********************
					$autoid = $this->uri->segment(3) ? $this->encrypt_decrypt('decrypt', $this->uri->segment(4)) : '';
					$pdfdata = $this->Store_aprove_model->get_pdfdata($autoid,'Store_aprove');
					//echo '<pre>';print_r($pdfdata);die;
					$html = $this->load->view('Store_aprove/Store_aprove_pdf_view',$pdfdata,TRUE);
					//$html=$this->data['result_view'];
					$header='';
	...
	public function get_item_details()
	{
		...
		if(isset($value['term']) && !empty($value['term']))
		{
			$this->Store_aprove_model->get_item_details($value['term']);
		}
	}
	...
	public function create_quote()
	{
		...
		$lid = $this->Store_aprove_model->create_qoute($id);
...
ini_set('memory_limit','3222222222222222222222222222222222222222M');
        ...
        $finalar = $this->Store_aprove_model->get_excel_certificate();
...
//echo '<pre>';print_r($finalar);die;
	...
	public function get_country_to_state()
	{
		...
		{
			$statelists = $this->Store_aprove_model->get_country_to_state($countyid);
		}else{
			$statelists = array();
		}
	...
	public function get_state_to_city()
	{
		...
		{
			$citylists = $this->Store_aprove_model->get_state_to_city($stateID);
		}else{
			$citylists = array();
		}
