...
class Production_manager extends CI_Controller {
	...
	public function __construct()
	{
		...
		$this->load->model('Production_manager_model');
	...
	public function index()
	{
		...
		{
			$this->session->set_flashdata('rights_error', "You Don't have rights to access Production_manager VIew functionality");
			redirect(base_url());
		}
		$this->data['main_content'] = 'Production_manager_grid_view';
		$this->load->view('includes/template',$this->data);
	}

	public function ajax()
	{
		$user = $this->Production_manager_model->Production_manager_report();
		//echo '<pre>';print_r($user['woi_id']);die;
		...
		$admin_users=$this->Production_manager_model->get_admin_users();
		$admin_select='';
		...
		if($right_status == false)
		{
			...
			// $editstr = '<a href="'.base_url().'Production_manager/store_approve/'.$user[$i]['woi_id'].'" class="btn btn-sm btn-outline green"><i class="fa fa-check-square"></i></a>';
		...
		$records["data"][] = array(
			  '<input type="checkbox" name="id[]" value="'.$user[$i]['master_item_id'].'">',
				...
				''.$user[$i]['woi_manager_qty'],
			  '<form action="'.base_url().'Production_manager/assign_production/?woid='.$user[$i]['woi_id'].'" method="post">'.$admin_select.''.$editstr.'</form>',
		);
		}
	...
	public function Production_manager_report()
	{
		...
		$this->data['admins'] = $this->Production_manager_model->get_admin();
		//$this->data['custometyps'] = $this->Production_manager_model->get_customertype();
		//$this->data['Production_manager'] = $this->Production_manager_model->get_Production_manager();
		$this->data['main_content'] = 'Production_manager_report';
		$this->load->view('includes/template',$this->data);
	}
	...
	public function assign_production()
	{
		...
		$this->Production_manager_model->assign_production($value,$woid);
		redirect(base_url('Production_manager'), 'refresh');
	}
	...
	public function store_approve()
	{
		...
		$this->Production_manager_model->store_approve($otwiid);
		redirect(base_url('Production_manager'), 'refresh');
	...
	{
		$this->data['custometyps'] = $this->Production_manager_model->get_customertype();
		$this->data['cities'] = $this->Production_manager_model->get_city();
		$this->data['countries'] = $this->Production_manager_model->get_country();
		$this->data['wo_no'] = $this->Production_manager_model->wo_no_get();
		$this->data['vendors'] = $this->Production_manager_model->get_masterparty();
		$this->data['admins'] = $this->Production_manager_model->get_admin();
		$this->data['main_content'] = 'Production_manager_form_view';
		$this->data['action'] = "Production_manager/add";
		$this->load->view('includes/template',$this->data);
	}

	public function Production_managertopo()
	{
		echo "<pre>"; print_r($this->input->get()); die;
		$lid = $this->indend_model->Production_managertopo();
		if(isset($lid))
		{
	...
	public function delete($id=false)
	{
		...
		if($enid && ($enid != ''))
		{
			$idencr = $this->uri->segment(3) ? $this->encrypt_decrypt('decrypt', $enid) : '';
			if(isset($id) && $id!= ''){
				$this->data['list'] = $this->Production_manager_model->get($idencr);
				if(!empty($this->data['list'])){
					$lid = $this->Production_manager_model->delete($idencr);
						if ($lid) {
						$this->session->set_flashdata('success', 'Production_manager deleted successfully.');
						redirect('Production_manager/Production_manager_report', 'refresh');
						} else {
							$this->session->set_flashdata('error', 'Production_manager not deleted successfully!!.');
							redirect('Production_manager/Production_manager_report', 'refresh'); 
						}						
				}else{
					$this->session->set_flashdata('error', 'Production_manager not Available!!');
			  		redirect('Production_manager/Production_manager_report', 'refresh'); 
			  	}
			}
			else{
			...
			redirect(base_url('Production_manager/Production_manager_report'), 'refresh'); 
		}
	}
	...
	public function delete_all()
	{
		...
		if($right_status == false)
		{
			$this->session->set_flashdata('rights_error', "You Don't have rights to access Production_manager Delete functionality");
			redirect(base_url());
		}
			...
			foreach($this->input->get('delid') as $enid)
			{
				if($enid && ($enid != ''))
				{
					...
					if(isset($idencr) && $idencr != ''){
						$this->data['list'] = $this->Production_manager_model->get($idencr);//die;
						if(!empty($this->data['list'])){
							$lid = $this->Production_manager_model->delete($idencr);
								if ($lid) {
								$this->session->set_flashdata('success', 'Production_manager inquiry deleted successfully.');
								} else {
									$this->session->set_flashdata('error', 'Production_manager inquiry not deleted successfully!!.');
								}						
						}else{
							$this->session->set_flashdata('error', 'Production_manager inquiry not Available!!');
						}
					}
					else{
							$this->session->set_flashdata('error', 'Production_manager inquiry not Available!!');
	...
	public function setbit_Production_manager()
	{
		$this->Production_manager_model->setbit_Production_manager();
	}
	...
	public function getsub_category()
	{
		...
		$this->data['sub_catlists'] = $this->Production_manager_model->get_subsource($this->abc);
	...
	public function item_description()
	{
		if($this->input->post() && $this->input->post('master_item_id'))
		{
			...
			$array = $this->Production_manager_model->get_item_description($id);
	...
	public function get_customer_information()
	{
		...
		{
			$this->Production_manager_model->get_customer_information($value['term']);
		}
	...
	public function get_hc()
	{
		...
		if(isset($id) && ($id != ''))
		{
			$value = $this->Production_manager_model->get_hcs($id);
			echo json_encode($value);
		}
	}

	public function delete_Production_manager_item($id=false)
	{
		$right_status = $this->check_rights('delete');
		if($right_status == false)
		{
			$this->session->set_flashdata('rights_error', "You Don't have rights to access Production_manager inquiry Delete functionality");
			redirect(base_url());
		}
		$enid = $this->uri->segment(3) ? $this->uri->segment(3) : '';
		...
		if($enid && ($enid != ''))
		{
			...
			if(isset($id) && $id!= ''){
				$this->session->set_userdata('tabno',2);
					$lid = $this->Production_manager_model->delete_Production_manager_item($idencr,$sa_id);
					//***********PDF File Code Start********************
					$autoid = $this->uri->segment(3) ? $this->encrypt_decrypt('decrypt', $this->uri->segment(4)) : '';
					// $pdfdata = $this->Production_manager_model->get_pdfdata($autoid,'Production_manager');
					// //echo '<pre>';print_r($pdfdata);die;
					// $html = $this->load->view('Production_manager/Production_manager_pdf_view',$pdfdata,TRUE);
					// //$html=$this->data['result_view'];
					// $header='';
					// $footer='';
					...
					//***********PDF File Code End********************
						if ($lid) {
						$this->session->set_flashdata('success', 'Production_manager inquiry deleted successfully.');
						redirect('Production_manager/other_details/'.$this->uri->segment(4), 'refresh');
						} else {
							$this->session->set_flashdata('error', 'Production_manager inquiry not deleted successfully!!.');
							redirect('Production_manager/other_details/'.$this->uri->segment(4), 'refresh'); 
						}						
			...
			$this->session->set_flashdata('error', 'Something went wrong');
			redirect(base_url('Production_manager/other_details/'.$this->uri->segment(4)), 'refresh'); 
		}
	}
		...
		if($enid && ($enid != ''))
		{
			...
			if(isset($id) && $id!= ''){
				$this->session->set_userdata('tabno',3);
					$lid = $this->Production_manager_model->status_act($idencr);
						if ($lid) {
	...
	public function status_deact($id=false)
	{
		...
		if($enid && ($enid != ''))
		{
			...
			if(isset($id) && $id!= ''){
				$this->session->set_userdata('tabno',3);
					$lid = $this->Production_manager_model->status_deact($idencr);
						if ($lid) {
						$this->session->set_flashdata('success', 'Followup status changed successfully.');
						redirect('inq-followup', 'refresh');
						} else {
	...
	public function delete_fup($id=false)
	{
		...
		if($enid && ($enid != ''))
		{
			...
			if(isset($id) && $id!= ''){
				$this->session->set_userdata('tabno',3);
					$lid = $this->Production_manager_model->delete_fup($idencr);
					//***********PDF File Code Start********************
					$autoid = $this->uri->segment(3) ? $this->encrypt_decrypt('decrypt', $this->uri->segment(4)) : '';
					$pdfdata = $this->Production_manager_model->get_pdfdata($autoid,'Production_manager');
					//echo '<pre>';print_r($pdfdata);die;
	...
	public function get_item_details()
	{
		...
		if(isset($value['term']) && !empty($value['term']))
		{
			$this->Production_manager_model->get_item_details($value['term']);
		}
	}
	...
	public function create_quote()
	{
		...
		$lid = $this->Production_manager_model->create_qoute($id);
...
ini_set('memory_limit','3222222222222222222222222222222222222222M');
        ...
        $finalar = $this->Production_manager_model->get_excel_certificate();
...
//echo '<pre>';print_r($finalar);die;
	...
	public function get_country_to_state()
	{
		...
		{
			$statelists = $this->Production_manager_model->get_country_to_state($countyid);
		}else{
			$statelists = array();
		}
	...
	public function get_state_to_city()
	{
		...
		{
			$citylists = $this->Production_manager_model->get_state_to_city($stateID);
		}else{
			$citylists = array();
		}
