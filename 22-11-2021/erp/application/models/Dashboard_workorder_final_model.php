...
class Dashboard_workorder_final_model extends CI_Model {
    ...
    {
        $this->db->select('SUM(otw_invftotal) as inv_total');
        $this->db->from('tbl_work_order');
        $this->db->join('tbl_outward','tbl_outward.otw_work_ord_id = tbl_work_order.wo_id');
        if($this->input->get('inq_start_date') && ($this->input->get('inq_start_date') != ''))
        {
        ...
        if($this->input->get('customer_search') && ($this->input->get('customer_search') != ''))
        {
            ...
            $this->db->like('LOWER(wo_customer_name)', strtolower($customer_search) );  
        }
        ...
        if($this->session->userdata('miconlogin') && is_array($this->session->userdata['miconlogin']) && !empty($this->session->userdata['miconlogin']) && isset($this->session->userdata['miconlogin']['typeid']) && ($this->encrypt_decrypt('decrypt',$this->session->userdata['miconlogin']['typeid']) != 3) &&  isset($this->session->userdata['miconlogin']['userid']) && ($this->session->userdata['miconlogin']['userid'] != '' && $this->encrypt_decrypt('decrypt',$this->session->userdata['miconlogin']['dep_id']) != 11  && $this->encrypt_decrypt('decrypt',$this->session->userdata['miconlogin']['dep_id']) != 2))
        {
    ...
    public function get_purchase_work_order()
    {
         $this->db->select('*');
        $this->db->from('tbl_work_order_item');
        $this->db->join('tbl_work_order','tbl_work_order.wo_id = tbl_work_order_item.woi_wo_id');
        $this->db->where('woi_admin_approve','1');
        $this->db->where('woi_purchase_approve','0');
    ...
    public function get_inv_no()
    {
        ...
        $autoid['inq_id'] = isset($autoid['inq_id']) ? $autoid['inq_id'] : 0;
    ...
    public function mf_approve($data)
    {
        ...
        $updatear = array('woi_mf_approve' => '1','woi_mf_approve_date' => date("Y-m-d H:i:s"));
        $this->db->where('woi_id',$data['wo_itemid']);
        $this->db->update('tbl_work_order_item',$updatear);
    ...
    {
        $value = array();
        $updatear = array('woi_store_approve' => '1','woi_store_approve_date' => date("Y-m-d H:i:s"),'woi_production_approve' => '2','woi_production_approve_date' => date("Y-m-d H:i:s"));
        $this->db->where('woi_id',$data['wo_itemid']);
        $this->db->update('tbl_work_order_item',$updatear);
        ...
        if($query->num_rows() == 0)
        {
                ...
                $autoid['otw_id'] = isset($autoid['otw_id']) ? $autoid['otw_id'] : 0;
    ...
    public function po_no_get()
    {
        ...
        $autoid['po_id'] = isset($autoid['po_id']) ? $autoid['po_id'] : 0;
        return 'PO-'.$year_string.'-'.($autoid['po_id']+1);
    }

    public function insert_purchase_data($data,$getdata)
    {
        //echo "<pre>";print_r($data);die;
        if($data['po_no'] == 0)
        {
            $this->db->select('*');
            $this->db->from('tbl_master_vendor');
            $this->db->where('master_vendor_id',$data['vendor']);
            $query = $this->db->get();
            $podata = $query->row_array();
            //echo '<pre>';print_r($podata);die;

            $this->db->select('*');
            $this->db->from('tbl_work_order');
            $this->db->where('wo_id',$getdata['wo_id']);
            $query = $this->db->get();
            $wodata = $query->row_array();
            //echo '<pre>';print_r($wodata);die;
            ...
            $item = array(
            'po_no' => $po_no_get,
            //'po_code' => $data['master_vendor_code'],
            'po_date' => isset($data['po_date']) ? date("Y-m-d", strtotime($data['po_date'])) : date("Y-m-d"),
            'vendor' => $podata['master_vendor_id'],
            'po_supplier' => $podata['master_vendor_com_name'],
            'po_contactno' => $podata['master_vendor_office_no'],
            'po_madeby' => $this->encrypt_decrypt('decrypt',$this->session->userdata['miconlogin']['userid']),
            'po_woid' => $getdata['wo_id'],
            'po_wono' => $wodata['wo_wo_no'],
            'po_completed' => '0',
            ...
            $this->db->join('tbl_work_order_item','tbl_work_order_item.woi_item_id = tbl_master_item.master_item_id','left');
            $this->db->where('master_item_id',$getdata['itemid']);
            $query = $this->db->get();
            $itmdata = $query->row_array();
            //echo "<pre>";print_r($itmdata);die;

            $item1 = array(
                'poi_po_id' => $lid,
                'poi_woitm_id' => $getdata['wo_itemid'],
                'poi_itm_name' => $itmdata['master_item_id'],
                'poi_itm_code' => $itmdata['master_item_code'],
                'poi_itm_title' => $itmdata['master_item_name'],
                'poi_itm_desc ' => $itmdata['master_item_description'],
                'poi_itm_part_no'=> isset($itmdata['master_item_part_no']) ? $itmdata['master_item_part_no'] : '',
                'poi_qty'=> $itmdata['woi_qty'],
        ...
        if($data['po_no'] != 0)
        {
            ...
            $this->db->select('*');
            $this->db->from('tbl_purchase_order');
            $this->db->where('po_id',$data['po_no']);
            $query = $this->db->get();
            $podata = $query->row_array();
            //echo "<pre>";print_r($podata);die;
    ...
    {
        //echo "<pre>";print_r($data);die;
        $updatear = array('woi_purchase_approve' => '1','woi_purchase_approve_date' => date("Y-m-d H:i:s"));
        $this->db->where('woi_id',$data['wo_itemid']);
        $this->db->update('tbl_work_order_item',$updatear);
    ...
    public function get_invoiceno($data)
    {
        ...
        if($data == 1)
        {
            ...
            //echo '<pre>';print_r($query->row_array());die;
            $autoid = $query->row_array();
            $autoid['otw_invno'] = isset($autoid['otw_invno']) ? $autoid['otw_invno'] : 0;
    ...
    public function get_challan_no()
    {
        ...
        //echo '<pre>';print_r($query->row_array());die;
        $autoid = $query->row_array();
        $autoid['otw_challan_no'] = isset($autoid['otw_challan_no']) ? $autoid['otw_challan_no'] : 0;
        //echo '<pre>';print_r($iparr);die;
    ...
    {
        //echo '<pre>';print_r($data);die;
        $updatear = array('otw_invno' => $data['inv_no'],'otw_completed' => '2','otw_invdate' => date("Y-m-d", strtotime($data['inv_date'])),'otw_challan_no' => $data['otw_challan_no'],'otw_invoicefile' => $data['invoice_file'],'otw_challan_date' => date("Y-m-d", strtotime($data['otw_challan_date'])),'otw_invftotal' => $data['inv_amount'],'otw_invftotal_withgst' => $data['inv_withgst_amount'],'otw_account_confirm_date' => date("Y-m-d H:i:s"));
        $this->db->where('otw_id',$data['otw_id']);
        $this->db->update('tbl_outward',$updatear);
       ...
       //echo '<pre>';print_r($woid['otw_work_ord_id']);die;
        ...
        $transaction = array(
                    'tran_payment_customer_id' => $otwdata['otw_customer_id'],
                    'tran_paymentitm_amt' => $data['inv_withgst_amount'],
                    'tran_paymentwo_id ' => $woid['otw_work_ord_id'],
                    'tran_paymentotw_id' => $data['otw_id'],
                    'tran_paymentcr_or_dr' => 2,
            ...
            $this->db->insert('tbl_payment',$transaction);
        ...
        if($otwdata['wo_demoreturn'] != 1 && $otwdata['otw_demoreturn'] != 1)
        {
            ...
            $item = array(
                ...
                'dis_invoice_file' => $data['invoice_file'],
    ...
    public function add_production_itemdetails($data,$postdata)
    {
        $update = array(
            'woi_productweight' => $postdata['woi_productweight'],
            'woi_item_remark_production' => $postdata['woi_item_remark_production'],
            'woi_product_drawing' => $postdata['woi_product_drawing']
            );
        $this->db->where('woi_id',$data['wo_itemid']);
...
*/
    ...
    public function get_demo_chaln_no()
    {
       ...
       // echo '<pre>';print_r($autoid['dis_challan_no']);die;
        ...
        $autoid['dis_challan_no'] = isset($autoid['dis_challan_no']) ? $autoid['dis_challan_no'] : 0;
    ...
    public function approve_production_bypass($data)
    {
        if(isset($data['wo_itemid']) && ($data['wo_itemid'] != '') && ($data['wo_itemid'] != 0))
        {
            $updatear = array('woi_production_bypass' => 1,'woi_production_bypasstime' => date("Y-m-d H:i:s"));
            $this->db->where('woi_id',$data['wo_itemid']);
            $this->db->update('tbl_work_order_item',$updatear);
    ...
    public function set_data_to_production()
    {
        die;
        // $this->db->select('*');
        // $this->db->from('tbl_work_order_item');
        ...
        // {
        // 	if(isset($value['woi_id']) && ($value['woi_id'] != '') && ($value['woi_id'] != 0))
	       // {
	       //     $updatear = array('woi_production_approve' => '2','woi_production_approve_date' => date("Y-m-d H:i:s"));
	       //     $this->db->where('woi_id',$value['woi_id']);
	       //     $this->db->update('tbl_work_order_item',$updatear);
	       // }
        // }
    }
