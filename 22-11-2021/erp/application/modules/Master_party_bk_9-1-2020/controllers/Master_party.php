...
class Master_party extends CI_Controller {
	...
	public function store_excel()
	{
        ...
        $fhead = array('Sr. No','COMPANY NAME','CONTACT PERSON','DEPARMANT','DESIGNATION','C/O AAVAD INSTRUMENT','PAST COMMENTS','DOCUMENTATION DONE','TYPE OF INDUSTRIES','E-MAIL ID','MOBILE NUMBER','PHONE NUMBER','LOCATION','ADDRESS','REMARKS');
        $i=0;
 		foreach ($finalar as $fkey => $fvalue) 
 		{ 	
			$i++;
 			if($finalar[$fkey]['master_party_id'])
 			{
 				$finalar[$fkey]['master_party_id'] = $i;
 			}
 		} 
