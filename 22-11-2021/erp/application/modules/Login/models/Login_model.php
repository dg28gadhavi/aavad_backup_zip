...
//echo "<pre>";print_r($this->session->userdata['miconlogin']);die;
			...
			$this->db->set('au_last_login',date('Y-m-d H:i:s'));
            $this->db->where('au_id',$result['data']['au_id']);
	...
	public function check_dbbackup()
	{
		$this->db->select('*');
		$this->db->from('tbl_dbbackup');
		$this->db->where('cdate',date('Y-m-d'));
		$query = $this->db->get();
		return $query->num_rows();
	}
	public function insert_dbbackup()
	{
		$this->db->where('id >',0);
		$this->db->delete('tbl_dbbackup');

		$db_data = array(
				'cdate' => date('Y-m-d')
				);
			$this->db->insert('tbl_dbbackup', $db_data);
	}
}
?>