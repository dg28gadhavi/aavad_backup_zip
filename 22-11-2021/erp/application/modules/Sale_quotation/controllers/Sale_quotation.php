...
class Sale_quotation extends CI_Controller {
	...
	public function ajax_salesqoute_report()
	{
		...
		$records["data"][] = array(
			  ...
			  // $id,
				...
				//''.$user[$i]['city_name'],
				''.$user[$i]['sa_address'],
				''.str_replace(",",", <br/>",$user[$i]['product_name_list']),
				''.$user[$i]['sa_grd_ttl'],
				''.$user[$i]['au_fname'],
				// ''.$user[$i]['sai_itm'],
				// ''.$user[$i]['sai_itm_qty'],
	...
	public function send_sms($id)
	{
		...
		$url = "http://ui.netsms.co.in/API/SendSMS.aspx?APIkey=pHzTOovNIyyqMUpoatJowtUyhI&SenderID=AAAVAD&SMSType=2&Mobile=".$pdfdata['inv']['sa_mobile']."&MsgText=".$msg."";
	...
	public function store_excel()
	{
        ...
        $fhead = array('Sr. No','Qoute. Date','Comapany name','client name','contact no','email id','city','Item Name','Item Desc','qty','product price','grand total','inq status','source','remark','Executive By');
        $i=0;
