...
class Dashboard_qoutation_model extends CI_Model {
	...
	function encrypt_decrypt($action, $string)
	{
	 ...
	 public function get_qua_folups()
    {
        ...
        $this->db->join('tbl_sale_quotation','tbl_sale_quotation.sa_id = tbl_sale_quotation_followup.fu_inq_id','left');
        ...
        $this->db->where('DATE_FORMAT(fu_followdate,"%Y-%m-%d") <=',$etdate);
        $this->db->where('DATE_FORMAT(fu_followdate,"%Y-%m-%d") >=',$stdate);
        $this->db->where('fu_isdelete',0);
		$this->db->where('fu_followupst',1);
        $query = $this->db->get();
	...
	public function get_inq_data()
    {
        ...
        if($this->session->userdata('miconlogin') && is_array($this->session->userdata['miconlogin']) && !empty($this->session->userdata['miconlogin']) && isset($this->session->userdata['miconlogin']['typeid']) && ($this->encrypt_decrypt('decrypt',$this->session->userdata['miconlogin']['typeid']) != 3) &&  isset($this->session->userdata['miconlogin']['userid']) && ($this->session->userdata['miconlogin']['userid'] != ''))
        {
            $this->db->where('sa_inq_by', $this->encrypt_decrypt('decrypt',$this->session->userdata['miconlogin']['userid']));
        }
        if($this->input->get('start_date') && $this->input->get('start_date') != "")
        {
            $start_date=$this->input->get('start_date');
...
/****************************************************************/
        ...
        if($this->session->userdata('miconlogin') && is_array($this->session->userdata['miconlogin']) && !empty($this->session->userdata['miconlogin']) && isset($this->session->userdata['miconlogin']['typeid']) && ($this->encrypt_decrypt('decrypt',$this->session->userdata['miconlogin']['typeid']) != 3) &&  isset($this->session->userdata['miconlogin']['userid']) && ($this->session->userdata['miconlogin']['userid'] != ''))
        {
            $this->db->where('tbl_sale_quotation.sa_inq_by', $this->encrypt_decrypt('decrypt',$this->session->userdata['miconlogin']['userid']));
        }
        if($this->input->get('start_date') && $this->input->get('start_date') != "")
        {
            $start_date=$this->input->get('start_date');
...
/****************************************************************/
       ...
       $this->db->where('sa_inq_by', 0);
        ...
        /***********************************************************/
        //echo "<pre>"; print_r($query->result_array()); die;
        $this->db->select('qs_id as qs_id,qs_name as status,count((sa_inq_st)) as count,SUM((sa_grd_ttl)) as total_amt');
        $this->db->from('tbl_sale_quotation');
       ...
       // echo "<pre>"; print_r($query->result_array()); die;
        ...
        $query = $this->db->get();
        $value['inq_bysts_zero'] = $query->row_array();
        ...
        foreach ($value['inq_bysts'] as $key => $inq_bysts) 
        {
            ...
            $this->db->join('tbl_quatation_status', 'tbl_quatation_status.qs_id = tbl_sale_quotation.sa_inq_st');
            $this->db->where('sa_isdeleted', '0');
            $this->db->where('sa_inq_by', $inq_bysts['status']);
            $this->db->where('sa_inq_st !=', 0);
            if($this->input->get('start_date') && $this->input->get('start_date') != "")
            {
                $start_date=$this->input->get('start_date');
