...
class Work_order_model extends CI_Model {
	...
	{   
		if($this->uri->segment(3))
		{
			...
			'wo_old_wo_no' => $data['wo_old_wo_no'],
			...
			'wo_charges' => json_encode($data['charge']),
		...
		$this->db->update('tbl_work_order',$item); 
	...
	public function other_add($data)
	{
		...
		$item = array(
			...
			'Work_order_sub' => $data['Work_order_sub'],
			'Work_order_desc' => $data['Work_order_desc'],
			'Work_order_tc_price' => $data['Work_order_tc_price'],
			'Work_order_tc_wrnty' => $data['Work_order_tc_wrnty'],
			'Work_order_tc_pf' => $data['Work_order_tc_pf'],
			'Work_order_tc_deli' => $data['Work_order_tc_deli'],
			'Work_order_tc_paynt' => $data['Work_order_tc_paynt'],
			'Work_order_tc_ovali' => $data['Work_order_tc_ovali'],
			'Work_order_tc_frght' => $data['Work_order_tc_frght'],
			'Work_order_tc_gst' => $data['Work_order_tc_gst'],
	...
	public function item_add($data)
	{
		...
		{
			$notimsg = 'New Item Add By '.$this->session->userdata['miconlogin']['fname'].' - '.$data['sqi_itm_pno'].' ';
			...
			if($type_id == 3)
			{
				$item1['woi_manager_approve'] = '1';
				$item1['woi_manager_approve_date'] = date("Y-m-d H:i:s");
				$notimsg .= 'Sales Manager Rights Automatically Approved';
				$assigndep = array('test','super_admin','sales_manager');
			}else{
				if($dep_id == 10)
				{
					$assigndep = array('test','sales_manager');
				}else if($dep_id == 11){
					$item1['woi_manager_approve'] = '1';
					$item1['woi_manager_approve_date'] = date("Y-m-d H:i:s");
					$item1['woi_admin_approve'] = '1';
					$item1['woi_admin_approve_date'] = date("Y-m-d H:i:s");
					$notimsg .= 'Sales Manager & Admin Rights Automatically Approved';
					$assigndep = array('test','super_admin','sales_manager','production_manager');
				}else if($dep_id == 1){
					$assigndep = array('test','sales_manager','sales');
				}else{
					echo "You Don't Have Rights to add item Now. Contact Administrator.";die;
				}
			}
		}
					//echo '<pre>'; print_r($item1); die;
				$this->db->insert('tbl_work_order_item',$item1);
	...
	public function item_edit($data,$sqiitemid)
	{
		...
		if($this->input->get('fromwofinal') && ($this->input->get('fromwofinal') == 1))
		{
			...
			if($type_id == 3)
			{
				...
				if($dep_id == 10)
				{
					...
					echo "You Don't Have Rights to edit item Now. Contact Administrator.";die;
				}
			}
		}
		...
		if($this->input->get('fromwofinal') && ($this->input->get('fromwofinal') == 1))
		{
			$notimsg = 'Item Update From '.$deb_qty['woi_part_no'].' '.$this->session->userdata['miconlogin']['fname'].' - '.$data['sqi_itm_pno'].' ';
			$type_id = $this->encrypt_decrypt('decrypt',$this->session->userdata['miconlogin']['typeid']);
			$dep_id =  $this->encrypt_decrypt('decrypt',$this->session->userdata['miconlogin']['dep_id']);
			$assigndep = array('test','super_admin','sales_manager','production_manager');
			if($type_id == 3)
			{
				...
				if($dep_id == 10)
				{
					...
					//echo '<pre>'; print_r($item1); die;
				$this->db->where('woi_id',$sqiitemid);
				$this->db->update('tbl_work_order_item',$item1);
				...
				$log = array(
					...
					'adlog_datetime' => date('Y-m-d H:i:s'),
					'adlog_ip' =>$_SERVER['REMOTE_ADDR'],
					'adlog_module' => 'Work_order Edit Item',
					'adlog_add' => 1,
					'adlog_userdetails' => $_SERVER['HTTP_USER_AGENT']
				);
			$this->db->insert('tbl_adminlogs',$log);
	...
	public function wo_confirm()
	{
		...
		$confirm =array(
			...
			'wo_confirm_or_not' => 1,
			'wo_confirm_or_not_date' => date('Y-m-d H:i:s')
		);
		$this->db->where('wo_id', $lid);
		$this->db->update('tbl_work_order', $confirm); 

		$confirmitm =array(
			'woi_manager_approve' => '1',
			'woi_manager_approve_date' => date('Y-m-d H:i:s')
		);
		$this->db->where('woi_wo_id', $lid);
		$this->db->update('tbl_work_order_item', $confirmitm); 
		...
		$this->db->insert('tbl_work_order_notification',$noti);
		$wonid = $this->db->insert_id();
		$assigndep = array('sales_manager','super_admin','production_manager');
		$this->assign_notification_rights($wonid,$assigndep,$single_user = false,$lid);
	}
	...
	public function get($id)
	{
		...
		$this->db->select('*');
		$this->db->from('tbl_work_order');
		//$this->db->join('tbl_master_party','tbl_work_order.vendor = tbl_master_party.master_party_id');
		$this->db->where('wo_id',$id);
	...
	public function get_edit_items()
	{
		...
		$this->db->from('tbl_work_order_item');
		//$this->db->join('tbl_master_party','tbl_work_order.vendor = tbl_master_party.master_party_id');
		$this->db->where('sqi_Work_order_id ',$id);
		//$this->db->where('sq_cid',$this->session->userdata['login']['aus_Id']);
		$query = $this->db->get();
		//echo "<pre>";print_r($query->result_array());die;
		return $query->result_array();
	...
	public function get_Work_order()
	{
		...
		$this->db->select('*');//sum(tbl_work_order.inv_grand_total) as grand_total_view
		$this->db->from('tbl_work_order');
		$this->db->where('dis_isdelete',0);
		$this->db->order_by('tbl_work_order.dis_id','DESC');
  ...
  //       }
	...
	public function get_Work_order_report()
	{
		die;
		$this->db->select('*');//sum(tbl_work_order.inv_grand_total) as grand_total_view
		$this->db->from('tbl_work_order');
		$this->db->where('sq_isdeleted',0);
		$this->db->join('tbl_mode_inquiry','tbl_work_order.sq_mode_inq = tbl_mode_inquiry.mode_inquiry_id');
		$this->db->join('tbl_admin_users','tbl_work_order.sq_end_st = tbl_admin_users.aus_Id');
		//$this->db->where('sq_cid',$this->session->userdata['login']['aus_Id']);
		$this->db->order_by('tbl_work_order.sq_id','DESC');
		if($this->input->get('party')){
			$this->db->where('tbl_work_order.sq_id', $this->input->get('party'));
		}
	...
	{
		//echo "hii"; die;
		$this->db->select('tbl_work_order.*,prepared_by.au_fname as prepared_by_fname,prepared_by.au_lname as prepared_by_lname,created_by.au_fname as created_by_fname,created_by.au_lname as created_by_lname');
		$this->db->from('tbl_work_order');
        ...
        if($this->input->post('wo_fainaltotal') && ($this->input->post('wo_fainaltotal') != ''))
        {
           $this->db->like('wo_fainaltotal', $this->input->post('wo_fainaltotal'));   
        }
        ...
        if($this->input->post('wo_created_by') && ($this->input->post('wo_created_by') != ''))
        {
           $this->db->like('created_by.au_fname', $this->input->post('wo_created_by'));   
        }
		...
		$this->db->where('wo_isdeleted', 0);
        if($this->session->userdata('miconlogin') && is_array($this->session->userdata['miconlogin']) && !empty($this->session->userdata['miconlogin']) && isset($this->session->userdata['miconlogin']['typeid']) && ($this->encrypt_decrypt('decrypt',$this->session->userdata['miconlogin']['typeid']) != 3) &&  isset($this->session->userdata['miconlogin']['userid']) && ($this->session->userdata['miconlogin']['userid'] != '' && $this->encrypt_decrypt('decrypt',$this->session->userdata['miconlogin']['dep_id']) != 11))
		{
			...
			{
				$this->db->group_start();
				// $this->db->where_in('tbl_work_order.wo_cid', $this->session->userdata['miconlogin']['all_users'],false);
				// $this->db->or_where_in('tbl_work_order.wo_preparedby',$this->session->userdata['miconlogin']['all_users'],false); 
				...
				$this->db->group_end();
				//$this->db->where('tbl_work_order.sq_cid', $this->encrypt_decrypt('decrypt',$this->session->userdata['miconlogin']['userid']));
			}else{
				$this->db->group_start();
				$this->db->where('tbl_work_order.wo_cid', $this->encrypt_decrypt('decrypt',$this->session->userdata['miconlogin']['userid']));
				$this->db->or_where('tbl_work_order.wo_preparedby',$this->encrypt_decrypt('decrypt',$this->session->userdata['miconlogin']['userid'])); 
	...
	public function wo_pdfdata()
	{
		...
		if($this->session->userdata('miconlogin') && is_array($this->session->userdata['miconlogin']) && !empty($this->session->userdata['miconlogin']) && isset($this->session->userdata['miconlogin']['typeid']) && ($this->encrypt_decrypt('decrypt',$this->session->userdata['miconlogin']['typeid']) != 3) &&  isset($this->session->userdata['miconlogin']['userid']) && ($this->session->userdata['miconlogin']['userid'] != ''))
		{
			if(isset($this->session->userdata['miconlogin']['all_users']) && is_array($this->session->userdata['miconlogin']['all_users']) && !empty($this->session->userdata['miconlogin']['all_users']))
			{
				$this->db->where_in('tbl_work_order.wo_id', $this->session->userdata['miconlogin']['all_users']);
				//$this->db->where('tbl_work_order.sq_cid', $this->encrypt_decrypt('decrypt',$this->session->userdata['miconlogin']['userid']));
			}else{
				$this->db->where('tbl_work_order.wo_id', $this->encrypt_decrypt('decrypt',$this->session->userdata['miconlogin']['userid']));
			}
		}
		if($this->input->post('wo_wo_no') && ($this->input->post('wo_wo_no') != ''))
        {
	...
	{
		$this->db->select('SUM(tbl_work_order_itax.sqit_tax_amount) as total,UPPER(tbl_work_order_itax.sqit_tax_name) as tax_name, tbl_work_order_itax.*');
		//SUM(IF(itmtax_tax_name = 'CGST', itmtax_tax_amount, 0)) AS 'CGST',
		$this->db->from('tbl_work_order_itax');
	...
	{
		$this->db->select('*');
		$this->db->from('tbl_work_order_btax');
	...
	public function get_items($id)
	{
		$this->db->select('tbl_work_order_item.*,(select ROUND(SUM(tcredit.tran_itm_qty),2) as tcreditpoints FROM tbl_transaction as tcredit WHERE tcredit.tran_cr_or_dr = 1 AND tcredit.tran_itm_id = tbl_work_order_item.woi_item_id AND tcredit.tran_is_hold = '."'0'".') as tcreditpoints,(select ROUND(SUM(tdebit.tran_itm_qty),2) as tdebitpoints FROM tbl_transaction as tdebit WHERE tdebit.tran_cr_or_dr = 2 AND tdebit.tran_itm_id = tbl_work_order_item.woi_item_id AND tdebit.tran_is_hold = '."'0'".') as tdebitpoints');
		$this->db->from('tbl_work_order_item');
		//$this->db->join('tbl_hsn_code','tbl_hsn_code.hsn_id = tbl_work_order_item.sqi_itm_hsncode','left');
		//$this->db->join('tbl_master_item','tbl_master_item.master_item_id = tbl_work_order_item.sqi_itm_pno','left');
	...
	public function add_sqoute($id)
	{
		...
		$item = array(
			...
			'sale_quotation_term' => $item['pdetails']['Work_order_desc'],
		...
		$this->db->select('*');
		$this->db->from('tbl_work_order_item');
		$this->db->where('sqi_Work_order_id',$this->input->get('id'));
		...
		$this->db->select('*');
		$this->db->from('tbl_work_order_bom');
	...
	public function Work_order_report()
	{
		$this->db->select('*');
		$this->db->from('tbl_work_order');
		//$this->db->join('tbl_master_party', 'tbl_master_party.master_party_id = tbl_work_order.vendor');
		if($this->input->get('party')){
			$this->db->where('tbl_work_order.sq_id', $this->input->get('party'));
		}
		if($this->input->get('created_start_date') && ($this->input->get('created_start_date') != ''))
	...
	//**************************************************End customer wise
		// $this->db->select('*');
		// $this->db->from('tbl_work_order_item');
		// $this->db->join('tbl_master_item', 'tbl_master_item.master_item_id = tbl_work_order_item.sqi_Work_order_id');
		// $this->db->join('tbl_work_order', 'tbl_work_order.sq_id = tbl_work_order_item.sqi_Work_order_id');
		// if($this->input->get('items')){
		// 	$this->db->where('tbl_work_order_item.sqi_Work_order_id', $this->input->get('items'));
		// }
		// if($this->input->get('created_start_date') && ($this->input->get('created_start_date') != ''))
		// {
		// 	$stdate = date("Y-m-d", strtotime($this->input->get('created_start_date')));
	...
	{
		$this->db->select('tbl_work_order.sq_id,DATE_FORMAT(tbl_work_order.sq_enq_date, "%d-%m-%Y"),tbl_work_order.vendor,tbl_work_order.sq_con_person,tbl_work_order.sq_mobile,tbl_work_order.sq_email,tbl_master_city.city_name,tbl_master_item.master_item_part_no,tbl_work_order_item.sqi_itm_qty,tbl_work_order_item.sqi_itm_price,tbl_work_order_item.sqi_itm_total,tbl_work_order.sq_end_st,source.source_cat_name as sourcename,tbl_work_order.sq_remarks');//sum(tbl_work_order.inv_grand_total) as grand_total_view
		$this->db->from('tbl_work_order');
		$this->db->join('tbl_work_order_item','tbl_work_order_item.sqi_Work_order_id = tbl_work_order.sq_id','left');
		$this->db->join('tbl_master_item','tbl_master_item.master_item_id = tbl_work_order_item.sqi_itm_pno','left');
		$this->db->join('tbl_mode_inquiry','tbl_work_order.sq_mode_inq = tbl_mode_inquiry.mode_inquiry_id','left');
		$this->db->join('tbl_country','tbl_work_order.sq_country = tbl_country.country_id','left');
		$this->db->join('tbl_master_city','tbl_work_order.sq_city = tbl_master_city.city_id','left');
		$this->db->join('tbl_master_state','tbl_work_order.sq_state = tbl_master_state.state_id','left');
		$this->db->join('tbl_sale_brand','tbl_work_order.sq_id = tbl_sale_brand.sqb_sqid','left');
		//$this->db->join('tbl_sale_quotation_brand','tbl_work_order.sa_id = tbl_sale_quotation_brand.sab_sq_id','left');
		$this->db->join('tbl_source_cat as source','tbl_work_order.sq_source_cat = source.source_cat_id');
		// $this->db->join('tbl_source_cat as subsource','tbl_work_order.sa_subsource_cat =  subsource.source_main_cat','left');
		//$this->db->join('tbl_admin_users','tbl_work_order.sq_end_st = tbl_admin_users.aus_Id');
		//$this->db->where('tbl_work_order_item.sai_oa_completed',0);
		$this->db->where('tbl_work_order_item.sqi_item_isdelete',0);
		$this->db->where('tbl_work_order.sq_isdeleted',0);
		if($this->session->userdata('miconlogin') && is_array($this->session->userdata['miconlogin']) && !empty($this->session->userdata['miconlogin']) && isset($this->session->userdata['miconlogin']['typeid']) && ($this->encrypt_decrypt('decrypt',$this->session->userdata['miconlogin']['typeid']) != 3) &&  isset($this->session->userdata['miconlogin']['userid']) && ($this->session->userdata['miconlogin']['userid'] != ''))
		{
		$this->db->where('tbl_work_order.sq_cid', $this->encrypt_decrypt('decrypt',$this->session->userdata['miconlogin']['userid']));
		}
		...
		if($this->input->get('vendor')){
			$this->db->where('tbl_work_order.sq_id', $this->input->get('vendor'));
		}
		if($this->input->get('conper')){
			$this->db->where('tbl_work_order.sq_id', $this->input->get('conper'));
		}
		if($this->input->get('created_start_date') && ($this->input->get('created_start_date') != ''))
		{
			$stdate = date("Y-m-d", strtotime($this->input->get('created_start_date')));
		...
		{
			$this->db->where('tbl_work_order.sq_country',$this->input->get('country'));
		}
		if($this->input->get('state') && ($this->input->get('state') != ''))
		{
			$this->db->where('tbl_work_order.sq_state',$this->input->get('state'));
		}
		if($this->input->get('city') && ($this->input->get('city') != ''))
		{
			$this->db->where('tbl_work_order.sq_city',$this->input->get('city'));
		}
		if($this->input->get('mobile') && ($this->input->get('mobile') != ''))
		{
			$this->db->where('tbl_work_order.sq_mobile',$this->input->get('mobile'));
		}
		if($this->input->get('status') && ($this->input->get('status') != ''))
		{
			...
			{
				$this->db->where('tbl_work_order.sq_inq_sts',1);
			}
			if($this->input->get('status') == 'Pending')
			{
				$this->db->where('tbl_work_order.sq_inq_sts',2);
			}
			if($this->input->get('status') == 'Completed')
			{
				$this->db->where('tbl_work_order.sq_inq_sts',3);
			}
			
		}
		if($this->input->get('sq_source_cat') && ($this->input->get('sq_source_cat') != ''))
		{
			$this->db->where('tbl_work_order.sq_source_cat',$this->input->get('sq_source_cat'));
		}
		if($this->input->get('sq_subsource_cat') && ($this->input->get('sq_subsource_cat') != ''))
		{
			$this->db->where('tbl_work_order.sq_subsource_cat',$this->input->get('sq_subsource_cat'));
		}
		if($this->input->get('sq_end_st') && ($this->input->get('sq_end_st') != ''))
		{
			$this->db->where('tbl_work_order.sq_end_st',$this->input->get('sq_end_st'));
		}
		//********************************************************************************************
	...
	public function wo_no_get()
	{
		...
		// $query = $this->db->get();
		// $code = $query->row_array();
		$autoid['wo_id'] = isset($autoid['wo_id']) ? $autoid['wo_id'] : 0;
    ...
    {
    	$this->db->select('sum(tbl_work_order.sq_grd_ttl) as grand_total_view');
    	$this->db->from('tbl_work_order');
    	$query = $this->db->get();
		//echo'<pre>';print_r($query->result_array());die;
		return $query->result_array();
    ...
    {
    	$this->db->select('sum(tbl_work_order_item.sqi_itm_ftotal) as sub_total_view');
    	$this->db->from('tbl_work_order_item');
    	$query = $this->db->get();
		//echo'<pre>';print_r($query->result_array());die;
		return $query->result_array();
    ...
    {
    	$this->db->select('sum(tbl_work_order_itax.sqit_tax_amount) as grand_tax_amount');
    	$this->db->from('tbl_work_order_itax');
    	$query = $this->db->get();
		return $query->result_array();
    }
    public function get_inqno_wise()
    {
    	$this->db->select('*');
		$this->db->from('tbl_work_order');
		$this->db->join('tbl_master_party', 'tbl_master_party.master_party_id = tbl_work_order.vendor','left');
		$this->db->order_by('tbl_work_order.sq_id','DESC');
    	$query = $this->db->get();
		return $query->result_array();
    }
    ...
    public function delete($id)
    {
        ...
        if($query->num_rows() > 0)
        {
        	foreach ($query->result_array() as $key => $value) {
	            ...
	            $this->db->where('woi_id',$wo_itemid);
	            ...
	            $noti = array(
	                    'wo_noti_woid' => $itemdata['woi_wo_id'],
	                    'wo_noti_wo_itmid' => $wo_itemid,
	            ...
	            $this->db->insert('tbl_work_order_notification',$noti);
	             $wonid = $this->db->insert_id();
	            $assigndep = array('test','super_admin','sales_manager','production_manager','production');
	            $this->assign_notification_rights($wonid,$assigndep,$single_user = false,$itemdata['woi_wo_id']);
        	}
        }
	...
	public function get_usalesdata($idenc)
	{
		$this->db->select('*');
		$this->db->from('tbl_work_order');
		$this->db->join('tbl_work_order_item', 'tbl_work_order.sq_id = tbl_work_order_item.sqi_Work_order_id');
		$this->db->join('tbl_master_item','tbl_work_order_item.sqi_itm_name = tbl_master_item.master_item_id');
	...
	{//tbl_followup_method
		$this->db->select('tbl_work_order.sq_id as id,tbl_work_order.sq_enq_date as inq_dt,tbl_sales_inq_followup.fu_followdate as followdate,tbl_sales_inq_followup.fu_id as fuid,tbl_work_order.sq_no as no,tbl_work_order.sq_con_person as name,tbl_work_order.sq_mobile as mno,tbl_work_order.sq_inq_sts as stname,tbl_admin_users.au_fname as executive,tbl_sales_inq_followup.fu_followupst as folst');
		$this->db->from('tbl_work_order');
		$this->db->join('tbl_sales_inq_followup', 'tbl_sales_inq_followup.fu_inq_id = tbl_work_order.sq_id','left');
		...
		if($this->input->post('name') && $this->input->post('name') != ''){
			$this->db->like('tbl_work_order.sq_con_person', $this->input->post('name'));
		}
		if($this->input->post('cno') && $this->input->post('cno') != ''){
			$this->db->like('tbl_work_order.sq_mobile', $this->input->post('cno'));
		}
		...
		if($this->input->post('status') && $this->input->post('status') != ''){
			$this->db->like('tbl_work_order.sq_inq_sts', $this->input->post('status'));
		}
	...
	public function get_listofollow()
	{
        ...
        $this->db->select('tbl_work_order.sq_id as id,tbl_work_order.sq_enq_date as inq_dt,tbl_sales_inq_followup.fu_followdate as followdate,tbl_sales_inq_followup.fu_remark as fu_remark,tbl_work_order.sq_mobile as mno,tbl_work_order.sq_con_person as name,tbl_work_order.sq_mobile as no,tbl_work_order.sq_inq_sts as stname,tbl_admin_users.au_fname as executive,tbl_sales_inq_followup.fu_id as fuid,tbl_sales_inq_followup.fu_followupst as folst');
        $this->db->from('tbl_work_order');
         $this->db->join('tbl_sales_inq_followup', 'tbl_sales_inq_followup.fu_inq_id = tbl_work_order.sq_id','left');
         $this->db->join('tbl_admin_users', 'tbl_admin_users.au_id = tbl_sales_inq_followup.fu_followexe','left');
	...
	public function delete_Work_order_item($id,$sa_id)
    {
  ...
  //       		$this->db->where('sqi_Work_order_id',$sa_id);
  //       		$query = $this->db->get();
  //       		$value['count']= $query->row_array();
  ...
  //       		$item = array(
		...
		$log = array(
					...
					'adlog_datetime' => date('Y-m-d H:i:s'),
					'adlog_ip' =>$_SERVER['REMOTE_ADDR'],
					'adlog_module' => 'Work_order Item delete',
					'adlog_delete' => 1
				);
			$this->db->insert('tbl_adminlogs',$log);
		return $id; 
	...
	public function create_qoute($id)
	{
		...
		$this->db->update('tbl_work_order',$stupdate);
		...
		$item = array(
			...
			'sale_quotation_sub' => $value['salesinq']['Work_order_sub'],
			...
			//'sq_grd_ttl_words' => $data['grdttlinword'],
			'sa_remarks' => $value['salesinq']['sq_remarks'],
			'sale_quotation_term' => $value['salesinq']['Work_order_desc'],
			'sa_tc_price' => $value['salesinq']['Work_order_tc_price'],
			'sa_tc_wrnty' => $value['salesinq']['Work_order_tc_wrnty'],
			'sa_tc_pf' => $value['salesinq']['Work_order_tc_pf'],
			'sa_tc_deli' => $value['salesinq']['Work_order_tc_deli'],
			'sa_tc_paynt' => $value['salesinq']['Work_order_tc_paynt'],
			'sa_tc_ovali' => $value['salesinq']['Work_order_tc_ovali'],
			'sa_tc_frght' => $value['salesinq']['Work_order_tc_frght'],
			'sa_tc_gst' => $value['salesinq']['Work_order_tc_gst'],
	...
	public function store_custoid()
	{
		...
		$query = $this->db->get();
		//echo "<pre>";print_r($query->result_array());die;
		$partydata = $query->result_array();
		foreach ($partydata as $key => $party) 
		{
			$this->db->set('wo_customer_id',$party['master_party_id']);
			$this->db->where('UPPER(wo_customer_name)',strtoupper($party['master_party_name']));
			$this->db->update('tbl_work_order');
		}
	}
}
