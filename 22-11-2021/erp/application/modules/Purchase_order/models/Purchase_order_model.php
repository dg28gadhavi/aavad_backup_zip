...
class Purchase_order_model extends CI_Model {
	...
	{   
		if($this->uri->segment(3))
		{
			...
			'po_con_person' => $data['sq_con_person'],
	...
	public function get($id)
	{
		...
		$this->db->join('tbl_master_vendor','tbl_purchase_order.vendor = tbl_master_vendor.master_vendor_id');
		$this->db->join('tbl_admin_users','tbl_purchase_order.po_cid = tbl_admin_users.au_id');
		$this->db->where('po_id',$id);
		//$this->db->where('sq_cid',$this->session->userdata['login']['aus_Id']);
		$query = $this->db->get();
		//echo "<pre>";print_r($query->result_array());die;
  ...
  //       }
	...
	public function get_pdfdata($id)
	{
		...
		$this->db->join('tbl_master_vendor', 'tbl_master_vendor.master_vendor_id = tbl_purchase_order.vendor','left');
	...
	{
		$sql = "SELECT * from tbl_master_vendor_contactperson WHERE contact_master_id = ".$q." AND contact_isdelete = 0";
	...
	{
		$sql = "SELECT * from tbl_master_vendor WHERE UPPER(master_vendor_com_name) LIKE '%".$this->db->escape_like_str(strtoupper($q))."%'";
		$query = $this->db->query($sql);
		//echo "<pre>"; print_r($query->result_array()); die;
		//$query = $this->db->get('tbl_master_item');
		if($query->num_rows() > 0)
		{
			foreach ($query->result_array() as $row)
			{
				$new_row['label']=htmlentities(stripslashes($row['master_vendor_com_name']));
				$new_row['value']=htmlentities(stripslashes($row['master_vendor_com_name']));
				$new_row['address']=htmlentities(stripslashes($row['master_vendor_address']));
				$new_row['country']=htmlentities(stripslashes($row['master_vendor_country']));
				$new_row['state_lists']=$this->get_state($new_row['country']);
				$new_row['state']=htmlentities(stripslashes($row['master_vendor_state']));
				//get_city
				$new_row['city_lists']=$this->get_city($new_row['state']);	
				$new_row['city']=htmlentities(stripslashes($row['master_vendor_city']));
				$new_row['cperson']=htmlentities(stripslashes($row['master_vendor_contact']));
				$new_row['ctype']=htmlentities(stripslashes($row['master_vendor_customertype']));
				$new_row['cno']=htmlentities(stripslashes($row['master_vendor_mobile_no']));
				$new_row['phone']=htmlentities(stripslashes($row['master_vendor_office_no']));
				$new_row['email']=htmlentities(stripslashes($row['master_vendor_email_address']));
				$new_row['webaddr']=htmlentities(stripslashes($row['master_vendor_website']));
				$new_row['customer_id']=htmlentities(stripslashes($row['master_vendor_id']));
				$row_set[] = $new_row; //build an array
			}
			echo json_encode($row_set); //format the array into json data
		}
