...
class Inward_model extends CI_Model {
  ...
  //       }
	...
	public function inw_no_get()
	{
		...
		$autoid['inw_id'] = isset($autoid['inw_id']) ? $autoid['inw_id'] : 0;
	...
	public function potoinward()
	{
		...
		$poi_woitm_id=$this->input->get('poi_woitm_id');
	...
	public function get_customer_information($q)
	{
		...
		if($query->num_rows() > 0)
		{
			foreach ($query->result_array() as $row)
			{
				...
				$new_row['address']=htmlentities(stripslashes($row['master_vendor_office_address']));
				...
				//$new_row['cperson']=htmlentities(stripslashes($row['master_vendor_contact_person']));
				$new_row['ctype']=htmlentities(stripslashes($row['master_vendor_customertype']));
				$new_row['cno']=htmlentities(stripslashes($row['master_vendor_contact_no']));
				$new_row['phone']=htmlentities(stripslashes($row['master_vendor_phone']));
