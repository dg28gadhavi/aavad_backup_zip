...
class Work_order_item_list_model extends CI_Model {
	...
	{   
		...
		$this->db->update('tbl_Work_order_item_list',$item); 
		...
		$lid = $id;
			...
			//echo '<pre>';print_r($item);die;
		$this->db->insert('tbl_Work_order_item_list',$item);
		...
		$log = array(
					...
					'adlog_datetime' => date('Y-m-d H:i:s'),
					'adlog_ip' =>$_SERVER['REMOTE_ADDR'],
					'adlog_module' => 'Work_order_item_list',
					'adlog_add' => 1,
					'adlog_userdetails' => $_SERVER['HTTP_USER_AGENT']
				);
			$this->db->insert('tbl_adminlogs',$log);
	...
	public function other_add($data)
	{
		...
		$item = array(
			...
			'Work_order_item_list_sub' => $data['Work_order_item_list_sub'],
			'Work_order_item_list_desc' => $data['Work_order_item_list_desc'],
			'Work_order_item_list_tc_price' => $data['Work_order_item_list_tc_price'],
			'Work_order_item_list_tc_wrnty' => $data['Work_order_item_list_tc_wrnty'],
			'Work_order_item_list_tc_pf' => $data['Work_order_item_list_tc_pf'],
			'Work_order_item_list_tc_deli' => $data['Work_order_item_list_tc_deli'],
			'Work_order_item_list_tc_paynt' => $data['Work_order_item_list_tc_paynt'],
			'Work_order_item_list_tc_ovali' => $data['Work_order_item_list_tc_ovali'],
			'Work_order_item_list_tc_frght' => $data['Work_order_item_list_tc_frght'],
			'Work_order_item_list_tc_gst' => $data['Work_order_item_list_tc_gst'],
	...
	public function item_add($data)
	{
		...
		$item1 = array(
				...
				$this->db->insert('tbl_Work_order_item_list_item',$item1);
				$item_lid = $this->db->insert_id();

				$this->db->select('SUM(woi_final_price) as count');
        		$this->db->from('tbl_Work_order_item_list_item');
        		...
        		$this->db->select('*');
        		$this->db->from('tbl_Work_order_item_list');
        		$this->db->where('wo_id',$lid);
        		$query = $this->db->get();
				...
				$log = array(
					...
					'adlog_datetime' => date('Y-m-d H:i:s'),
					'adlog_ip' =>$_SERVER['REMOTE_ADDR'],
					'adlog_module' => 'Work_order_item_list Item',
					'adlog_add' => 1,
					'adlog_userdetails' => $_SERVER['HTTP_USER_AGENT']
				);
			$this->db->insert('tbl_adminlogs',$log);
	...
	public function item_edit($data,$sqiitemid)
	{
		...
		$item1 = array(
					...
					//echo '<pre>'; print_r($item1); die;
				$this->db->where('woi_id',$sqiitemid);
				$this->db->update('tbl_Work_order_item_list_item',$item1);
				...
				$log = array(
					...
					'adlog_datetime' => date('Y-m-d H:i:s'),
					'adlog_ip' =>$_SERVER['REMOTE_ADDR'],
					'adlog_module' => 'Work_order_item_list Edit Item',
					'adlog_add' => 1,
					'adlog_userdetails' => $_SERVER['HTTP_USER_AGENT']
				);
			$this->db->insert('tbl_adminlogs',$log);
	...
	public function admin_qty_aprove($data)
	{
		...
		$this->db->update('tbl_Work_order_item_list_item', $confirm); 
	}
	public function manager_qty_aprove($data)
	{
		...
		$response['msg']="";
		$this->db->select('tbl_Work_order_item_list_item.*,(select ROUND(SUM(tcredit.tran_itm_qty),2) as tcreditpoints FROM tbl_transaction as tcredit WHERE tcredit.tran_cr_or_dr = 1 AND tcredit.tran_itm_id = tbl_Work_order_item_list_item.woi_item_id AND tcredit.tran_is_hold = '."'0'".') as tcreditpoints,(select ROUND(SUM(tdebit.tran_itm_qty),2) as tdebitpoints FROM tbl_transaction as tdebit WHERE tdebit.tran_cr_or_dr = 2 AND tdebit.tran_itm_id = tbl_Work_order_item_list_item.woi_item_id AND tdebit.tran_is_hold = '."'0'".') as tdebitpoints');
		$this->db->from('tbl_Work_order_item_list_item');
		$this->db->where('woi_id', $this->input->get('itm_id'));
		$query = $this->db->get();
	...
	public function wo_confirm()
	{
		...
		$this->db->where('wo_id', $lid);
		$this->db->update('tbl_Work_order_item_list', $confirm); 
		...
		$this->db->insert('tbl_Work_order_item_list_notification',$noti);
	...
	public function assign_notification_rights($wonid,$assigndep,$single_user,$Work_order_item_list_id)
	{
		...
		$this->db->from('tbl_Work_order_item_list');
		$this->db->where('wo_id',$Work_order_item_list_id);
		$query = $this->db->get();
	...
	public function get($id)
	{
		...
		$this->db->select('*');
		$this->db->from('tbl_Work_order_item_list');
		//$this->db->join('tbl_master_party','tbl_Work_order_item_list.vendor = tbl_master_party.master_party_id');
		$this->db->where('wo_id',$id);
	...
	public function get_edit_items()
	{
		...
		$this->db->from('tbl_Work_order_item_list_item');
		//$this->db->join('tbl_master_party','tbl_Work_order_item_list.vendor = tbl_master_party.master_party_id');
		$this->db->where('sqi_Work_order_item_list_id ',$id);
		//$this->db->where('sq_cid',$this->session->userdata['login']['aus_Id']);
		$query = $this->db->get();
		//echo "<pre>";print_r($query->result_array());die;
		return $query->result_array();
	...
	public function get_Work_order_item_list()
	{
		
		$this->db->select('*');//sum(tbl_Work_order_item_list.inv_grand_total) as grand_total_view
		$this->db->from('tbl_Work_order_item_list');
		$this->db->where('dis_isdelete',0);
		$this->db->order_by('tbl_Work_order_item_list.dis_id','DESC');
  ...
  //       }
	...
	public function get_Work_order_item_list_report()
	{
		die;
		$this->db->select('*');//sum(tbl_Work_order_item_list.inv_grand_total) as grand_total_view
		$this->db->from('tbl_Work_order_item_list');
		$this->db->where('sq_isdeleted',0);
		$this->db->join('tbl_mode_inquiry','tbl_Work_order_item_list.sq_mode_inq = tbl_mode_inquiry.mode_inquiry_id');
		$this->db->join('tbl_admin_users','tbl_Work_order_item_list.sq_end_st = tbl_admin_users.aus_Id');
		//$this->db->where('sq_cid',$this->session->userdata['login']['aus_Id']);
		$this->db->order_by('tbl_Work_order_item_list.sq_id','DESC');
		if($this->input->get('party')){
			$this->db->where('tbl_Work_order_item_list.sq_id', $this->input->get('party'));
		}
	...
	public function all_where_condition(){
		$this->db->where('woi_isdelete',0);
		$this->db->where('wo_isdeleted',0);
		...
		if($this->input->get('inv_start_date') && ($this->input->get('inv_start_date') != ''))
		{
			$stdate = date("Y-m-d", strtotime($this->input->get('inv_start_date')));
			$this->db->where('otw_invdate >=',$stdate);
		}
		if($this->input->get('inv_end_date') && ($this->input->get('inv_end_date') != ''))
		{
			$stdate = date("Y-m-d", strtotime($this->input->get('inv_end_date')));
			$this->db->where('otw_invdate <=',$stdate);
		}
        ...
        {
           $this->db->like('tbl_master_party.master_party_name', $this->input->post('vendor'));   
        }
        if($this->input->get('vendor_search') && ($this->input->get('vendor_search') != ''))
        {
           $this->db->like('tbl_master_party.master_party_name', $this->input->get('vendor_search'));   
        }
        if($this->input->post('wo_wo_date') && ($this->input->post('wo_wo_date') != ''))
        {
           $this->db->like('wo_wo_date', $this->input->post('wo_wo_date'));   
        }
         if($this->input->post('woi_part_no') && ($this->input->post('woi_part_no') != ''))
        {
           $this->db->like('woi_part_no', $this->input->post('woi_part_no'));   
        }
        if($this->input->get('part_no_search') && ($this->input->get('part_no_search') != ''))
        {
           $this->db->like('woi_part_no', $this->input->get('part_no_search'));   
        }
        ...
        if($this->input->get('customer_search') && ($this->input->get('customer_search') != ''))
        {
           $this->db->like('wo_customer_name', $this->input->get('customer_search'));   
        }
         if($this->input->post('woi_price') && ($this->input->post('woi_price') != ''))
        {
           $this->db->like('woi_price', $this->input->post('woi_price'));   
        }
         if($this->input->post('woi_discount') && ($this->input->post('woi_discount') != ''))
        {
           $this->db->like('woi_discount', $this->input->post('woi_discount'));   
        }
         if($this->input->post('woi_final_price') && ($this->input->post('woi_final_price') != ''))
        {
           $this->db->like('woi_final_price', $this->input->post('woi_final_price'));   
        }
        ...
        {
           $this->db->like('wo_preparedby', $this->input->post('wo_preparedby'));   
        }
        ...
        if($this->input->get('address_search') && ($this->input->get('address_search') != ''))
        {
        	$address_search = $this->input->get('address_search');
            $this->db->like('LOWER(wo_address)', strtolower($address_search) );  
        }
		$this->db->where('wo_isdeleted', 0);

		$type_id = $this->encrypt_decrypt('decrypt',$this->session->userdata['miconlogin']['typeid']);

		if($this->input->get('adminapproved') == 1)
		{
			$this->db->where('woi_admin_approve','1');
			$this->db->where('wo_confirm_or_not',1);
		}
		if($this->input->get('adminapproved') == 2){
			$this->db->where('woi_admin_approve','0');
			$this->db->where('wo_confirm_or_not',1);
			$this->db->where('woi_manager_approve','1');
		}

		if($this->input->get('smapproved') == 1)
		{
			$this->db->where('woi_manager_approve','1');
			$this->db->where('wo_confirm_or_not',1);
		}
		if($this->input->get('smapproved') == 2){
			$this->db->where('woi_manager_approve','0');
			$this->db->where('wo_confirm_or_not',1);
		}

		if($this->input->get('pmapproved') == 1)
		{
			$this->db->where('woi_promanager_approve','1');
			$this->db->where('wo_confirm_or_not',1);
		}
		if($this->input->get('storeapproved') == 1)
		{
			//$this->db->where('woi_production_approve','1');
			$this->db->where('woi_store_approve','1');
		}
		if($this->input->get('storeapproved') == 2)
		{
			//$this->db->where('woi_production_approve','1');
			$this->db->where('wo_confirm_or_not',1);
			$this->db->where('woi_store_approve','0');
		}
		if($this->input->get('productionapproved') == 1)
		{
			$this->db->where('woi_admin_approve','1'); 
	        $this->db->where('woi_manager_approve','1');
	        $this->db->where('woi_promanager_approve','1');
	        $this->db->where_in('woi_production_approve',array('3','1'));
	        $this->db->where('wo_confirm_or_not',1);

		}
		if($this->input->get('productionapproved') == 2)
		{
			$this->db->where('woi_production_approve','0');
			$this->db->where('wo_confirm_or_not',1);
		}

		if($this->input->get('pmapproved') == 2){
	...
	public function wo_report($Start,$end)
	{
		//echo "hii"; die;
		$this->db->select('*');
		...
		//$this->db->join('tbl_outward_item','tbl_work_order_item.woi_id = tbl_outward_item.otwi_woitemid','left');
		$this->db->join('tbl_outward','tbl_work_order.wo_id = tbl_outward.otw_work_ord_id','left');
	...
	public function wo_report_count()
	{
		//echo "hii"; die;
		$this->db->select('*');
		$this->db->from('tbl_work_order');
	...
	public function get_total_counts()
	{
		...
		$this->db->select('SUM(woi_qty) as sumqty,SUM(woi_total) as ttl_wo');
		$this->db->from('tbl_work_order');
		$this->db->join('tbl_admin_users','tbl_admin_users.au_id = tbl_work_order.wo_preparedby');
		$this->db->join('tbl_work_order_item','tbl_work_order_item.woi_wo_id = tbl_work_order.wo_id');
		//$this->db->join('tbl_outward_item','tbl_work_order_item.woi_id = tbl_outward_item.otwi_woitemid','left');
	...
	public function wo_pdfdata()
	{
		...
		if($this->session->userdata('miconlogin') && is_array($this->session->userdata['miconlogin']) && !empty($this->session->userdata['miconlogin']) && isset($this->session->userdata['miconlogin']['typeid']) && ($this->encrypt_decrypt('decrypt',$this->session->userdata['miconlogin']['typeid']) != 3) &&  isset($this->session->userdata['miconlogin']['userid']) && ($this->session->userdata['miconlogin']['userid'] != ''))
		{
			if(isset($this->session->userdata['miconlogin']['all_users']) && is_array($this->session->userdata['miconlogin']['all_users']) && !empty($this->session->userdata['miconlogin']['all_users']))
			{
				$this->db->where_in('tbl_Work_order_item_list.wo_id', $this->session->userdata['miconlogin']['all_users']);
				//$this->db->where('tbl_Work_order_item_list.sq_cid', $this->encrypt_decrypt('decrypt',$this->session->userdata['miconlogin']['userid']));
			}else{
				$this->db->where('tbl_Work_order_item_list.wo_id', $this->encrypt_decrypt('decrypt',$this->session->userdata['miconlogin']['userid']));
			}
		}
		if($this->input->post('wo_wo_no') && ($this->input->post('wo_wo_no') != ''))
        {
        ...
        $this->db->order_by('tbl_Work_order_item_list.wo_id','DESC');
		$query = $this->db->get();
		//echo "<pre>";print_r($query->result_array());die;
		return $query->result_array();
	}
	...
	{
		$this->db->select('SUM(tbl_Work_order_item_list_itax.sqit_tax_amount) as total,UPPER(tbl_Work_order_item_list_itax.sqit_tax_name) as tax_name, tbl_Work_order_item_list_itax.*');
		//SUM(IF(itmtax_tax_name = 'CGST', itmtax_tax_amount, 0)) AS 'CGST',
		$this->db->from('tbl_Work_order_item_list_itax');
	...
	{
		$this->db->select('*');
		$this->db->from('tbl_Work_order_item_list_btax');
	...
	public function get_pdfdata($id)
	{
		...
		$this->db->from('tbl_Work_order_item_list');
		$this->db->join('tbl_admin_users as testedby','tbl_Work_order_item_list.wo_testedby = testedby.au_id','left');
		$this->db->join('tbl_admin_users as preparedby','tbl_Work_order_item_list.wo_preparedby = preparedby.au_id','left');
		$this->db->where('tbl_Work_order_item_list.wo_id',$id);
		$this->db->order_by('tbl_Work_order_item_list.wo_id','DESC');
		$query = $this->db->get();
		...
		$this->db->select('*');
		$this->db->from('tbl_Work_order_item_list_item');
		$this->db->join('tbl_master_item','tbl_Work_order_item_list_item.woi_item_id = tbl_master_item.master_item_id','left');
		$this->db->where('tbl_Work_order_item_list_item.woi_wo_id',$id);
		$this->db->where('tbl_Work_order_item_list_item.woi_isdelete',0);
		$query = $this->db->get();
	...
	public function get_items($id)
	{
		$this->db->select('tbl_Work_order_item_list_item.*,(select ROUND(SUM(tcredit.tran_itm_qty),2) as tcreditpoints FROM tbl_transaction as tcredit WHERE tcredit.tran_cr_or_dr = 1 AND tcredit.tran_itm_id = tbl_Work_order_item_list_item.woi_item_id AND tcredit.tran_is_hold = '."'0'".') as tcreditpoints,(select ROUND(SUM(tdebit.tran_itm_qty),2) as tdebitpoints FROM tbl_transaction as tdebit WHERE tdebit.tran_cr_or_dr = 2 AND tdebit.tran_itm_id = tbl_Work_order_item_list_item.woi_item_id AND tdebit.tran_is_hold = '."'0'".') as tdebitpoints');
		$this->db->from('tbl_Work_order_item_list_item');
		//$this->db->join('tbl_hsn_code','tbl_hsn_code.hsn_id = tbl_Work_order_item_list_item.sqi_itm_hsncode','left');
		//$this->db->join('tbl_master_item','tbl_master_item.master_item_id = tbl_Work_order_item_list_item.sqi_itm_pno','left');
	...
	public function add_sqoute($id)
	{
		...
		$item = array(
			...
			'sale_quotation_term' => $item['pdetails']['Work_order_item_list_desc'],
		...
		$this->db->select('*');
		$this->db->from('tbl_Work_order_item_list_item');
		$this->db->where('sqi_Work_order_item_list_id',$this->input->get('id'));
		...
		$this->db->select('*');
		$this->db->from('tbl_Work_order_item_list_bom');
	...
	public function Work_order_item_list_report()
	{
		$this->db->select('*');
		$this->db->from('tbl_Work_order_item_list');
		//$this->db->join('tbl_master_party', 'tbl_master_party.master_party_id = tbl_Work_order_item_list.vendor');
		if($this->input->get('party')){
			$this->db->where('tbl_Work_order_item_list.sq_id', $this->input->get('party'));
		}
		if($this->input->get('created_start_date') && ($this->input->get('created_start_date') != ''))
	...
	//**************************************************End customer wise
		// $this->db->select('*');
		// $this->db->from('tbl_Work_order_item_list_item');
		// $this->db->join('tbl_master_item', 'tbl_master_item.master_item_id = tbl_Work_order_item_list_item.sqi_Work_order_item_list_id');
		// $this->db->join('tbl_Work_order_item_list', 'tbl_Work_order_item_list.sq_id = tbl_Work_order_item_list_item.sqi_Work_order_item_list_id');
		// if($this->input->get('items')){
		// 	$this->db->where('tbl_Work_order_item_list_item.sqi_Work_order_item_list_id', $this->input->get('items'));
		// }
		// if($this->input->get('created_start_date') && ($this->input->get('created_start_date') != ''))
		// {
		// 	$stdate = date("Y-m-d", strtotime($this->input->get('created_start_date')));
	...
	{
		$this->db->select('tbl_Work_order_item_list.sq_id,DATE_FORMAT(tbl_Work_order_item_list.sq_enq_date, "%d-%m-%Y"),tbl_Work_order_item_list.vendor,tbl_Work_order_item_list.sq_con_person,tbl_Work_order_item_list.sq_mobile,tbl_Work_order_item_list.sq_email,tbl_master_city.city_name,tbl_master_item.master_item_part_no,tbl_Work_order_item_list_item.sqi_itm_qty,tbl_Work_order_item_list_item.sqi_itm_price,tbl_Work_order_item_list_item.sqi_itm_total,tbl_Work_order_item_list.sq_end_st,source.source_cat_name as sourcename,tbl_Work_order_item_list.sq_remarks');//sum(tbl_Work_order_item_list.inv_grand_total) as grand_total_view
		$this->db->from('tbl_Work_order_item_list');
		$this->db->join('tbl_Work_order_item_list_item','tbl_Work_order_item_list_item.sqi_Work_order_item_list_id = tbl_Work_order_item_list.sq_id','left');
		$this->db->join('tbl_master_item','tbl_master_item.master_item_id = tbl_Work_order_item_list_item.sqi_itm_pno','left');
		$this->db->join('tbl_mode_inquiry','tbl_Work_order_item_list.sq_mode_inq = tbl_mode_inquiry.mode_inquiry_id','left');
		$this->db->join('tbl_country','tbl_Work_order_item_list.sq_country = tbl_country.country_id','left');
		$this->db->join('tbl_master_city','tbl_Work_order_item_list.sq_city = tbl_master_city.city_id','left');
		$this->db->join('tbl_master_state','tbl_Work_order_item_list.sq_state = tbl_master_state.state_id','left');
		$this->db->join('tbl_sale_brand','tbl_Work_order_item_list.sq_id = tbl_sale_brand.sqb_sqid','left');
		//$this->db->join('tbl_sale_quotation_brand','tbl_Work_order_item_list.sa_id = tbl_sale_quotation_brand.sab_sq_id','left');
		$this->db->join('tbl_source_cat as source','tbl_Work_order_item_list.sq_source_cat = source.source_cat_id');
		// $this->db->join('tbl_source_cat as subsource','tbl_Work_order_item_list.sa_subsource_cat =  subsource.source_main_cat','left');
		//$this->db->join('tbl_admin_users','tbl_Work_order_item_list.sq_end_st = tbl_admin_users.aus_Id');
		//$this->db->where('tbl_Work_order_item_list_item.sai_oa_completed',0);
		$this->db->where('tbl_Work_order_item_list_item.sqi_item_isdelete',0);
		$this->db->where('tbl_Work_order_item_list.sq_isdeleted',0);
		if($this->session->userdata('miconlogin') && is_array($this->session->userdata['miconlogin']) && !empty($this->session->userdata['miconlogin']) && isset($this->session->userdata['miconlogin']['typeid']) && ($this->encrypt_decrypt('decrypt',$this->session->userdata['miconlogin']['typeid']) != 3) &&  isset($this->session->userdata['miconlogin']['userid']) && ($this->session->userdata['miconlogin']['userid'] != ''))
		{
		$this->db->where('tbl_Work_order_item_list.sq_cid', $this->encrypt_decrypt('decrypt',$this->session->userdata['miconlogin']['userid']));
		}
		...
		if($this->input->get('vendor')){
			$this->db->where('tbl_Work_order_item_list.sq_id', $this->input->get('vendor'));
		}
		if($this->input->get('conper')){
			$this->db->where('tbl_Work_order_item_list.sq_id', $this->input->get('conper'));
		}
		if($this->input->get('created_start_date') && ($this->input->get('created_start_date') != ''))
		{
			$stdate = date("Y-m-d", strtotime($this->input->get('created_start_date')));
		...
		{
			$this->db->where('tbl_Work_order_item_list.sq_country',$this->input->get('country'));
		}
		if($this->input->get('state') && ($this->input->get('state') != ''))
		{
			$this->db->where('tbl_Work_order_item_list.sq_state',$this->input->get('state'));
		}
		if($this->input->get('city') && ($this->input->get('city') != ''))
		{
			$this->db->where('tbl_Work_order_item_list.sq_city',$this->input->get('city'));
		}
		if($this->input->get('mobile') && ($this->input->get('mobile') != ''))
		{
			$this->db->where('tbl_Work_order_item_list.sq_mobile',$this->input->get('mobile'));
		}
		if($this->input->get('status') && ($this->input->get('status') != ''))
		{
			...
			{
				$this->db->where('tbl_Work_order_item_list.sq_inq_sts',1);
			}
			if($this->input->get('status') == 'Pending')
			{
				$this->db->where('tbl_Work_order_item_list.sq_inq_sts',2);
			}
			if($this->input->get('status') == 'Completed')
			{
				$this->db->where('tbl_Work_order_item_list.sq_inq_sts',3);
			}
			
		}
		if($this->input->get('sq_source_cat') && ($this->input->get('sq_source_cat') != ''))
		{
			$this->db->where('tbl_Work_order_item_list.sq_source_cat',$this->input->get('sq_source_cat'));
		}
		if($this->input->get('sq_subsource_cat') && ($this->input->get('sq_subsource_cat') != ''))
		{
			$this->db->where('tbl_Work_order_item_list.sq_subsource_cat',$this->input->get('sq_subsource_cat'));
		}
		if($this->input->get('sq_end_st') && ($this->input->get('sq_end_st') != ''))
		{
			$this->db->where('tbl_Work_order_item_list.sq_end_st',$this->input->get('sq_end_st'));
		}
		//********************************************************************************************
	...
	public function wo_no_get()
	{
		...
		return $code['pre_Work_order_item_list'].''.($autoid['wo_id']+1);
	}
    public function get_report_result()
    {
    	$this->db->select('sum(tbl_Work_order_item_list.sq_grd_ttl) as grand_total_view');
    	$this->db->from('tbl_Work_order_item_list');
    	$query = $this->db->get();
		//echo'<pre>';print_r($query->result_array());die;
		return $query->result_array();
    ...
    {
    	$this->db->select('sum(tbl_Work_order_item_list_item.sqi_itm_ftotal) as sub_total_view');
    	$this->db->from('tbl_Work_order_item_list_item');
    	$query = $this->db->get();
		//echo'<pre>';print_r($query->result_array());die;
		return $query->result_array();
    ...
    {
    	$this->db->select('sum(tbl_Work_order_item_list_itax.sqit_tax_amount) as grand_tax_amount');
    	$this->db->from('tbl_Work_order_item_list_itax');
    	$query = $this->db->get();
		return $query->result_array();
    }
    public function get_inqno_wise()
    {
    	$this->db->select('*');
		$this->db->from('tbl_Work_order_item_list');
		$this->db->join('tbl_master_party', 'tbl_master_party.master_party_id = tbl_Work_order_item_list.vendor','left');
		$this->db->order_by('tbl_Work_order_item_list.sq_id','DESC');
    	$query = $this->db->get();
		return $query->result_array();
    }
    ...
    public function delete($id)
    {
		...
		$this->db->where('wo_id', $id);
		$this->db->update('tbl_Work_order_item_list');
		$log = array(
					'adlog_name' => $this->session->userdata['miconlogin']['email'],
					'adlog_datetime' => date('Y-m-d H:i:s'),
					'adlog_ip' =>$_SERVER['REMOTE_ADDR'],
	...
	public function get_usalesdata($idenc)
	{
		$this->db->select('*');
		$this->db->from('tbl_Work_order_item_list');
		$this->db->join('tbl_Work_order_item_list_item', 'tbl_Work_order_item_list.sq_id = tbl_Work_order_item_list_item.sqi_Work_order_item_list_id');
		$this->db->join('tbl_master_item','tbl_Work_order_item_list_item.sqi_itm_name = tbl_master_item.master_item_id');
	...
	{//tbl_followup_method
		$this->db->select('tbl_Work_order_item_list.sq_id as id,tbl_Work_order_item_list.sq_enq_date as inq_dt,tbl_sales_inq_followup.fu_followdate as followdate,tbl_sales_inq_followup.fu_id as fuid,tbl_Work_order_item_list.sq_no as no,tbl_Work_order_item_list.sq_con_person as name,tbl_Work_order_item_list.sq_mobile as mno,tbl_Work_order_item_list.sq_inq_sts as stname,tbl_admin_users.au_fname as executive,tbl_sales_inq_followup.fu_followupst as folst');
		$this->db->from('tbl_Work_order_item_list');
		$this->db->join('tbl_sales_inq_followup', 'tbl_sales_inq_followup.fu_inq_id = tbl_Work_order_item_list.sq_id','left');
		...
		if($this->input->post('name') && $this->input->post('name') != ''){
			$this->db->like('tbl_Work_order_item_list.sq_con_person', $this->input->post('name'));
		}
		if($this->input->post('cno') && $this->input->post('cno') != ''){
			$this->db->like('tbl_Work_order_item_list.sq_mobile', $this->input->post('cno'));
		}
		...
		if($this->input->post('status') && $this->input->post('status') != ''){
			$this->db->like('tbl_Work_order_item_list.sq_inq_sts', $this->input->post('status'));
		}
	...
	public function get_listofollow()
	{
        ...
        $this->db->select('tbl_Work_order_item_list.sq_id as id,tbl_Work_order_item_list.sq_enq_date as inq_dt,tbl_sales_inq_followup.fu_followdate as followdate,tbl_sales_inq_followup.fu_remark as fu_remark,tbl_Work_order_item_list.sq_mobile as mno,tbl_Work_order_item_list.sq_con_person as name,tbl_Work_order_item_list.sq_mobile as no,tbl_Work_order_item_list.sq_inq_sts as stname,tbl_admin_users.au_fname as executive,tbl_sales_inq_followup.fu_id as fuid,tbl_sales_inq_followup.fu_followupst as folst');
        $this->db->from('tbl_Work_order_item_list');
         $this->db->join('tbl_sales_inq_followup', 'tbl_sales_inq_followup.fu_inq_id = tbl_Work_order_item_list.sq_id','left');
         $this->db->join('tbl_admin_users', 'tbl_admin_users.au_id = tbl_sales_inq_followup.fu_followexe','left');
  ...
  //       		$item = array(
		...
		$log = array(
					...
					'adlog_datetime' => date('Y-m-d H:i:s'),
					'adlog_ip' =>$_SERVER['REMOTE_ADDR'],
					'adlog_module' => 'Work_order_item_list Item delete',
					'adlog_delete' => 1
				);
			$this->db->insert('tbl_adminlogs',$log);
		return $id; 
	...
	public function create_qoute($id)
	{
		...
		$this->db->update('tbl_Work_order_item_list',$stupdate);
		...
		$item = array(
			...
			'sale_quotation_sub' => $value['salesinq']['Work_order_item_list_sub'],
			...
			//'sq_grd_ttl_words' => $data['grdttlinword'],
			'sa_remarks' => $value['salesinq']['sq_remarks'],
			'sale_quotation_term' => $value['salesinq']['Work_order_item_list_desc'],
			'sa_tc_price' => $value['salesinq']['Work_order_item_list_tc_price'],
			'sa_tc_wrnty' => $value['salesinq']['Work_order_item_list_tc_wrnty'],
			'sa_tc_pf' => $value['salesinq']['Work_order_item_list_tc_pf'],
			'sa_tc_deli' => $value['salesinq']['Work_order_item_list_tc_deli'],
			'sa_tc_paynt' => $value['salesinq']['Work_order_item_list_tc_paynt'],
			'sa_tc_ovali' => $value['salesinq']['Work_order_item_list_tc_ovali'],
			'sa_tc_frght' => $value['salesinq']['Work_order_item_list_tc_frght'],
			'sa_tc_gst' => $value['salesinq']['Work_order_item_list_tc_gst'],
