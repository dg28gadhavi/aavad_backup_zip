...
class Outward_model extends CI_Model {
	...
	public function serialkeys_add($data)
	{
		...
		//die;
		$this->db->where('outward_item_id',$inseritem);
		//$this->db->where('outward_mainotwid',$insotwid);
		$this->db->delete('tbl_outward_serail_key');
		if(isset($data['outward_serial_keyname']) && $data['outward_serial_keyname'] != '')
		{
			foreach($data['outward_serial_keyname'] as $ikey => $serialkey){
				$serial = array(
					'outward_mainotwid' => $insotwid,
					'outward_item_id' => $inseritem,
					'outward_serial_keyname' => $data['outward_serial_keyname'][$ikey],
					'outward_serial_cdate' => date('Y-m-d H:i:s'),
					'outward_serial_udate' => date('Y-m-d H:i:s')
				);
				$this->db->insert('tbl_outward_serail_key',$serial);	
			}
		}
		return $inseritem;
	}
	...
	public function item_edit($data,$sqiitemid)
	{
		...
		if($creditcutstock >= 0 && $data['otwi_qty'] > 0)
		{
			...
			$item1 = array(
						...
						//echo '<pre>'; print_r($item1); die;
					$this->db->where('otwi_id',$sqiitemid);
					$this->db->update('tbl_outward_item',$item1);
  ...
  //       }
	...
	public function sales_inq_report()
	{
		$this->db->select('*');
		$this->db->from('tbl_outward');
		if($this->encrypt_decrypt('decrypt',$this->session->userdata['miconlogin']['dep_id']) == 9)
		{
			$this->db->join('tbl_work_order_item','tbl_work_order_item.woi_wo_id = tbl_outward.otw_work_ord_id');
        ...
        if($this->encrypt_decrypt('decrypt',$this->session->userdata['miconlogin']['dep_id']) == 9)
		{
			$this->db->order_by('woi_production_approve_date','DESC');
		}
		else{
	...
	public function otw_no_get()
	{
		...
		return $code['pre_Outward'].''.($autoid['otw_id']+1); */
		if(date("m") < 4)
		{
		...
		return 'OTW-'.$year_string.'-'.($autoid['otw_id']+1);
	}
	...
	public function delete_Outward_item($id,$sa_id)
    {
    	$this->db->select('*');
		$this->db->from('tbl_outward_item');
  ...
  //       		$this->db->where('sqi_Outward_id',$sa_id);
  //       		$query = $this->db->get();
  //       		$value['count']= $query->row_array();
  //       		//echo "<pre>";print_r($value['count']['count']);die;
  //       		$item = array(
		...
		$log = array(
					...
					'adlog_datetime' => date('Y-m-d H:i:s'),
					'adlog_ip' =>$_SERVER['REMOTE_ADDR'],
					'adlog_module' => 'Outward Item delete',
					'adlog_delete' => 1
				);
			$this->db->insert('tbl_adminlogs',$log);
		return $id; 
