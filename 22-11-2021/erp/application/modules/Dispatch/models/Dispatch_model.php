...
class Dispatch_model extends CI_Model {
	...
	{   
		if($this->uri->segment(3))
		{
			$id = $this->encrypt_decrypt('decrypt',$this->uri->segment(3));
   			$item = array(
			//'dis_no' => $data['dis_no'],
			'dis_vendor' => $data['vendor'],
			'dis_party_name' => $data['dis_party_name'],
			'dis_cust_name' => $data['dis_cust_name'],
			'dis_cust_email' => $data['dis_cust_email'],
			'dis_cust_address' => $data['dis_cust_address'],
			'dis_po_no' => $data['dis_po_no'],
			'dis_po_date' => date("Y-m-d", strtotime($data['dis_po_date'])),
			'dis_docket_no' => $data['dis_docket_no'],
			'dis_docket_date' => date("Y-m-d", strtotime($data['dis_docket_date'])),
			'dis_job_cno' => $data['dis_job_cno'],
			'dis_courier_name' => $data['dis_courier_name'],
			'dis_courier_type' => $data['dis_courier_type'],
			'dis_remark' => $data['dis_remark'],
			'dis_cid' => $this->encrypt_decrypt('decrypt',$this->session->userdata['miconlogin']['userid']),
			//'dis_cdate' => $data['dis_cdate'],
			'dis_udate' => $data['dis_udate']
			);
	...
	public function other_add($data)
	{
		...
		$item = array(
			...
			'Dispatch_sub' => $data['Dispatch_sub'],
			'Dispatch_desc' => $data['Dispatch_desc'],
			'Dispatch_tc_price' => $data['Dispatch_tc_price'],
			'Dispatch_tc_wrnty' => $data['Dispatch_tc_wrnty'],
			'Dispatch_tc_pf' => $data['Dispatch_tc_pf'],
			'Dispatch_tc_deli' => $data['Dispatch_tc_deli'],
			'Dispatch_tc_paynt' => $data['Dispatch_tc_paynt'],
			'Dispatch_tc_ovali' => $data['Dispatch_tc_ovali'],
			'Dispatch_tc_frght' => $data['Dispatch_tc_frght'],
			'Dispatch_tc_gst' => $data['Dispatch_tc_gst'],
	...
	public function item_add($data)
	{
		...
		$item1 = array(
					...
					'disi_itm_name' => $data['sqi_itm_pno_id'],
					'disi_itm_code' => $data['disi_itm_code'],
					'disi_itm_title' => $data['sqi_itm_title'],
					'disi_partno'=> isset($data['sqi_itm_pno']) ? $data['sqi_itm_pno'] : '',
					'disi_qty'=> $data['disi_qty'],
					'disi_unit'=> $data['disi_unit'],
					'disi_itm_price'=> $data['disi_itm_price'],
					'disi_itm_total'=> $data['disi_itm_total'],
					'disi_itm_discount'=> $data['disi_itm_discount'],
					'disi_ftotal'=> $data['disi_ftotal'],
					'disi_udate' => date('Y-m-d H:i:s')
					);
					//echo '<pre>'; print_r($item1); die;
				$this->db->insert('tbl_dispatch_item',$item1);

				$this->db->select('SUM(disi_ftotal) as count');
        		$this->db->from('tbl_dispatch_item');
        		$this->db->where('disi_isdelete','0');
        		$this->db->where('disi_dispatch_id',$lid);
        		$query = $this->db->get();
        		$value['count']= $query->row_array();
        		//echo "<pre>";print_r($value['count']['count']);die;
        		$item = array(
					'dis_ftotal' => $value['count']['count']
					);
					//echo '<pre>';print_r($item);die;
				$this->db->where('dis_id',$lid);
				$this->db->update('tbl_dispatch',$item);
	...
	public function item_edit($data,$sqiitemid)
	{
		...
		$item1 = array(
					...
					//echo '<pre>'; print_r($item1); die;
				$this->db->where('disi_id',$sqiitemid);
				$this->db->update('tbl_dispatch_item',$item1);
	...
	public function itmtodis()
	{
		$disid = $this->encrypt_decrypt('decrypt',$this->uri->segment(3));
		...
		{
				$item1 = array(
					'disi_dispatch_id' => $disid,
					'disi_itm_name' => $values['master_item_id'],
					'disi_itm_code' => $values['master_item_code'],
					'disi_itm_title' => $values['master_item_name'],
					'disi_partno'=> isset($values['master_item_part_no']) ? $values['master_item_part_no'] : '',
					'disi_qty'=> $itmqty,
					'disi_unit'=> '1',
					'disi_itm_price'=> $values['master_item_price'],
					'disi_itm_total'=> '0',
					'disi_itm_discount'=> '0',
					'disi_ftotal'=> '0',
					'disi_udate' => date('Y-m-d H:i:s')
	...
	public function get($id)
	{
		...
		//$this->db->join('tbl_master_party','tbl_Dispatch.vendor = tbl_master_party.master_party_id');
		$this->db->where('dis_id',$id);
	...
	public function get_edit_items()
	{
		...
		$this->db->from('tbl_dispatch_item');
		//$this->db->join('tbl_master_party','tbl_Dispatch.vendor = tbl_master_party.master_party_id');
		$this->db->where('sqi_Dispatch_id ',$id);
		//$this->db->where('sq_cid',$this->session->userdata['login']['aus_Id']);
		$query = $this->db->get();
		//echo "<pre>";print_r($query->result_array());die;
		return $query->result_array();
	...
	public function get_Dispatch()
	{
		...
		$this->db->select('*');//sum(tbl_Dispatch.inv_grand_total) as grand_total_view
		$this->db->from('tbl_dispatch');
		$this->db->where('dis_isdelete',0);
		$this->db->order_by('tbl_dispatch.dis_id','DESC');
  ...
  //       }
	...
	public function get_Dispatch_report()
	{
		die;
		$this->db->select('*');//sum(tbl_Dispatch.inv_grand_total) as grand_total_view
		$this->db->from('tbl_Dispatch');
		$this->db->where('sq_isdeleted',0);
		$this->db->join('tbl_mode_inquiry','tbl_Dispatch.sq_mode_inq = tbl_mode_inquiry.mode_inquiry_id');
		$this->db->join('tbl_admin_users','tbl_Dispatch.sq_end_st = tbl_admin_users.aus_Id');
		//$this->db->where('sq_cid',$this->session->userdata['login']['aus_Id']);
		$this->db->order_by('tbl_Dispatch.sq_id','DESC');
		if($this->input->get('party')){
			$this->db->where('tbl_Dispatch.sq_id', $this->input->get('party'));
		}
	...
	public function get_disptch_count()
	{
		$this->db->select('*');
		$this->db->from('tbl_dispatch_item');
		$this->db->join('tbl_dispatch','tbl_dispatch.dis_id  = tbl_dispatch_item.disi_dispatch_id ','left');
		$this->db->join('tbl_work_order','tbl_work_order.wo_id = tbl_dispatch.dis_woid','left');
		...
		$this->db->join('tbl_admin_users','tbl_admin_users.au_id = tbl_dispatch.dis_cid','left');
		...
		if($this->session->userdata('miconlogin') && is_array($this->session->userdata['miconlogin']) && !empty($this->session->userdata['miconlogin']) && isset($this->session->userdata['miconlogin']['typeid']) && ($this->encrypt_decrypt('decrypt',$this->session->userdata['miconlogin']['typeid']) != 3) &&  isset($this->session->userdata['miconlogin']['userid']) && ($this->session->userdata['miconlogin']['userid'] != ''))
		{
			...
			{
				//$this->db->where_in('tbl_dispatch.dis_id', $this->session->userdata['miconlogin']['all_users']);
				//$this->db->where('tbl_Dispatch.sq_cid', $this->encrypt_decrypt('decrypt',$this->session->userdata['miconlogin']['userid']));
			}else{
				//$this->db->where('tbl_dispatch.dis_id', $this->encrypt_decrypt('decrypt',$this->session->userdata['miconlogin']['userid']));
			}
		}
		if($this->input->get('status') == 1)
    	{
			$this->db->where('dis_wostatus','1');
			$this->db->where('dis_issendmail',0);
			$this->db->where('dis_isdelete',0);
    	}
    	if($this->input->get('status') == 2)
    	{
			$this->db->where('dis_wostatus','2');
			$this->db->where('dis_issendmail',1);
			$this->db->where('dis_isdelete',0);
    	}
        if($this->input->get('dis_no') && ($this->input->get('dis_no') != ''))
        {
           $this->db->like('dis_no', $this->input->get('dis_no'));   
        }
        if($this->input->post('dis_vendor') && ($this->input->post('dis_vendor') != ''))
        {
           $this->db->like('dis_vendor', $this->input->post('dis_vendor'));   
        }
         if($this->input->post('dis_party_name') && ($this->input->post('dis_party_name') != ''))
        {
           $this->db->like('dis_party_name', $this->input->post('dis_party_name'));   
        }
         if($this->input->post('dis_cust_name') && ($this->input->post('dis_cust_name') != ''))
        {
           $this->db->like('dis_cust_name', $this->input->post('dis_cust_name'));   
        }
         if($this->input->post('wo_po_no') && ($this->input->post('wo_po_no') != ''))
        {
           $this->db->like('wo_po_no', $this->input->post('wo_po_no'));   
        }
         if($this->input->post('dis_po_date') && ($this->input->post('dis_po_date') != ''))
        {
           $this->db->like('dis_po_date', $this->input->post('dis_po_date'));   
        }
        if($this->input->post('dis_docket_no') && ($this->input->post('dis_docket_no') != ''))
        {
           $this->db->like('dis_docket_no', $this->input->post('dis_docket_no'));   
        }
         if($this->input->post('dis_docket_date') && ($this->input->post('dis_docket_date') != ''))
        {
           $this->db->like('dis_docket_date', $this->input->post('dis_docket_date'));   
        }
        if($this->input->post('dis_job_cno') && ($this->input->post('dis_job_cno') != ''))
        {
           $this->db->like('dis_job_cno', $this->input->post('dis_job_cno'));   
        }
        ...
        if($this->input->post('dis_courier_name') && ($this->input->post('dis_courier_name') != ''))
        {
           $this->db->like('dis_courier_name', $this->input->post('dis_courier_name'));   
        }
        if($this->input->post('dis_courier_type') && ($this->input->post('dis_courier_type') != ''))
        {
           $this->db->like('dis_courier_type', $this->input->post('dis_courier_type'));   
        }
         if($this->input->post('dis_remark') && ($this->input->post('dis_remark') != ''))
        {
           $this->db->like('dis_remark', $this->input->post('dis_remark'));   
        }
        ...
        if($this->input->post('disi_partno') && ($this->input->post('disi_partno') != ''))
        {
           $this->db->like('disi_partno', $this->input->post('disi_partno'));   
        }
        if($this->input->post('dis_created_date') && ($this->input->post('dis_created_date') != ''))
        {
           $this->db->like('dis_cdate', $this->input->post('dis_created_date'));   
        }
	...
	public function get_disptchdemo_count()
	{
		$this->db->select('*');
		$this->db->from('tbl_dispatch_item');
		$this->db->join('tbl_dispatch','tbl_dispatch.dis_id  = tbl_dispatch_item.disi_dispatch_id ','left');
		...
		$this->db->join('tbl_admin_users','tbl_admin_users.au_id = tbl_dispatch.dis_cid','left');
		$this->db->where('dis_isdelete',0);
		$this->db->where('disi_demoreturn',0);
		$this->db->where('tbl_work_order.wo_type',3);
		if($this->session->userdata('miconlogin') && is_array($this->session->userdata['miconlogin']) && !empty($this->session->userdata['miconlogin']) && isset($this->session->userdata['miconlogin']['typeid']) && ($this->encrypt_decrypt('decrypt',$this->session->userdata['miconlogin']['typeid']) != 3) &&  isset($this->session->userdata['miconlogin']['userid']) && ($this->session->userdata['miconlogin']['userid'] != ''))
		{
			if(isset($this->session->userdata['miconlogin']['all_users']) && is_array($this->session->userdata['miconlogin']['all_users']) && !empty($this->session->userdata['miconlogin']['all_users']))
			{
		...
		if($this->input->post('dis_no') && ($this->input->post('dis_no') != ''))
        {
           $this->db->like('dis_no', $this->input->post('dis_no'));   
        }
        if($this->input->post('dis_vendor') && ($this->input->post('dis_vendor') != ''))
        {
           $this->db->like('dis_vendor', $this->input->post('dis_vendor'));   
	...
	public function get_demoreturn_count()
	{
		$this->db->select('*');
		$this->db->from('tbl_dispatch_item');
		$this->db->join('tbl_dispatch','tbl_dispatch.dis_id  = tbl_dispatch_item.disi_dispatch_id ','left');
		...
		$this->db->join('tbl_admin_users','tbl_admin_users.au_id = tbl_dispatch.dis_cid','left');
		$this->db->where('dis_isdelete',0);
		$this->db->where('disi_demoreturn',1);
		$this->db->where('tbl_work_order.wo_type',3);
		if($this->session->userdata('miconlogin') && is_array($this->session->userdata['miconlogin']) && !empty($this->session->userdata['miconlogin']) && isset($this->session->userdata['miconlogin']['typeid']) && ($this->encrypt_decrypt('decrypt',$this->session->userdata['miconlogin']['typeid']) != 3) &&  isset($this->session->userdata['miconlogin']['userid']) && ($this->session->userdata['miconlogin']['userid'] != ''))
		{
			if(isset($this->session->userdata['miconlogin']['all_users']) && is_array($this->session->userdata['miconlogin']['all_users']) && !empty($this->session->userdata['miconlogin']['all_users']))
	...
	{
		//echo "<pre>"; print_r($this->input->get('status')); die;
		$this->db->select('*');
		$this->db->from('tbl_dispatch');
		$this->db->join('tbl_dispatch_item','tbl_dispatch.dis_id  = tbl_dispatch_item.disi_dispatch_id','left');
		$this->db->join('tbl_work_order_item','tbl_work_order_item.woi_id  = tbl_dispatch_item.disi_woitmid','left');
		$this->db->join('tbl_work_order','tbl_work_order.wo_id = tbl_dispatch.dis_woid','left');
		$this->db->join('tbl_outward','tbl_outward.otw_id = tbl_dispatch.dis_otwid','left');
		$this->db->join('tbl_master_party','tbl_master_party.master_party_id = tbl_work_order.wo_customer_id','left');
		$this->db->join('tbl_admin_users','tbl_admin_users.au_id = tbl_dispatch.dis_cid','left');
        ...
        }else if(isset($posdata) && isset($posdata['order']) && isset($posdata['order'][0]) && isset($posdata['order'][0]['column']) && ($posdata['order'][0]['column'] == 11))
        {
        	if($posdata['order'][0]['dir'] == 'asc')
        	{
        		$this->db->order_by('dis_po_no','ASC');
        	}else if($posdata['order'][0]['dir'] == 'desc'){
        		$this->db->order_by('dis_po_no','DESC');
        	}
        }else if(isset($posdata) && isset($posdata['order']) && isset($posdata['order'][0]) && isset($posdata['order'][0]['column']) && ($posdata['order'][0]['column'] == 12))
        {
        	if($posdata['order'][0]['dir'] == 'asc')
        	{
        		$this->db->order_by('otw_invno','ASC');
        	}else if($posdata['order'][0]['dir'] == 'desc'){
        		$this->db->order_by('otw_invno','DESC');
        	}
        }else if(isset($posdata) && isset($posdata['order']) && isset($posdata['order'][0]) && isset($posdata['order'][0]['column']) && ($posdata['order'][0]['column'] == 13))
        {
        	if($posdata['order'][0]['dir'] == 'asc')
        	{
        		$this->db->order_by('dis_docket_date','ASC');
        	}else if($posdata['order'][0]['dir'] == 'desc'){
        		$this->db->order_by('dis_docket_date','DESC');
        	}
        }else if(isset($posdata) && isset($posdata['order']) && isset($posdata['order'][0]) && isset($posdata['order'][0]['column']) && ($posdata['order'][0]['column'] == 14))
        {
        	if($posdata['order'][0]['dir'] == 'asc')
        	{
        		$this->db->order_by('dis_cdate','ASC');
        	}else if($posdata['order'][0]['dir'] == 'desc'){
        		$this->db->order_by('dis_cdate','DESC');
        	}
        }
        else{
        	$this->db->order_by('dis_id','DESC');
	...
	public function demo_dispatch_report($Start,$end)
	{
		$this->db->select('*');
		$this->db->from('tbl_dispatch');
		$this->db->join('tbl_dispatch_item','tbl_dispatch.dis_id  = tbl_dispatch_item.disi_dispatch_id','left');
	...
	public function demoreturn_report($Start,$end)
	{
		$this->db->select('*');
		$this->db->from('tbl_dispatch');
		$this->db->join('tbl_dispatch_item','tbl_dispatch.dis_id  = tbl_dispatch_item.disi_dispatch_id','left');
		...
		$this->db->join('tbl_admin_users','tbl_admin_users.au_id = tbl_dispatch.dis_cid','left');
		$this->db->where('dis_isdelete',0);
		//$this->db->where('dis_demoreturn',1);
		$this->db->where('disi_demoreturn',1);
		$this->db->where('tbl_work_order.wo_type',3);
		if($this->session->userdata('miconlogin') && is_array($this->session->userdata['miconlogin']) && !empty($this->session->userdata['miconlogin']) && isset($this->session->userdata['miconlogin']['typeid']) && ($this->encrypt_decrypt('decrypt',$this->session->userdata['miconlogin']['typeid']) != 3) &&  isset($this->session->userdata['miconlogin']['userid']) && ($this->session->userdata['miconlogin']['userid'] != ''))
		{
	...
	{
		$this->db->select('SUM(tbl_Dispatch_itax.sqit_tax_amount) as total,UPPER(tbl_Dispatch_itax.sqit_tax_name) as tax_name, tbl_Dispatch_itax.*');
		//SUM(IF(itmtax_tax_name = 'CGST', itmtax_tax_amount, 0)) AS 'CGST',
		$this->db->from('tbl_Dispatch_itax');
	...
	{
		$this->db->select('*');
		$this->db->from('tbl_Dispatch_btax');
	...
	public function set_mailbit($id)
	{
		//die;
		$this->db->set('dis_issendmail',1);
		$this->db->set('dis_wostatus','2');
		$this->db->where('dis_id', $id);
		$this->db->update('tbl_dispatch');
	}

	public function get_dis_sts()
	{
		$value = array();
		$this->db->select('COUNT(dis_id) as count');
        $this->db->from('tbl_dispatch');
        $this->db->where('dis_wostatus','1');
        //$this->db->where('dis_issendmail',0);
        $this->db->where('dis_isdelete',0);
        ...
        $value['totalpending'] = $query->row_array();
        ...
        $query = $this->db->get();
        //echo $this->db->last_query(); die;
        $value['totalcomplete'] = $query->row_array();
	...
	{
		$value = array();
		$this->db->select('*,preparedby.au_fname as preparedbyf,preparedby.au_lname as preparedbyl');
		$this->db->from('tbl_dispatch');
		$this->db->join('tbl_work_order','tbl_work_order.wo_id = tbl_dispatch.dis_woid','left');
		...
		$this->db->select('*');
		$this->db->from('tbl_dispatch_item');
		$this->db->join('tbl_master_item','tbl_dispatch_item.disi_itm_name = tbl_master_item.master_item_id','left');
	...
	public function get_items($id)
	{
		$this->db->select('*');
		$this->db->from('tbl_dispatch_item');
		//$this->db->join('tbl_hsn_code','tbl_hsn_code.hsn_id = tbl_Dispatch_item.sqi_itm_hsncode','left');
		//$this->db->join('tbl_master_item','tbl_master_item.master_item_id = tbl_dispatch_item.sqi_itm_pno','left');
		$this->db->where('disi_dispatch_id',$id);
		$this->db->where('disi_isdelete',0);
	...
	public function add_sqoute($id)
	{
		...
		$item = array(
			...
			'sale_quotation_term' => $item['pdetails']['Dispatch_desc'],
		...
		$this->db->select('*');
		$this->db->from('tbl_dispatch_item');
		$this->db->where('sqi_Dispatch_id',$this->input->get('id'));
		$this->db->where('sqi_is_bom !=',1);
		$query = $this->db->get();
	...
	public function Dispatch_report()
	{
		$this->db->select('*');
		$this->db->from('tbl_Dispatch');
		//$this->db->join('tbl_master_party', 'tbl_master_party.master_party_id = tbl_Dispatch.vendor');
		if($this->input->get('party')){
			$this->db->where('tbl_Dispatch.sq_id', $this->input->get('party'));
		}
		if($this->input->get('created_start_date') && ($this->input->get('created_start_date') != ''))
	...
	//**************************************************End customer wise
		// $this->db->select('*');
		// $this->db->from('tbl_Dispatch_item');
		// $this->db->join('tbl_master_item', 'tbl_master_item.master_item_id = tbl_Dispatch_item.sqi_Dispatch_id');
		// $this->db->join('tbl_Dispatch', 'tbl_Dispatch.sq_id = tbl_Dispatch_item.sqi_Dispatch_id');
		// if($this->input->get('items')){
		// 	$this->db->where('tbl_Dispatch_item.sqi_Dispatch_id', $this->input->get('items'));
		// }
		// if($this->input->get('created_start_date') && ($this->input->get('created_start_date') != ''))
		// {
		// 	$stdate = date("Y-m-d", strtotime($this->input->get('created_start_date')));
	...
	{
		$this->db->select('tbl_Dispatch.sq_id,DATE_FORMAT(tbl_Dispatch.sq_enq_date, "%d-%m-%Y"),tbl_Dispatch.vendor,tbl_Dispatch.sq_con_person,tbl_Dispatch.sq_mobile,tbl_Dispatch.sq_email,tbl_master_city.city_name,tbl_master_item.master_item_part_no,tbl_dispatch_item.sqi_itm_qty,tbl_dispatch_item.sqi_itm_price,tbl_dispatch_item.sqi_itm_total,tbl_Dispatch.sq_end_st,source.source_cat_name as sourcename,tbl_Dispatch.sq_remarks');//sum(tbl_Dispatch.inv_grand_total) as grand_total_view
		$this->db->from('tbl_Dispatch');
		$this->db->join('tbl_dispatch_item','tbl_dispatch_item.sqi_Dispatch_id = tbl_Dispatch.sq_id','left');
		$this->db->join('tbl_master_item','tbl_master_item.master_item_id = tbl_dispatch_item.sqi_itm_pno','left');
		$this->db->join('tbl_mode_inquiry','tbl_Dispatch.sq_mode_inq = tbl_mode_inquiry.mode_inquiry_id','left');
		$this->db->join('tbl_country','tbl_Dispatch.sq_country = tbl_country.country_id','left');
		$this->db->join('tbl_master_city','tbl_Dispatch.sq_city = tbl_master_city.city_id','left');
		$this->db->join('tbl_master_state','tbl_Dispatch.sq_state = tbl_master_state.state_id','left');
		$this->db->join('tbl_sale_brand','tbl_Dispatch.sq_id = tbl_sale_brand.sqb_sqid','left');
		//$this->db->join('tbl_sale_quotation_brand','tbl_Dispatch.sa_id = tbl_sale_quotation_brand.sab_sq_id','left');
		$this->db->join('tbl_source_cat as source','tbl_Dispatch.sq_source_cat = source.source_cat_id');
		// $this->db->join('tbl_source_cat as subsource','tbl_Dispatch.sa_subsource_cat =  subsource.source_main_cat','left');
		//$this->db->join('tbl_admin_users','tbl_Dispatch.sq_end_st = tbl_admin_users.aus_Id');
		//$this->db->where('tbl_Dispatch_item.sai_oa_completed',0);
		$this->db->where('tbl_dispatch_item.sqi_item_isdelete',0);
		$this->db->where('tbl_Dispatch.sq_isdeleted',0);
		if($this->session->userdata('miconlogin') && is_array($this->session->userdata['miconlogin']) && !empty($this->session->userdata['miconlogin']) && isset($this->session->userdata['miconlogin']['typeid']) && ($this->encrypt_decrypt('decrypt',$this->session->userdata['miconlogin']['typeid']) != 3) &&  isset($this->session->userdata['miconlogin']['userid']) && ($this->session->userdata['miconlogin']['userid'] != ''))
		{
		$this->db->where('tbl_Dispatch.sq_cid', $this->encrypt_decrypt('decrypt',$this->session->userdata['miconlogin']['userid']));
		}
		...
		if($this->input->get('vendor')){
			$this->db->where('tbl_Dispatch.sq_id', $this->input->get('vendor'));
		}
		if($this->input->get('conper')){
			$this->db->where('tbl_Dispatch.sq_id', $this->input->get('conper'));
		}
		if($this->input->get('created_start_date') && ($this->input->get('created_start_date') != ''))
		{
			$stdate = date("Y-m-d", strtotime($this->input->get('created_start_date')));
		...
		{
			$this->db->where('tbl_Dispatch.sq_country',$this->input->get('country'));
		}
		if($this->input->get('state') && ($this->input->get('state') != ''))
		{
			$this->db->where('tbl_Dispatch.sq_state',$this->input->get('state'));
		}
		if($this->input->get('city') && ($this->input->get('city') != ''))
		{
			$this->db->where('tbl_Dispatch.sq_city',$this->input->get('city'));
		}
		if($this->input->get('mobile') && ($this->input->get('mobile') != ''))
		{
			$this->db->where('tbl_Dispatch.sq_mobile',$this->input->get('mobile'));
		}
		if($this->input->get('status') && ($this->input->get('status') != ''))
		{
			...
			{
				$this->db->where('tbl_Dispatch.sq_inq_sts',1);
			}
			if($this->input->get('status') == 'Pending')
			{
				$this->db->where('tbl_Dispatch.sq_inq_sts',2);
			}
			if($this->input->get('status') == 'Completed')
			{
				$this->db->where('tbl_Dispatch.sq_inq_sts',3);
			}
			
		}
		if($this->input->get('sq_source_cat') && ($this->input->get('sq_source_cat') != ''))
		{
			$this->db->where('tbl_Dispatch.sq_source_cat',$this->input->get('sq_source_cat'));
		}
		if($this->input->get('sq_subsource_cat') && ($this->input->get('sq_subsource_cat') != ''))
		{
			$this->db->where('tbl_Dispatch.sq_subsource_cat',$this->input->get('sq_subsource_cat'));
		}
		if($this->input->get('sq_end_st') && ($this->input->get('sq_end_st') != ''))
		{
			$this->db->where('tbl_Dispatch.sq_end_st',$this->input->get('sq_end_st'));
		}
		//********************************************************************************************
	...
	public function demo_no_get($id)
	{
		if(date("m") < 4)
		{
		...
		return 'DR-'.$year_string.'-'.($id);
	}

	public function dis_no_get()
	{
		...
		return $code['pre_dispatch'].''.($autoid['dis_id']+1); */
		if(date("m") < 4)
		{
			$last_year = date('y', strtotime('-1 year'));
			$this_year = date('y');
		...
		$autoid['dis_id'] = isset($autoid['dis_id']) ? $autoid['dis_id'] : 0;
		return 'DIS-'.$year_string.'-'.($autoid['dis_id']+1);
	}
    public function get_report_result()
    {
    	$this->db->select('sum(tbl_Dispatch.sq_grd_ttl) as grand_total_view');
    	$this->db->from('tbl_Dispatch');
    	$query = $this->db->get();
		//echo'<pre>';print_r($query->result_array());die;
		return $query->result_array();
    ...
    {
    	$this->db->select('sum(tbl_dispatch_item.sqi_itm_ftotal) as sub_total_view');
    	$this->db->from('tbl_dispatch_item');
    	$query = $this->db->get();
		//echo'<pre>';print_r($query->result_array());die;
		return $query->result_array();
    ...
    {
    	$this->db->select('sum(tbl_Dispatch_itax.sqit_tax_amount) as grand_tax_amount');
    	$this->db->from('tbl_Dispatch_itax');
    	$query = $this->db->get();
		return $query->result_array();
    }
    public function get_inqno_wise()
    {
    	$this->db->select('*');
		$this->db->from('tbl_Dispatch');
		$this->db->join('tbl_master_party', 'tbl_master_party.master_party_id = tbl_Dispatch.vendor','left');
		$this->db->order_by('tbl_Dispatch.sq_id','DESC');
    	$query = $this->db->get();
		return $query->result_array();
    }
	...
	public function get_usalesdata($idenc)
	{
		$this->db->select('*');
		$this->db->from('tbl_Dispatch');
		$this->db->join('tbl_dispatch_item', 'tbl_Dispatch.sq_id = tbl_dispatch_item.sqi_Dispatch_id');
		$this->db->join('tbl_master_item','tbl_dispatch_item.sqi_itm_name = tbl_master_item.master_item_id');
	...
	{//tbl_followup_method
		$this->db->select('tbl_Dispatch.sq_id as id,tbl_Dispatch.sq_enq_date as inq_dt,tbl_sales_inq_followup.fu_followdate as followdate,tbl_sales_inq_followup.fu_id as fuid,tbl_Dispatch.sq_no as no,tbl_Dispatch.sq_con_person as name,tbl_Dispatch.sq_mobile as mno,tbl_Dispatch.sq_inq_sts as stname,tbl_admin_users.au_fname as executive,tbl_sales_inq_followup.fu_followupst as folst');
		$this->db->from('tbl_Dispatch');
		$this->db->join('tbl_sales_inq_followup', 'tbl_sales_inq_followup.fu_inq_id = tbl_Dispatch.sq_id','left');
		...
		if($this->input->post('name') && $this->input->post('name') != ''){
			$this->db->like('tbl_Dispatch.sq_con_person', $this->input->post('name'));
		}
		if($this->input->post('cno') && $this->input->post('cno') != ''){
			$this->db->like('tbl_Dispatch.sq_mobile', $this->input->post('cno'));
		}
		...
		if($this->input->post('status') && $this->input->post('status') != ''){
			$this->db->like('tbl_Dispatch.sq_inq_sts', $this->input->post('status'));
		}
	...
	public function get_listofollow()
	{
        ...
        $this->db->select('tbl_Dispatch.sq_id as id,tbl_Dispatch.sq_enq_date as inq_dt,tbl_sales_inq_followup.fu_followdate as followdate,tbl_sales_inq_followup.fu_remark as fu_remark,tbl_Dispatch.sq_mobile as mno,tbl_Dispatch.sq_con_person as name,tbl_Dispatch.sq_mobile as no,tbl_Dispatch.sq_inq_sts as stname,tbl_admin_users.au_fname as executive,tbl_sales_inq_followup.fu_id as fuid,tbl_sales_inq_followup.fu_followupst as folst');
        $this->db->from('tbl_Dispatch');
         $this->db->join('tbl_sales_inq_followup', 'tbl_sales_inq_followup.fu_inq_id = tbl_Dispatch.sq_id','left');
         $this->db->join('tbl_admin_users', 'tbl_admin_users.au_id = tbl_sales_inq_followup.fu_followexe','left');
	...
	public function delete_dispatch_item($id,$sa_id)
    {
    	//echo "<pre>"; print_r($sa_id); die;
		$this->db->set('disi_isdelete',1);
		$this->db->where('disi_id', $id);
		$this->db->update('tbl_dispatch_item');

		// $this->db->select('SUM(sqi_itm_ftotal) as count');
  //       		$this->db->from('tbl_dispatch_item');
  //       		$this->db->where('sqi_item_isdelete',0);
  //       		$this->db->where('sqi_Dispatch_id',$sa_id);
  //       		$query = $this->db->get();
  //       		$value['count']= $query->row_array();
  //       		//echo "<pre>";print_r($value['count']['count']);die;
  //       		$item = array(
		...
		$log = array(
					...
					'adlog_datetime' => date('Y-m-d H:i:s'),
					'adlog_ip' =>$_SERVER['REMOTE_ADDR'],
					'adlog_module' => 'Dispatch Item delete',
					'adlog_delete' => 1
				);
			$this->db->insert('tbl_adminlogs',$log);
		return $id; 
	...
	public function create_qoute($id)
	{
		...
		$this->db->update('tbl_Dispatch',$stupdate);
		...
		$item = array(
			...
			'sale_quotation_sub' => $value['salesinq']['Dispatch_sub'],
			...
			//'sq_grd_ttl_words' => $data['grdttlinword'],
			'sa_remarks' => $value['salesinq']['sq_remarks'],
			'sale_quotation_term' => $value['salesinq']['Dispatch_desc'],
			'sa_tc_price' => $value['salesinq']['Dispatch_tc_price'],
			'sa_tc_wrnty' => $value['salesinq']['Dispatch_tc_wrnty'],
			'sa_tc_pf' => $value['salesinq']['Dispatch_tc_pf'],
			'sa_tc_deli' => $value['salesinq']['Dispatch_tc_deli'],
			'sa_tc_paynt' => $value['salesinq']['Dispatch_tc_paynt'],
			'sa_tc_ovali' => $value['salesinq']['Dispatch_tc_ovali'],
			'sa_tc_frght' => $value['salesinq']['Dispatch_tc_frght'],
			'sa_tc_gst' => $value['salesinq']['Dispatch_tc_gst'],
	...
	// public function old_dis_to_new_dis()
	// {
	...
	public function get_create_invoice($id)
	{
		//die;
		$this->db->select('*');
		$this->db->from('tbl_dispatch_item');
		...
		$this->db->set('wo_iscompleted',0);
		$this->db->set('wo_demoreturn',1);
		$this->db->where('wo_id', $datas['disi_woid']);
		$this->db->update('tbl_work_order');

		$this->db->set('woi_account_approve','0');
		$this->db->set('woi_demoreturn',1);
		$this->db->where('woi_id', $datas['disi_woitmid']);
		$this->db->update('tbl_work_order_item');

		$this->db->set('otw_demoreturn',1);
		$this->db->set('otw_completed','1');
		$this->db->where('otw_id', $datas['disi_otwid']);
		$this->db->update('tbl_outward');

		$this->db->set('otwi_demoreturn',1);
		$this->db->where('otwi_id', $datas['disi_otwitm']);
		$this->db->update('tbl_outward_item');

		$this->db->set('dis_demoreturn',1);
		$this->db->where('dis_id', $datas['disi_dispatch_id']);
		$this->db->update('tbl_dispatch');

		$this->db->set('disi_demoreturn',1);
		$this->db->where('disi_id', $datas['disi_id']);
		$this->db->update('tbl_dispatch_item');

	}
	
	public function get_freereturn_stock($id,$data)
	{
		...
		$this->db->set('dis_return_return_no',$data['return_no']);
		$this->db->set('die_demoreturn_jobno',$data['jobcardno']);
		$this->db->set('die_demoreturn_remark',$data['remark']);
		$this->db->set('dis_demoreturn_date',date('Y-m-d H:i:s'));
		$this->db->where('dis_id', $datas['disi_dispatch_id']);
		$this->db->update('tbl_dispatch');
	...
	public function get_return_stock($id,$data)
	{
		$this->db->select('*');
		$this->db->from('tbl_dispatch_item');
		$this->db->where('disi_id',$id);
		...
		$this->db->set('dis_return_return_no',$data['return_no']);
		$this->db->set('dis_return_challan_no',$data['challan_no']);
		$this->db->set('dis_return_challan_date',date("Y-m-d", strtotime($data['challan_date'])));
		$this->db->where('dis_id', $datas['disi_dispatch_id']);
		$this->db->update('tbl_dispatch');

		$this->db->set('outward_serial_isdelete','1');
		$this->db->where('outward_mainotwid', $datas['disi_otwid']);
		$this->db->where('outward_item_id', $datas['disi_otwitm']);
		$this->db->update('tbl_outward_serail_key');


		$transaction = array(
			'tran_itm_id' => $datas['disi_itm_name'],
			'tran_itm_qty' => $datas['disi_qty'],
			'tran_cr_or_dr' => 1,
			'tran_add_or_edit' => 1,
			'tran_demoreturn' => 1,
			'tran_wo_id' => $datas['disi_woid'],
			'tran_wo_itemid' => $datas['disi_woitmid'],
