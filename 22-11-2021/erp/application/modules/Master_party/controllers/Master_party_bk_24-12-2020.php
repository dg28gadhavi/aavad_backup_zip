...
class Master_party extends CI_Controller {
	...
	public function ajax()
	{
		...
		if($right_status == false)
		{
			...
			{
				$deletestr = '<a href="'.base_url().'Master_party/delete/'.$idenc.'"onclick="return confirm('."'Are you sure you want to Delete this record?'".')" class="btn btn-sm btn-outline red"><i class="fa fa-trash"></i></a>';
			}else{
				$deletestr = '';
			}
		...
		$records["data"][] = array(
				...
				''.$user[$i]['master_party_office_address'],
				''.$user[$i]['master_party_location'],
				''.$user[$i]['master_party_mobile_no'],
				''.$user[$i]['master_party_email_address'],
				''.$user[$i]['master_party_docdone'],
				''.$user[$i]['ctype_name'],
				''.$user[$i]['master_party_co_ai'],
				$user[$i]['contact_pname']."</br>".$user[$i]['contact_mobile']."</br>".$user[$i]['contact_phone'],
				''.$user[$i]['master_party_remark'].'<br/>dt: '.$user[$i]['master_party_udate'],
				''.$editstr.''.$deletestr.'',
		);
		}
