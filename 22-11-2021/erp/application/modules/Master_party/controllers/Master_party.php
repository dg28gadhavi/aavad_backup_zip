...
class Master_party extends CI_Controller {
	...
	public function index()
	{
		...
		{
			$this->session->set_flashdata('rights_error', "You Don't have rights to access Master Party View functionality");
			redirect(base_url());
		}
	...
	public function store_excel()
	{
        ...
        $this->excel->getActiveSheet()->setTitle('File');
        $finalar = $this->master_party_model->get_excel_certificate();
        $fhead = array('C/O AAVAD INSTRUMENT','DOCUMENTATION DONE','TYPE OF INDUSTRIES','LOCATION','COMPANY NAME','CONTACT PERSON','DEPARMANT','ADDRESS','PINCODE','PHONE NUMBER','MOBILE NUMBER','E-MAIL ID','REMARK','PRODUCT USEGE');
        $i=0;
 		$datas = array_unshift($finalar, $fhead);
		//echo '<pre>';print_r($finalar);die;
 		$this->excel->getActiveSheet()->fromArray($finalar);
        $filename='aavad.xls'; //save our workbook as this file name
	...
	{
		$user = $this->master_party_model->get_Master_party_report();
		...
		$id = ($i + 1);
		$idenc = $this->encrypt_decrypt('encrypt',$user[$i]['master_party_id']);
		$right_status = $this->check_rights('edit');
		if($right_status == false)
		{
			$editstr = '';
		}else{
			$editstr = '<a href="'.base_url().'Master_party/quatation_tab/'.$idenc.'" class="btn btn-sm btn-outline green"><i class="fa fa-pencil"></i></a>';
		}
		...
		if($right_status == false)
		{
			...
			{
				$deletestr = '<a href="'.base_url().'Master_party/delete/'.$idenc.'?cid='.$user[$i]['contact_id'].'"onclick="return confirm('."'Are you sure you want to Delete this record?'".')" class="btn btn-sm btn-outline red"><i class="fa fa-trash"></i></a>';
			}else{
				$deletestr = '';
			}
		...
		$records["data"][] = array(
				...
				$user[$i]['contact_pname'],
				$user[$i]['contact_address'],
				''.$user[$i]['contact_location'],
				$user[$i]['contact_mobile'].',<br/>'.$user[$i]['contact_phone'],
				$user[$i]['contact_email'],
				''.$user[$i]['master_party_docdone'],
				''.$user[$i]['ctype_name'],
				''.$user[$i]['master_party_co_ai'],
				$user[$i]['contact_remark'].'<br/><br/>Dt: '.$user[$i]['contact_udate'],
				''.$editstr.''.$deletestr.'',
		);
		}
	...
	public function add()
	{
		...
		if($right_status == false)
		{
			$this->session->set_flashdata('rights_error', "You Don't have rights to access Master Party Add functionality");
			redirect(base_url());
		}
		...
		if($success == TRUE)
		{
			...
			{	//echo '<pre>';print_r($this->input->post());die;
				...
				$value['master_party_cdate'] = date('Y-m-d H:i:s');
				$value['master_party_udate'] = date('Y-m-d H:i:s');
				$this->session->set_userdata('qtabno',2);
				$lid = $this->master_party_model->add($value);
				$this->session->set_userdata('qtabno',1);
				/*//***********PDF File Code Start********************
				$pdfdata = $this->master_party_model->get_pdfdata($lid,'master_party');
				//echo '<pre>';print_r($pdfdata);die;
				$html = $this->load->view('Master_party/Master_party_pdf_view',$pdfdata,TRUE);
				...
				if($lid)
				{
					$this->session->set_flashdata('success', 'Master Party added successfully.');
					redirect(base_url('Master_party/quatation_tab'."/".$this->encrypt_decrypt('encrypt',$lid)), 'refresh');
				}else{
					$this->session->set_flashdata('error', 'Master Party not added successfully!!');
	...
	public function edit($id = FALSE)
	{
		...
		if($right_status == false)
		{
			$this->session->set_flashdata('rights_error', "You Don't have rights to access Master Party Edit functionality");
			redirect(base_url());
		}
		//require 'Zebra_Image.php';
		...
		if($enid && ($enid != ''))
		{
			...
			if($success == FALSE)
			{
				if(isset($idencr) && $idencr != ''){
					...
					{
						//echo "hi"; die;
						$this->data['countries'] = $this->master_party_model->get_country();
						$this->data['states'] = $this->master_party_model->get_state();
						$this->data['cities'] = $this->master_party_model->get_city();
						$this->data['brands'] = $this->master_party_model->get_salesbrand();
						$this->data['master_party_com_names'] = $this->master_party_model->get_masterparty();
						$this->data['modeinquries'] = $this->master_party_model->get_modeinquiry();
						$this->data['sources'] = $this->master_party_model->get_sourcecat();
						$this->data['subsources'] = $this->master_party_model->get_sourcesub_category();
	...
	public function validation() 
	{
		if($this->input->post(NULL,TRUE))
		{
			...
			if($this->uri->segment(2) == 'add'){
				$this->form_validation->set_rules('master_party_com_name', 'master_party_com_name', 'trim|required');  
	...
	{
		//$this->data['getMaster_party'] = $this->master_party_model->addtMaster_party();
		$this->data['custometyps'] = $this->master_party_model->get_customertype();
		$this->data['currencys'] = $this->master_party_model->get_currency();
		$this->data['party_code'] = $this->master_party_model->sa_no_get();
		$this->data['admins'] = $this->master_party_model->get_admin();
		$this->data['countries'] = $this->master_party_model->get_country();
	...
	public function quatation_tab()
	{
		...
		$this->data['countries'] = $this->master_party_model->get_country();
		$this->data['list'] = $this->master_party_model->get($idencr);
		$this->data['items'] = $this->master_party_model->get_items($idencr);
		...
		if($this->input->get('acttype') && $this->input->get('itemid') && ($this->input->get('acttype') == 'edit'))
		{
			...
			$this->data['edit_items'] = $this->master_party_model->get_edit_inqitems($idencr,$eitemid);
			//echo '<pre>';print_r($this->data['edit_items']);die;
			$this->data['action_item'] = "Master_party/item_edit/".$this->uri->segment(3).'?acttype=edit&itemid='.$eitemid;
		}else{
			$this->data['action_item'] = "Master_party/item_details/".$this->uri->segment(3);
		}
		...
		$this->data['citys'] = $this->master_party_model->get_city_data();
		$this->data['tax_cats'] = $this->master_party_model->get_taxs_data();
		$this->data['main_content'] = 'Masterparty_tab_form_view';
		$this->data['action_bd'] = "Master_party/add/".$this->uri->segment(3);
		// $this->data['action_item'] = "Master_party/item_details/".$this->uri->segment(3);
		//$this->data['action_other'] = "Master_party/other_details/".$this->uri->segment(3);
		//$this->data['action_follow'] = "Master_party/follow_details/".$this->uri->segment(3);
		//$enid =$this->uri->segment(3) ? $this->encrypt_decrypt('decrypt', $this->uri->segment(3)) : '';
		//$this->data['item_data'] = $this->master_party_model->get_item_data($enid);
		$this->load->view('includes/template',$this->data);
	}
		...
		ini_set('memory_limit', '-1');
			...
			//echo '<pre>';print_r($this->input->post(NULL,FALSE));die;
				...
				$this->session->set_userdata('tabno',2);
					...
					$result = $this->master_party_model->item_img($value);
					//if(count($result) > 0)
					//{
				...
				$lid = $this->master_party_model->item_edit($value,$sqiitemid);
				if($lid)
				{
					$this->session->set_flashdata('success', 'Details of item Edited successfully.');
					redirect(base_url('Master_party/quatation_tab/'.$this->uri->segment(3).'?acttype=edit&itemid='.$sqiitemid), 'refresh');
				}else{
					$this->session->set_flashdata('error', 'Details of item not Edited successfully!!');
					redirect(base_url('Master_party/quatation_tab'), 'refresh');
				}
			 	redirect(base_url('Master_party'), 'refresh');
			}else{
			$this->quatation_tab();
	...
	public function item_details()
	{
		...
		if($right_status == false)
		{
			...
			redirect(base_url());
				...
				$lid = $this->master_party_model->item_add($value,$enid);
				if($lid)
				{
					//echo "$lid";die();
		...
		if($enid && ($enid != ''))
		{
			...
			if(isset($id) && $id!= ''){
				$this->session->set_userdata('tabno',3);
					$lid = $this->master_party_model->delete_fup($idencr);
						if ($lid) {
	...
	public function delete_items($id=false)
	{
		...
		if($right_status == false)
		{
			$this->session->set_flashdata('rights_error', "You Don't have rights to access Master Party Delete functionality");
			redirect(base_url());
		}
...

		{
			...
			if(isset($itemid) && $itemid!= '')
			{
					...
					$lid = $this->master_party_model->delete_select_item($itemid,$enid);
						...
						redirect(base_url('Master_party/item_details/'.$this->encrypt_decrypt('encrypt',$enid)), 'refresh');
	...
	public function delete($id=false)
	{
		...
		if($enid && ($enid != ''))
		{
			...
			if(isset($id) && $id!= ''){
				...
				if(!empty($this->data['list']))
				{
				    $custoid = $this->input->get('cid');
					$lid = $this->master_party_model->delete($idencr,$custoid);
						if ($lid) {
						$this->session->set_flashdata('success', 'Master Party Deleted successfully.');
						redirect('Master_party', 'refresh');
						} else {
							$this->session->set_flashdata('error', 'Master Party not Deleted successfully!!.');
							redirect('Master_party', 'refresh'); 
						}						
				}else{
					$this->session->set_flashdata('error', 'Master Party not Available!!');
			  		redirect('Master_party', 'refresh'); 
			  	}
			}
			else{
				...
				if ($this->csvimport->get_array($file_path)) {
					...
					if(is_array($csv_array) && !empty($csv_array))
					{
						foreach ($csv_array as $row)
						{
							...
							if(isset($row['COMPANY NAME']) && $row['COMPANY NAME'] != '' && $row['CONTACT PERSON'] != '')
							{
	...
	public function check_rights($type)
	{
		...
		if(isset($this->session->userdata['miconlogin']['rightsid']) && ($this->session->userdata['miconlogin']['rightsid'] != '') && isset($this->session->userdata['miconlogin']['typeid']) && ($this->session->userdata['miconlogin']['typeid'] != ''))
		{
			...
			if($typeid != 3)
			{
				...
				$finalrights = $this->global_model->get_rights($rightsid,$moduleid = 4,$type);
	...
	public function getsub_category()
	{
		...
		$this->data['sub_catlists'] = $this->master_party_model->get_subsource($this->abc);
			...
			if(isset($id) && $id!= ''){
				$this->session->set_userdata('qtabno',3);
					$lid = $this->master_party_model->delete_fup($idencr);
						...
						if ($lid) {
						$this->session->set_flashdata('success', 'Followup detail deleted successfully.');
						redirect('Master_party/quatation_tab/'.$this->uri->segment(4), 'refresh');
						} else {
							$this->session->set_flashdata('error', 'Followup detail not deleted successfully!!.');
							redirect('Master_party/quatation_tab/'.$this->uri->segment(4), 'refresh'); 
						}						
	...
	public function delete_itms($id=false)
	{
		...
		if($enid && ($enid != ''))
		{
			...
			{
				$this->session->set_userdata('qtabno',2);
					$lid = $this->master_party_model->delete_itms($idencr,$ca_id);
					//***********PDF File Code Start********************
	...
	public function mail()
	{
		$enid = $this->uri->segment(3) ? $this->uri->segment(3) : '';
		$idencr = $this->uri->segment(3) ? $this->encrypt_decrypt('decrypt', $enid) : '';
		$this->data['lists'] = $this->master_party_model->get($idencr);
		//$this->data['getMaster_party'] = $this->master_party_model->addtMaster_party();
		$this->data['main_content'] = 'sale_qoutation_mail_view';
		$this->data['action_mail'] = "Master_party/send_mail/".$this->uri->segment(3);
		$this->load->view('includes/template',$this->data);
	}
